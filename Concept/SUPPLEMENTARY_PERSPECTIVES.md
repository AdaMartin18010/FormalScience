...
C = ∀node: ⟦state⟧_node₁ ≡ ⟦state⟧_node₂
    语义域完全同步

A = ∀request: ∃response within timeout
    语法规则总是可应用

P = Network_Partition ⇒ System_Continue
    即使语法传播受阻，系统继续运作

【Eventual Consistency（最终一致性）】：
  ⟦state⟧ₜ₁ ≠ ⟦state⟧ₜ₂  （短期不一致）
  但 lim_{t→∞} ⟦state⟧ₜ → ⟦state⟧_final

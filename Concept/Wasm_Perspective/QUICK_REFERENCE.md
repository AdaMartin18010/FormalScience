# WebAssembly 快速参考 (Quick Reference)

## 📋 目录

- [WebAssembly 快速参考 (Quick Reference)](#webassembly-快速参考-quick-reference)
  - [📋 目录](#-目录)
  - [1 核心命题速查](#1-核心命题速查)
    - [1.1 确定性](#11-确定性)
    - [1.2 类型安全](#12-类型安全)
    - [1.3 验证复杂度](#13-验证复杂度)
    - [1.4 性能界限](#14-性能界限)
    - [1.5 沙箱隔离](#15-沙箱隔离)
  - [2 类型系统速查](#2-类型系统速查)
    - [2.1 值类型](#21-值类型)
    - [2.2 函数类型](#22-函数类型)
    - [2.3 类型判断](#23-类型判断)
  - [3 指令集速查](#3-指令集速查)
    - [3.1 算术指令](#31-算术指令)
    - [3.2 控制流](#32-控制流)
    - [3.3 内存指令](#33-内存指令)
    - [3.4 SIMD 指令](#34-simd-指令)
  - [4 性能数据速查](#4-性能数据速查)
    - [4.1 编译策略](#41-编译策略)
    - [4.2 指令延迟Skylake cycles](#42-指令延迟skylake-cycles)
    - [4.3 开销](#43-开销)
  - [5 安全性速查](#5-安全性速查)
    - [5.1 已防御](#51-已防御)
    - [5.2 未完全防御](#52-未完全防御)
  - [6 应用场景速查](#6-应用场景速查)
    - [6.1 强适配](#61-强适配)
    - [6.2 不适配](#62-不适配)
  - [7 命令速查](#7-命令速查)
    - [7.1 编译](#71-编译)
    - [7.2 运行](#72-运行)
    - [7.3 分析](#73-分析)
  - [8 关键公式速查](#8-关键公式速查)
    - [8.1 ROI 模型](#81-roi-模型)
    - [8.2 盈亏平衡点](#82-盈亏平衡点)
    - [8.3 成本函数](#83-成本函数)
    - [8.4 加速比](#84-加速比)
  - [9 工具链速查](#9-工具链速查)
    - [9.1 运行时](#91-运行时)
    - [9.2 编译器](#92-编译器)
    - [9.3 验证工具](#93-验证工具)
  - [10 常见陷阱速查](#10-常见陷阱速查)
    - [10.1 除零](#101-除零)
    - [10.2 越界](#102-越界)
    - [10.3 类型不匹配](#103-类型不匹配)
    - [10.4 整数溢出](#104-整数溢出)
  - [11 优化技巧速查](#11-优化技巧速查)
    - [11.1 编译器优化](#111-编译器优化)
    - [11.2 算法优化](#112-算法优化)
    - [11.3 部署优化](#113-部署优化)
  - [12 批判性检查清单](#12-批判性检查清单)
    - [12.1 性能预期](#121-性能预期)
    - [12.2 安全预期](#122-安全预期)
    - [12.3 生态预期](#123-生态预期)
  - [13 延伸阅读](#13-延伸阅读)
    - [13.1 理论](#131-理论)
    - [13.2 实践](#132-实践)
    - [13.3 批判](#133-批判)

---

## 1 核心命题速查

### 1.1 确定性

\[
\forall M, S : \llbracket M \rrbracket(S) \text{ 唯一确定}
\]

### 1.2 类型安全

\[
\text{Validate}(M) \implies \text{Progress}(M) \wedge \text{Preservation}(M)
\]

### 1.3 验证复杂度

\[
T(\text{Validate}) = O(|M|)
\]

### 1.4 性能界限

\[
\text{Perf}_{\text{wasm}} \in [0.85, 0.95] \times \text{Perf}_{\text{native}}
\]

### 1.5 沙箱隔离

\[
\text{LinearMemory}_{\text{wasm}} \cap \text{HostMemory} = \emptyset
\]

---

## 2 类型系统速查

### 2.1 值类型

```text
i32 | i64 | f32 | f64 | v128
```

### 2.2 函数类型

```text
[t1*] -> [t2*]  // 多返回值已支持
```

### 2.3 类型判断

\[
C \vdash I : [t_1^_] \to [t_2^_]
\]

---

## 3 指令集速查

### 3.1 算术指令

```text
i32.add    [i32 i32] -> [i32]
i32.mul    [i32 i32] -> [i32]
i32.div_s  [i32 i32] -> [i32]  // 除零 trap
f32.sqrt   [f32] -> [f32]
```

### 3.2 控制流

```text
block [t*] ... end       [] -> [t*]
loop [t*] ... end        [] -> [t*]
if [t*] ... else ... end [i32] -> [t*]
br i                     [t1*, t*] -> [t2*]
call x                   [t1*] -> [t2*]
```

### 3.3 内存指令

```text
i32.load     [i32] -> [i32]
i32.store    [i32 i32] -> []
memory.size  [] -> [i32]
memory.grow  [i32] -> [i32]
```

### 3.4 SIMD 指令

```text
i32x4.add    [v128 v128] -> [v128]
f32x4.mul    [v128 v128] -> [v128]
i8x16.splat  [i32] -> [v128]
```

---

## 4 性能数据速查

### 4.1 编译策略

| 策略 | 启动 | 吞吐 |
|------|------|------|
| 解释器 | <1 ms | 1-10% |
| Baseline JIT | 5-10 ms | 50-70% |
| Optimizing JIT | 30-100 ms | 85-95% |
| AOT | 0 ms | 90-98% |

### 4.2 指令延迟Skylake cycles

| 指令 | 延迟 | 吞吐/周期 |
|------|------|----------|
| i32.add | 1 | 4 |
| i32.mul | 3 | 1 |
| f64.div | 4-13 | 1 |
| memory.load | 3-200 | 取决缓存 |

### 4.3 开销

```
验证：1-2 ms/MB
边界检查：5-10%
栈→寄存器：5-10%
总抽象税：5-15%
```

---

## 5 安全性速查

### 5.1 已防御

- ✅ 栈溢出 / ROP / JOP
- ✅ 堆溢出（边界检查）
- ✅ UAF（无指针）
- ✅ 类型混淆（运行时检查）

### 5.2 未完全防御

- ⚠️ Spectre（推测执行泄漏）
- ⚠️ 侧信道（时序泄漏）
- ⚠️ DoS（资源耗尽）
- ⚠️ 实现漏洞（JIT 编译器）

---

## 6 应用场景速查

### 6.1 强适配

- Serverless（冷启动 <5 ms）
- 边缘 AI（GPU 加速）
- IoT（差分 KB 级）
- 区块链（确定性）
- 插件系统（沙箱）

### 6.2 不适配

- DOM 密集操作
- 大堆（>4 GB，Memory64 改善中）
- 动态代码生成

---

## 7 命令速查

### 7.1 编译

```bash
# C/C++
emcc -O3 -s WASM=1 code.c -o code.wasm

# Rust
cargo build --target wasm32-wasi --release

# WAT -> WASM
wat2wasm code.wat -o code.wasm

# WASM -> WAT
wasm2wat code.wasm -o code.wat
```

### 7.2 运行

```bash
# Node.js
node --experimental-wasm-modules code.wasm

# WasmEdge
wasmedge code.wasm

# AOT 预编译
wasmedgec code.wasm code.so
wasmedge code.so
```

### 7.3 分析

```bash
# 反汇编
wasm-objdump -d code.wasm

# 验证
wasm-validate code.wasm

# 优化
wasm-opt -O3 code.wasm -o optimized.wasm
```

---

## 8 关键公式速查

### 8.1 ROI 模型

\[
\text{Savings} = \sum (\Delta \text{Traffic} + \Delta \text{Memory} + \Delta \text{DevOps})
\]

### 8.2 盈亏平衡点

```text
6 个月（基于 10k 边缘节点）
```

### 8.3 成本函数

\[
C_{\text{total}} = C_{\text{compute}} + C_{\text{network}} + C_{\text{storage}} + C_{\text{devops}}
\]

### 8.4 加速比

\[
\text{Speedup}_{\text{SIMD}} \approx \frac{\text{lanes}}{1.2} \approx 3-12×
\]

---

## 9 工具链速查

### 9.1 运行时

- WasmEdge（边缘优化）
- Wasmtime（功能完整）
- Wasmer（商业友好）
- WAMR（超轻量 <100 KB）

### 9.2 编译器

- Emscripten（C/C++）
- wasm-pack（Rust）
- AssemblyScript（TypeScript-like）
- TinyGo（Go 子集）

### 9.3 验证工具

- Isabelle/Wasm（定理证明）
- Wasm-Coq（Coq 机械化）
- K-Wasm（可执行语义）

---

## 10 常见陷阱速查

### 10.1 除零

```wasm
i32.const 0
i32.div_s  ;; trap!
```

### 10.2 越界

```wasm
i32.const 0xFFFFFFFF
i32.load   ;; trap!
```

### 10.3 类型不匹配

```wasm
(call_indirect (type $sig) (i32.const 0))
;; 若 table[0] 类型≠$sig，则 trap!
```

### 10.4 整数溢出

```wasm
i32.const 0x80000000  ;; INT_MIN
i32.const -1
i32.div_s             ;; trap!
```

---

## 11 优化技巧速查

### 11.1 编译器优化

```rust
// 启用 LTO
[profile.release]
lto = true

// 启用 SIMD
#[target_feature(enable = "simd128")]
```

### 11.2 算法优化

1. 减少边界检查（循环外提）
2. 内存对齐（64 字节）
3. 避免间接调用
4. SIMD 向量化

### 11.3 部署优化

- AOT 预编译（启动最快）
- 差分更新（流量最省）
- 多实例池化（密度最高）

---

## 12 批判性检查清单

### 12.1 性能预期

- ❓ 是否高估了性能？（抽象税 5-15%）
- ❓ 是否忽略了 JIT 预热？
- ❓ 是否考虑了内存访问模式？

### 12.2 安全预期

- ❓ 是否认为沙箱绝对安全？（Spectre 未防御）
- ❓ 是否依赖形式化验证？（实现仍可能有漏洞）
- ❓ 是否考虑了侧信道？（时序泄漏）

### 12.3 生态预期

- ❓ 是否高估了工具链成熟度？
- ❓ 是否忽略了 JS 互操作开销？
- ❓ 是否考虑了路径依赖？（JS 霸权）

---

## 13 延伸阅读

### 13.1 理论

- 01_Foundational_Theory（形式化语义、类型系统）
- 06_Philosophical_Foundations（可移植性理论）

### 13.2 实践

- 03_Runtime_Systems（编译策略）
- 04_System_Integration（WASI、内存共享）
- 05_Application_Patterns（Serverless、IoT）

### 13.3 批判

- 各章节"批判性分析"
- FAQ Q19-Q22

---

**使用建议**：

- 开发时：对照指令集和类型规则
- 优化时：参考性能数据和优化技巧
- 决策时：检查批判性清单

_本快速参考旨在提供"即查即用"的关键信息，同时保持批判性视角。_

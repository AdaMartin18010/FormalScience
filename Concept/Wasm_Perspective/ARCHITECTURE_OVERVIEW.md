- --
topic: "WebAssembly 架构概览"
dependencies: []
status: "review"
author: "FormalScience Project"
date: "2026-04-12"
version: "1.0.0"
tags: ["递归", "类型", "形式化", "计算", "逻辑"]
category: "reference"
priority: "medium"
- --
...
- [WebAssembly 架构概览](#webassembly-架构概览)
  - [1. 目录 {#目录}](#1-目录-目录)
  - [2. 概述 {#概述}](#2-概述-概述)
  - [1. 📐 抽象层级视图 {#-抽象层级视图}](#1--抽象层级视图--抽象层级视图)
  - [2. 🧩 核心组件架构 {#-核心组件架构}](#2--核心组件架构--核心组件架构)
    - [1 模块（Module） {#模块module}](#1-模块module-模块module)
    - [2 线性内存（Linear Memory） {#线性内存linear-memory}](#2-线性内存linear-memory-线性内存linear-memory)
    - [3 执行栈（Execution Stack） {#执行栈execution-stack}](#3-执行栈execution-stack-执行栈execution-stack)
    - [4 表（Table） {#表table}](#4-表table-表table)
    - [5 全局变量（Globals） {#全局变量globals}](#5-全局变量globals-全局变量globals)
  - [3. 🔄 执行模型 {#-执行模型}](#3--执行模型--执行模型)
    - [3 1 指令执行流程 {#1-指令执行流程}](#3-1-指令执行流程-1-指令执行流程)
    - [3 2 编译策略对比 {#2-编译策略对比}](#3-2-编译策略对比-2-编译策略对比)
  - [4. 🔗 互操作架构 {#-互操作架构}](#4--互操作架构--互操作架构)
    - [4 1 Wasm ↔ 宿主通信 {#1-wasm--宿主通信}](#4-1-wasm--宿主通信-1-wasm--宿主通信)
    - [4 2 跨边界调用开销 {#2-跨边界调用开销}](#4-2-跨边界调用开销-2-跨边界调用开销)
  - [5. 🔒 安全架构 {#-安全架构}](#5--安全架构--安全架构)
    - [5 1 多层安全模型 {#1-多层安全模型}](#5-1-多层安全模型-1-多层安全模型)
    - [5 2 威胁模型 {#2-威胁模型}](#5-2-威胁模型-2-威胁模型)
  - [6. 📊 性能架构 {#-性能架构}](#6--性能架构--性能架构)
    - [6 1 性能瓶颈分析 {#1-性能瓶颈分析}](#6-1-性能瓶颈分析-1-性能瓶颈分析)
    - [6 2 优化策略 {#2-优化策略}](#6-2-优化策略-2-优化策略)
  - [7. 🏗️ 系统集成架构 {#-系统集成架构}](#7-️-系统集成架构--系统集成架构)
    - [7 1 典型部署架构：边缘 Serverless {#1-典型部署架构边缘-serverless}](#7-1-典型部署架构边缘-serverless-1-典型部署架构边缘-serverless)
  - [8. 🔮 演化方向 {#-演化方向}](#8--演化方向--演化方向)
    - [8 1 提案阶段（Stage 4 → Stage 0） {#1-提案阶段stage-4--stage-0}](#8-1-提案阶段stage-4--stage-0-1-提案阶段stage-4--stage-0)
  - [9. 🎯 设计权衡 {#-设计权衡}](#9--设计权衡--设计权衡)
    - [9 1 核心决策及其影响 {#1-核心决策及其影响}](#9-1-核心决策及其影响-1-核心决策及其影响)
  - [10. 📖 参考架构文档 {#-参考架构文档}](#10--参考架构文档--参考架构文档)
...
### 2 线性内存（Linear Memory） {#线性内存linear-memory}

- _定义_*：连续字节数组，Wasm 模块的唯一可寻址内存
...
### 3 执行栈（Execution Stack） {#执行栈execution-stack}
...
### 7 1 典型部署架构：边缘 Serverless {#1-典型部署架构边缘-serverless}
...
### 8 1 提案阶段（Stage 4 → Stage 0） {#1-提案阶段stage-4--stage-0}

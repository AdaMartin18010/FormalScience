...
- [WebAssembly 最佳实践与常见陷阱](#webassembly-最佳实践与常见陷阱)
  ...
  - [8 调试最佳实践](#8-调试最佳实践)
    - [8.1 ✅ DO（推荐做法）](#81--do推荐做法)
      - [1 . 使用 Source Maps](#1--使用-source-maps)
      - [2 . 添加日志钩子](#2--添加日志钩子)
      - [3 . 使用断言](#3--使用断言)
    - [8.2 ❌ DON'T（避免做法）](#82--dont避免做法)
      - [1 . 避免在生产环境留下调试代码](#1--避免在生产环境留下调试代码)
  - [9 部署最佳实践](#9-部署最佳实践)
    - [9.1 ✅ DO（推荐做法）](#91--do推荐做法)
      - [1 . 启用压缩](#1--启用压缩)
      - [2 . 使用 CDN](#2--使用-cdn)
      - [3 . 版本化部署](#3--版本化部署)
      - [4 . 监控性能](#4--监控性能)

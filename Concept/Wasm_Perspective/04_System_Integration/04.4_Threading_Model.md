...
- [4.4 线程模型 (Threading Model)](#44-线程模型-threading-model)
  ...
  - [1 核心命题](#1-核心命题)
    - [1 主定理](#1-主定理)
  - [2 Web Workers 集成](#2-web-workers-集成)
    - [1 基本架构](#1-基本架构)
    - [2.2 启动开销](#22-启动开销)
  - [3 原子操作](#3-原子操作)
    - [1 原子指令集](#1-原子指令集)
    - [3.2 内存顺序](#32-内存顺序)
  - [4 同步原语](#4-同步原语)
    - [1 Futex（Fast Userspace Mutex）](#1-futexfast-userspace-mutex)
    - [4.2 互斥锁实现](#42-互斥锁实现)
    - [4.3 条件变量](#43-条件变量)
  - [5 线程安全性](#5-线程安全性)
    - [1 数据竞争检测](#1-数据竞争检测)
    - [5.2 False Sharing](#52-false-sharing)
    - [5.3 ABA 问题](#53-aba-问题)
  - [6 性能分析](#6-性能分析)
    - [1 原子操作开销](#1-原子操作开销)
    - [6.2 锁竞争分析](#62-锁竞争分析)
  - [7 批判性分析](#7-批判性分析)
    - [1 共享内存并发的根本困境](#1-共享内存并发的根本困境)
    - [7.2 Futex 的非可移植性](#72-futex-的非可移植性)
    - [7.3 性能的不确定性](#73-性能的不确定性)
  - [8 实践案例](#8-实践案例)
    - [1 案例1：并行排序](#1-案例1并行排序)
    - [8.2 案例2：生产者-消费者队列](#82-案例2生产者-消费者队列)
  - [参考文献](#参考文献)

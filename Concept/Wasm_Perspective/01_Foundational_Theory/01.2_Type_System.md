...
- [1.2 Type System](#12-type-system)
  - [1. 目录 {#目录}](#1-目录-目录)
  - [1. 核心命题 {#核心命题}](#1-核心命题-核心命题)
    - [1 主定理（类型安全性） {#主定理类型安全性}](#1-主定理类型安全性-主定理类型安全性)
  - [2. 类型语法 {#类型语法}](#2-类型语法-类型语法)
    - [1 值类型 (Value Types) {#值类型-value-types}](#1-值类型-value-types-值类型-value-types)
    - [2 2 函数类型 (Function Types) {#2-函数类型-function-types}](#2-2-函数类型-function-types-2-函数类型-function-types)
    - [2 3 限制类型 (Limits) {#3-限制类型-limits}](#2-3-限制类型-limits-3-限制类型-limits)
  - [3. 类型规则 {#类型规则}](#3-类型规则-类型规则)
    - [1 指令类型 {#指令类型}](#1-指令类型-指令类型)
    - [3 2 核心规则 {#2-核心规则}](#3-2-核心规则-2-核心规则)
    - [3 3 内存指令 {#3-内存指令}](#3-3-内存指令-3-内存指令)
  - [4. 类型安全性 {#类型安全性}](#4-类型安全性-类型安全性)
    - [1 证明框架 {#证明框架}](#1-证明框架-证明框架)
    - [4 2 Progress 证明 {#2-progress-证明}](#4-2-progress-证明-2-progress-证明)
    - [4 3 Preservation 证明 {#3-preservation-证明}](#4-3-preservation-证明-3-preservation-证明)
    - [4 4 类型擦除 {#4-类型擦除}](#4-4-类型擦除-4-类型擦除)
  - [5. 类型推导 {#类型推导}](#5-类型推导-类型推导)
    - [1 栈类型推导算法 {#栈类型推导算法}](#1-栈类型推导算法-栈类型推导算法)
    - [5 2 类型检查器的正确性 {#2-类型检查器的正确性}](#5-2-类型检查器的正确性-2-类型检查器的正确性)
  - [6. 子类型与多态 {#子类型与多态}](#6-子类型与多态-子类型与多态)
    - [1 栈多态性 {#栈多态性}](#1-栈多态性-栈多态性)
    - [6 2 引用类型（Reference Types Proposal） {#2-引用类型reference-types-proposal}](#6-2-引用类型reference-types-proposal-2-引用类型reference-types-proposal)
  - [7. 批判性分析 {#批判性分析}](#7-批判性分析-批判性分析)
    - [1 类型系统的局限 {#类型系统的局限}](#1-类型系统的局限-类型系统的局限)
    - [7 2 类型擦除的哲学困境 {#2-类型擦除的哲学困境}](#7-2-类型擦除的哲学困境-2-类型擦除的哲学困境)
    - [7 3 标准化的权衡 {#3-标准化的权衡}](#7-3-标准化的权衡-3-标准化的权衡)
  - [8. 案例研究：`select` 类型多态 {#案例研究select-类型多态}](#8-案例研究select-类型多态-案例研究select-类型多态)
    - [1 问题 {#问题}](#1-问题-问题)
    - [8 2 类型规则 {#2-类型规则}](#8-2-类型规则-2-类型规则)
    - [8 3 实现挑战 {#3-实现挑战}](#8-3-实现挑战-3-实现挑战)
  - [9. 形式化验证工具 {#形式化验证工具}](#9-形式化验证工具-形式化验证工具)
...
### 1 值类型 (Value Types) {#值类型-value-types}
...
### 2 2 函数类型 (Function Types) {#2-函数类型-function-types}
...
### 4 3 Preservation 证明 {#3-preservation-证明}
...
- _输入_*：指令序列 \( I^* \)，上下文 \( C \)
- _输出_*：类型 \( [t_1^_] \to [t_2^_] \) 或 type error
...
### 6 2 引用类型（Reference Types Proposal） {#2-引用类型reference-types-proposal}
...
## 8. 案例研究：`select` 类型多态 {#案例研究select-类型多态}

...
- [1.4 Execution Model](#14-execution-model)
  ...
  - [3 确定性保证](#3-确定性保证)
    - [1 浮点确定性](#1-浮点确定性)
    - [3.2 内存模型确定性](#32-内存模型确定性)
    - [3.3 指令周期可预测性](#33-指令周期可预测性)
  - [4 并发模型](#4-并发模型)
    - [1 线程抽象](#1-线程抽象)
    - [4.2 内存顺序](#42-内存顺序)
    - [4.3 数据竞争](#43-数据竞争)
  - [5 性能模型](#5-性能模型)
    - [1 成本模型](#1-成本模型)
    - [5.2 性能预测模型](#52-性能预测模型)
  - [6 陷阱机制](#6-陷阱机制)
    - [1 陷阱分类](#1-陷阱分类)
  - [7 栈式执行的形式化](#7-栈式执行的形式化)
    - [1 栈机器 vs 寄存器机器](#1-栈机器-vs-寄存器机器)
  - [8 执行策略](#8-执行策略)
    - [1 解释执行](#1-解释执行)
    - [8.2 基线 JIT（Baseline Compilation）](#82-基线-jitbaseline-compilation)
    - [8.3 优化 JIT（Optimizing Compilation）](#83-优化-jitoptimizing-compilation)
    - [8.4 AOT（Ahead-of-Time Compilation）](#84-aotahead-of-time-compilation)
  - [9 批判性分析](#9-批判性分析)
    - [1 抽象税（Abstraction Tax）](#1-抽象税abstraction-tax)
    - [9.2 确定性的幻觉](#92-确定性的幻觉)
    - [9.3 并发模型的复杂性](#93-并发模型的复杂性)
  - [10 案例研究：循环展开的正确性](#10-案例研究循环展开的正确性)
    - [1 源代码](#1-源代码)
    - [10.2 展开后](#102-展开后)
    - [10.3 等价性证明](#103-等价性证明)
  - [参考文献](#参考文献)
...
### 8.2 基线 JIT（Baseline Compilation）
...
### 8.3 优化 JIT（Optimizing Compilation）
...
### 8.4 AOT（Ahead-of-Time Compilation）

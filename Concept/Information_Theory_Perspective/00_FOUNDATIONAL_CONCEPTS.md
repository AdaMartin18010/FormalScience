...
## 1. 信息论基础概念 {#信息论基础概念} {#信息论基础概念-信息论基础概念}

### 1 1 信息熵 (Shannon Entropy) {#1-信息熵-shannon-entropy} {#1-信息熵-shannon-entropy-1-信息熵-sh}

### 3 2 # 1.1.1 权威定义 {#-111-权威定义} {#2--111-权威定义--111-权威定义}
...
### 3 3 # 1.1.2 数学定义 {#-112-数学定义} {#3--112-数学定义--112-数学定义}
...
### 3 4 # 1.1.3 连续情况 {#-113-连续情况} {#4--113-连续情况--113-连续情况}

对于连续随机变量 $X$，其概率密度函数为 $f(x)$，微分熵定义为：

$$h(X) = -\int_{-\infty}^{\infty} f(x) \log_2 f(x) dx$$

### 3 5 # 1.1.4 性质 {#-114-性质} {#5--114-性质--114-性质}
...
### 1 2 互信息 (Mutual Information) {#2-互信息-mutual-information} {#2-互信息-mutual-information-2-互信息}

### 3 7 # 1.2.1 权威定义 {#-121-权威定义} {#7--121-权威定义--121-权威定义}

根据[Wikipedia互信息条目](https://en.wikipedia.org/wiki/Mutual_information)：

- *互信息**是度量两个随机变量之间相互依赖程度的量，表示一个变量包含关于另一个变量的信息量。

### 3 8 # 1.2.2 数学定义 {#-122-数学定义} {#8--122-数学定义--122-数学定义}
...
### 3 9 # 1.2.3 等价定义 {#-123-等价定义} {#9--123-等价定义--123-等价定义}

$$I(X;Y) = H(X) - H(X|Y) = H(Y) - H(Y|X) = H(X) + H(Y) - H(X,Y)$$

### 3 10 # 1.2.3 性质 {#-123-性质} {#10--123-性质--123-性质}
...
### 1 3 信道容量 (Channel Capacity) {#3-信道容量-channel-capacity} {#3-信道容量-channel-capacity-3-信道容量}

### 3 12 # 1.3.1 权威定义 {#-131-权威定义} {#12--131-权威定义--131-权威定义}

根据[Wikipedia信道容量条目](https://en.wikipedia.org/wiki/Channel_capacity)：

- *信道容量**是信息论中信道能够可靠传输信息的最大速率。

### 3 13 # 1.3.2 数学定义 {#-132-数学定义} {#13--132-数学定义--132-数学定义}
...
### 3 14 # 1.3.3 香农信道编码定理 {#-133-香农信道编码定理} {#14--133-香农信道编码定理--133-香农信道编码定理}

对于任何码率 $R < C$，存在码长为 $n$ 的码，使得错误概率 $P_e^{(n)} \to 0$ 当 $n \to \infty$。

对于任何码率 $R > C$，所有码的错误概率 $P_e^{(n)} \to 1$ 当 $n \to \infty$。

### 1 4 率失真理论 (Rate-Distortion Theory) {#4-率失真理论-rate-distortion-theory} {#4-率失真理论-rate-distortion-theory}

### 3 16 # 1.4.1 权威定义 {#-141-权威定义} {#16--141-权威定义--141-权威定义}

根据Cover & Thomas (2006)：

- *率失真理论**研究在给定失真约束下，信源编码的最小码率。

### 3 17 # 1.4.2 数学定义 {#-142-数学定义} {#17--142-数学定义--142-数学定义}
...
## 2. 语义信息论扩展 {#语义信息论扩展} {#语义信息论扩展-语义信息论扩展}

### 2 1 语义熵 (Semantic Entropy) {#1-语义熵-semantic-entropy} {#1-语义熵-semantic-entropy-1-语义熵-s}

### 4 2 # 2.1.1 定义 {#-211-定义} {#2--211-定义--211-定义}
...
### 4 3 # 2.1.2 性质 {#-212-性质} {#3--212-性质--212-性质}

1. **语义敏感性**：$H_s(X) \leq H(X)$
2. **权重依赖性**：语义熵依赖于权重函数的选择
3. **上下文相关性**：语义权重可能依赖于上下文

### 2 2 语义互信息 (Semantic Mutual Information) {#2-语义互信息-semantic-mutual-inform} {#2-语义互信息-semantic-mutual-inform}

### 4 5 # 2.2.1 定义 {#-221-定义} {#5--221-定义--221-定义}
...
### 2 3 DIKWP模型形式化 {#3-dikwp模型形式化} {#3-dikwp模型形式化-3-dikwp模型形式化}

### 4 7 # 2.3.1 五层结构 {#-231-五层结构} {#7--231-五层结构--231-五层结构}
...
### 4 8 # 2.3.2 语义转换函数 {#-232-语义转换函数} {#8--232-语义转换函数--232-语义转换函数}
...
## 3. 多视角信息论 {#多视角信息论} {#多视角信息论-多视角信息论}

### 3 1 工程-通信视角 {#1-工程-通信视角} {#1-工程-通信视角-1-工程-通信视角}

### 5 2 # 3.1.1 核心概念 {#-311-核心概念} {#2--311-核心概念--311-核心概念}

- **信息** = 不确定性的减少量
- **核心量**：熵 $H(X)$、信道容量 $C = \max I(X;Y)$
- **工具**：AWGN容量公式 $C = B \log_2(1 + SNR)$

### 5 3 # 3.1.2 应用 {#-312-应用} {#3--312-应用--312-应用}

- 信道编码设计
- 通信系统优化
- 错误控制编码

### 3 2 统计-推断视角 {#2-统计-推断视角} {#2-统计-推断视角-2-统计-推断视角}

### 5 5 # 3.2.1 核心概念 {#-321-核心概念} {#5--321-核心概念--321-核心概念}

- **信息** = 区分分布的"距离"
- **核心量**：KL散度 $D(p||q)$、Fisher信息 $I(\theta)$
- **工具**：最大熵原理、信息投影

### 5 6 # 3.2.2 应用 {#-322-应用} {#6--322-应用--322-应用}

- 参数估计
- 假设检验
- 贝叶斯推断

### 3 3 编码-压缩视角 {#3-编码-压缩视角} {#3-编码-压缩视角-3-编码-压缩视角}

### 5 8 # 3.3.1 核心概念 {#-331-核心概念} {#8--331-核心概念--331-核心概念}

- **信息** = 最短可解码码字长度
- **核心量**：期望码长 $\bar{L} \geq H(X)$
- **工具**：算术编码、范围编码、ANS

### 5 9 # 3.3.2 应用 {#-332-应用} {#9--332-应用--332-应用}

- 数据压缩
- 无损编码
- 有损压缩

### 3 4 算法-复杂度视角 {#4-算法-复杂度视角} {#4-算法-复杂度视角-4-算法-复杂度视角}

### 5 11 # 3.4.1 核心概念 {#-341-核心概念} {#11--341-核心概念--341-核心概念}

- **信息** = 生成数据的最短程序长度 $K(x)$
- **核心量**：条件复杂度 $K(x|y)$、互信息 $I_K(x;y) = K(x) + K(y) - K(x,y)$
- **工具**：不可压缩性法证、MDL

### 5 12 # 3.4.2 应用 {#-342-应用} {#12--342-应用--342-应用}

- 算法复杂度分析
- 随机性检测
- 模型选择

### 3 5 热力学-统计物理视角 {#5-热力学-统计物理视角} {#5-热力学-统计物理视角-5-热力学-统计物理视角}

### 5 14 # 3.5.1 . 核心概念 {#-351--核心概念} {#14--351--核心概念--351--核心概念}

- **信息** = 物理态的负熵 $-\Delta S$
- **核心量**：Landauer界限 $kT \ln 2$/擦除bit
- **工具**：Szilárd引擎、Maxwell妖

### 5 15 # 3.5.2 . 应用 {#-352--应用} {#15--352--应用--352--应用}

- 计算热力学
- 信息热机
- 量子信息处理

### 3 6 几何-信息视角 {#6-几何-信息视角} {#6-几何-信息视角-6-几何-信息视角}

### 5 17 # 3.6.1 核心概念 {#-361-核心概念} {#17--361-核心概念--361-核心概念}

- **信息** = 流形上的"弧度"
- **核心量**：Fisher-Rao度量 $g_{ij}(\theta)$、信息体积
- **工具**：自然梯度、测地线

### 5 18 # 3.6.2 应用 {#-362-应用} {#18--362-应用--362-应用}

- 统计流形
- 信息几何
- 优化算法

### 3 7 语义-价值视角 {#7-语义-价值视角} {#7-语义-价值视角-7-语义-价值视角}

### 5 20 # 3.7.1 核心概念 {#-371-核心概念} {#20--371-核心概念--371-核心概念}

- **信息** = 减少语义不确定性的"意义单元"
- **核心量**：语义熵 $H_s$、语义互信息 $I_s$、目的函数 $P$
- **工具**：逻辑概率、语义信息不等式

### 5 21 # 3.7.2 应用 {#-372-应用} {#21--372-应用--372-应用}

- 自然语言处理
- 知识表示
- 人工智能

### 3 8 生物-进化视角 {#8-生物-进化视角} {#8-生物-进化视角-8-生物-进化视角}

### 5 23 # 3.8.1 核心概念 {#-381-核心概念} {#23--381-核心概念--381-核心概念}

- **信息** = 能被自然选择"看见"的遗传/表观差异
- **核心量**：选择信息 $I_{sel}$、有效信息 $EI$
- **工具**：数字生命、GWAS分析

### 5 24 # 3.8.2 应用 {#-382-应用} {#24--382-应用--382-应用}

- 进化生物学
- 基因组学
- 系统生物学

## 4. 形式化证明 {#形式化证明} {#形式化证明-形式化证明}

### 4 1 香农熵的极值性质 {#1-香农熵的极值性质} {#1-香农熵的极值性质-1-香农熵的极值性质}

- _定理_*：对于 $|\mathcal{X}| = n$，$H(X) \leq \log_2 n$，等号成立当且仅当 $X$ 是均匀分布。
...
### 4 2 互信息的非负性 {#2-互信息的非负性} {#2-互信息的非负性-2-互信息的非负性}

- _定理_*：$I(X;Y) \geq 0$，等号成立当且仅当 $X$ 和 $Y$ 独立。
...
### 4 3 数据处理不等式 {#3-数据处理不等式} {#3-数据处理不等式-3-数据处理不等式}

- _定理_*：对于马尔可夫链 $X \to Y \to Z$，有 $I(X;Y) \geq I(X;Z)$。
...
## 5. 实际应用 {#实际应用} {#实际应用-实际应用}

### 5 1 通信系统设计 {#1-通信系统设计} {#1-通信系统设计-1-通信系统设计}

### 7 2 # 5.1.1 信道编码 {#-511-信道编码} {#2--511-信道编码--511-信道编码}

- LDPC码：接近香农限的纠错码
- Polar码：5G标准采用的编码方案
- Turbo码：4G系统的基础编码

### 7 3 # 5.1.2 容量计算 {#-512-容量计算} {#3--512-容量计算--512-容量计算}

- AWGN信道：$C = B \log_2(1 + SNR)$
- 衰落信道：$C = \mathbb{E}[\log_2(1 + |h|^2 SNR)]$
- MIMO信道：$C = \log_2 \det(I + H H^H)$

### 5 2 数据压缩 {#2-数据压缩} {#2-数据压缩-2-数据压缩}

### 7 5 # 5.2.1 无损压缩 {#-521-无损压缩} {#5--521-无损压缩--521-无损压缩}

- Huffman编码：最优前缀码
- 算术编码：接近熵率的编码
- LZ系列：字典压缩算法

### 7 6 # 5.2.2 有损压缩 {#-522-有损压缩} {#6--522-有损压缩--522-有损压缩}

- JPEG：基于DCT的图像压缩
- MP3：基于心理声学的音频压缩
- H.264：基于运动估计的视频压缩

### 5 3 机器学习 {#3-机器学习} {#3-机器学习-3-机器学习}

### 7 8 # 5.3.1 特征选择 {#-531-特征选择} {#8--531-特征选择--531-特征选择}

- 互信息特征选择
- 信息增益
- 最大相关最小冗余

### 7 9 # 5.3.2 模型评估 {#-532-模型评估} {#9--532-模型评估--532-模型评估}

- 交叉熵损失
- KL散度
- 信息瓶颈

## 6. 前沿研究方向 {#前沿研究方向} {#前沿研究方向-前沿研究方向}

### 6 1 量子信息论 {#1-量子信息论} {#1-量子信息论-1-量子信息论}

- 量子熵
- 量子信道容量
- 量子纠错

### 6 2 网络信息论 {#2-网络信息论} {#2-网络信息论-2-网络信息论}

- 网络编码
- 分布式存储
- 多用户信道

### 6 3 语义信息论 {#3-语义信息论} {#3-语义信息论-3-语义信息论}

- 语义信道
- 语义容量
- 语义压缩

### 6 4 生物信息论 {#4-生物信息论} {#4-生物信息论-4-生物信息论}

- 基因组信息
- 蛋白质折叠
- 进化信息

## 7. 工具和软件 {#工具和软件} {#工具和软件-工具和软件}

### 7 1 数学软件 {#1-数学软件} {#1-数学软件-1-数学软件}

- MATLAB：信息论工具箱
- Python：scipy.stats, sklearn
- R：entropy, infotheo包

### 7 2 专业工具 {#2-专业工具} {#2-专业工具-2-专业工具}

- IT++：C++信息论库
- Communications Toolbox：MATLAB通信工具箱
- GNU Radio：软件无线电平台

### 7 3 可视化工具 {#3-可视化工具} {#3-可视化工具-3-可视化工具}

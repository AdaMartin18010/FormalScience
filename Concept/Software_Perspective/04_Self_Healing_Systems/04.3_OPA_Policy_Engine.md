...
- [4.3 OPA Policy Engine](#43-opa-policy-engine)
  - [📋 目录](#-目录)
  - [1 📊 核心概念深度分析](#1--核心概念深度分析)
  - [2 核心定义](#2-核心定义)
  - [3 为什么需要 OPA？](#3-为什么需要-opa)
    - [1 传统策略管理的问题](#1-传统策略管理的问题)
    - [3.2 OPA 解决方案](#32-opa-解决方案)
  - [4 OPA 架构](#4-opa-架构)
    - [1 部署模式](#1-部署模式)
      - [1 模式 1：Sidecar（微服务）](#1-模式-1sidecar微服务)
      - [2 模式 2：Gateway（集中式）](#2-模式-2gateway集中式)
      - [3 模式 3：Kubernetes Admission Controller](#3-模式-3kubernetes-admission-controller)
  - [5 Rego 语言深度解析](#5-rego-语言深度解析)
    - [1 核心概念](#1-核心概念)
      - [1 规则（Rules）](#1-规则rules)
      - [2 数据查询（Queries）](#2-数据查询queries)
      - [3 函数（Functions）](#3-函数functions)
    - [5.2 实战示例](#52-实战示例)
      - [1 示例 1：API 授权](#1-示例-1api-授权)
      - [2 示例 2：Kubernetes 策略（Gatekeeper）](#2-示例-2kubernetes-策略gatekeeper)
      - [3 示例 3：自愈决策](#3-示例-3自愈决策)
  - [6 OPA 集成方式](#6-opa-集成方式)
    - [1 HTTP API](#1-http-api)
    - [2 Go 库（嵌入式）](#2-go-库嵌入式)
    - [3 Envoy 外部授权](#3-envoy-外部授权)
  - [7 OPA Bundle（策略分发）](#7-opa-bundle策略分发)
    - [1 Bundle 结构](#1-bundle-结构)
    - [7.2 构建 Bundle](#72-构建-bundle)
    - [7.3 OPA 拉取 Bundle](#73-opa-拉取-bundle)
  - [8 OPA 性能优化](#8-opa-性能优化)
    - [1 编译策略](#1-编译策略)
    - [2 缓存决策结果](#2-缓存决策结果)
    - [3 部分评估（Partial Evaluation）](#3-部分评估partial-evaluation)
  - [9 OPA vs 其他策略引擎](#9-opa-vs-其他策略引擎)
  - [10 关键洞察](#10-关键洞察)
    - [1 洞察 1：策略即代码 = 可测试](#1-洞察-1策略即代码--可测试)
    - [10.2 洞察 2：声明式 \> 命令式](#102-洞察-2声明式--命令式)
    - [10.3 洞察 3：集中策略 = 安全保障](#103-洞察-3集中策略--安全保障)
  - [11 权威参考与标准 | Authoritative References](#11-权威参考与标准--authoritative-references)
    - [1 官方文档与规范](#1-官方文档与规范)
    - [11.2 CNCF生态系统](#112-cncf生态系统)
    - [11.3 学术与工业白皮书](#113-学术与工业白皮书)
    - [11.4 行业最佳实践](#114-行业最佳实践)
    - [11.5 标准与规范](#115-标准与规范)
    - [11.6 开源生态系统](#116-开源生态系统)
    - [11.7 权威书籍与教程](#117-权威书籍与教程)
    - [11.8 会议演讲与教程](#118-会议演讲与教程)
    - [11.9 比较与替代方案](#119-比较与替代方案)
    - [11.10 在线资源](#1110-在线资源)
    - [11.11 开源贡献统计](#1111-开源贡献统计)
...
#### 3 模式 3：Kubernetes Admission Controller
...
#### 2 示例 2：Kubernetes 策略（Gatekeeper）
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [Chomsky层级](../../CONCEPT_CROSS_INDEX.md#51-chomsky层级-chomsky-hierarchy-七视角) - Rego语言的形式语言基础
  - [反身性](../../CONCEPT_CROSS_INDEX.md#31-反身性-reflexivity-七视角) - 策略引擎在元层级操作自身的能力
  - [Gödel不完备定理](../../CONCEPT_CROSS_INDEX.md#101-gödel不完备定理-gödels-incompleteness-theorems-七视角) - 策略验证的理论限制

...
- [4.2 OTLP Observability](#42-otlp-observability)
  - [📋 目录](#-目录)
  - [1 📊 核心概念深度分析](#1--核心概念深度分析)
  - [2 核心定义](#2-核心定义)
  - [3 为什么需要 OTLP？](#3-为什么需要-otlp)
    - [1 传统可观测性的碎片化](#1-传统可观测性的碎片化)
    - [3.2 OTLP 解决方案](#32-otlp-解决方案)
  - [4 OTLP 架构](#4-otlp-架构)
    - [1 完整数据流](#1-完整数据流)
    - [4.2 三大信号类型](#42-三大信号类型)
      - [1 Metrics（指标）](#1-metrics指标)
      - [2 Traces（追踪）](#2-traces追踪)
      - [3 Logs（日志）](#3-logs日志)
  - [5 OTLP Collector 深度解析](#5-otlp-collector-深度解析)
    - [1 三大组件](#1-三大组件)
      - [1 Receivers（接收器）](#1-receivers接收器)
      - [2 Processors（处理器）](#2-processors处理器)
      - [3 Exporters（导出器）](#3-exporters导出器)
    - [5.2 Pipeline 配置](#52-pipeline-配置)
  - [6 自动埋点 vs 手动埋点](#6-自动埋点-vs-手动埋点)
    - [1 自动埋点（Auto-Instrumentation）](#1-自动埋点auto-instrumentation)
    - [6.2 手动埋点（Manual Instrumentation）](#62-手动埋点manual-instrumentation)
  - [7 关联查询：从 Metrics 到 Traces 到 Logs](#7-关联查询从-metrics-到-traces-到-logs)
    - [1 典型场景](#1-典型场景)
      - [1 步骤 1：Metrics 定位时间和服务](#1-步骤-1metrics-定位时间和服务)
      - [2 步骤 2：Traces 定位慢请求](#2-步骤-2traces-定位慢请求)
      - [3 步骤 3：Logs 定位根因](#3-步骤-3logs-定位根因)
    - [7.2 自动关联配置](#72-自动关联配置)
  - [8 性能优化](#8-性能优化)
    - [1 采样策略](#1-采样策略)
    - [2 批处理](#2-批处理)
    - [3 属性限制](#3-属性限制)
  - [9 成本分析](#9-成本分析)
    ...
    - [9.2 成本优化](#92-成本优化)
  - [10 关键洞察](#10-关键洞察)
    - [1 洞察 1：统一 \> 完美](#1-洞察-1统一--完美)
    - [10.2 洞察 2：自动埋点覆盖 80%](#102-洞察-2自动埋点覆盖-80)
    - [10.3 洞察 3：关联是核心价值](#103-洞察-3关联是核心价值)
  - [11 权威参考与标准 | Authoritative References](#11-权威参考与标准--authoritative-references)
    - [1 官方标准与规范](#1-官方标准与规范)
    - [11.2 CNCF官方文档](#112-cncf官方文档)
    - [11.3 学术研究](#113-学术研究)
    - [11.4 工业界白皮书](#114-工业界白皮书)
    - [11.5 行业标准与实践](#115-行业标准与实践)
    - [11.6 RFC与协议](#116-rfc与协议)
    - [11.7 开源实现与工具](#117-开源实现与工具)
    - [11.8 大学课程](#118-大学课程)
    - [11.9 在线资源](#119-在线资源)
    - [11.10 性能基准与案例](#1110-性能基准与案例)
    - [11.11 最新发展（2024-2025）](#1111-最新发展2024-2025)
    - [11.12 验证与采用统计（截至2025-10-27）](#1112-验证与采用统计截至2025-10-27)
  - [12 相关主题](#12-相关主题)
...
<details>
<summary><b>📡👁️ 点击展开：OTLP统一可观测性核心洞察</b></summary>
...
### 6.2 手动埋点（Manual Instrumentation）
...
## 7 关联查询：从 Metrics 到 Traces 到 Logs

### 1 典型场景

**问题**：P95 延迟飙升到 5 秒

#### 1 步骤 1：Metrics 定位时间和服务
...
7. **Sigelman, B. H., et al. (2010)**. "Dapper, a Large-Scale Distributed Systems Tracing Infrastructure". _Google Technical Report_.
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [熵](../../CONCEPT_CROSS_INDEX.md#71-熵-entropy-七视角) - OTLP数据中的信息不确定性度量
  - [互信息](../../CONCEPT_CROSS_INDEX.md#111-互信息-mutual-information-七视角) - Metrics/Logs/Traces之间的信息关联
  - [通信复杂度](../../CONCEPT_CROSS_INDEX.md#56-通信复杂度-communication-complexity-七视角) - OTLP数据传输的成本

# 9.8 Case Study Flash Sale System
...
- [9.8 Case Study Flash Sale System](#98-case-study-flash-sale-system)
  - [📋 目录](#-目录)
  ...
  - [4 核心技术方案](#4-核心技术方案)
    ...
    - [2 多级限流](#2-多级限流)
      - [1 Level 1：Nginx限流](#1-level-1nginx限流)
      - [2 Level 2：Redis令牌桶限流](#2-level-2redis令牌桶限流)
  ...
  - [7 成本分析](#7-成本分析)
    - [1 资源配置](#1-资源配置)
  - [8 关键洞察](#8-关键洞察)
    - [1 洞察 1：拦截在前，处理在后](#1-洞察-1拦截在前处理在后)
    - [8.2 洞察 2：空间换时间](#82-洞察-2空间换时间)
    - [8.3 洞察 3：异步削峰](#83-洞察-3异步削峰)
  - [9 相关主题](#9-相关主题)
    - [9.1 跨视角链接](#91-跨视角链接)
    - [9.2 相关文档](#92-相关文档)
  - [10 附录：压测脚本](#10-附录压测脚本)
...
<details>
<summary><b>🔥🚀 点击展开：秒杀系统核心洞察</b></summary>

**终极洞察**: 秒杀系统=高并发+超卖防护+系统隔离的三重挑战。场景：双11秒杀（100万人抢100件商品，峰值10万QPS，成功率<0.01%）。核心挑战：①高并发：瞬时流量冲击②超卖问题：库存一致性③系统稳定性：不影响正常业务④用户体验：快速响应。架构设计：①CDN静态资源②Nginx限流（令牌桶）③Redis预热库存（原子扣减DECR）④消息队列异步处理（削峰填谷）⑤分布式锁（防超卖）⑥服务隔离（单独集群）⑦降级兜底（售罄提示）。关键技术：①前端防刷：按钮置灰、验证码②缓存预热：提前加载到Redis③原子操作：DECR保证原子性④异步化：MQ削峰⑤限流降级：Sentinel/Hystrix⑥监控告警：实时Metrics。压测验证：①基准测试：单机QPS②容量规划：集群规模③全链路压测：模拟真实流量。关键经验：①流量层层过滤（CDN→限流→缓存→队列）②最小化DB压力③快速失败优雅降级。
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [CAP定理](../../CONCEPT_CROSS_INDEX.md#53-cap定理-cap-theorem-七视角) - 秒杀系统中的一致性、可用性、分区容错权衡
  - [一致性模型详解](../../CONCEPT_CROSS_INDEX.md#55-一致性模型详解-consistency-models-七视角) - 库存一致性的保证机制
  - [通信复杂度](../../CONCEPT_CROSS_INDEX.md#56-通信复杂度-communication-complexity-七视角) - 高并发场景下的通信成本

# 3.5 External Memory Algorithms
...
<details>
<summary><b>💾🔬 点击展开：外部存储算法核心洞察</b></summary>

**终极洞察**: 大数据算法=I/O优先的世界。当n≫M（内存大小）时，磁盘I/O成为瓶颈（比CPU慢10⁶倍）。DAM模型（Disk Access Machine）：参数(M,B)，内存M字，块大小B字，每次I/O传输B字，目标：最小化I/O次数而非CPU操作。经典算法：①外部归并排序：Θ((n/B)·log_{M/B}(n/M)) I/Os，M=B²时达到最优②B-树：高度O(log_B n)，每次查询O(log_B n) I/Os③外部哈希：线性扫描+分桶，O(n/B) I/Os。Cache-Oblivious算法：不需要知道M和B的参数，自动适应所有层次缓存（L1/L2/L3/RAM/SSD），如Funnelsort、Cache-Oblivious B-Tree。I/O下界：排序Ω((n/B)·log_{M/B}(n/M))，矩阵乘法Ω(n³/B√M)。实际应用：数据库查询、MapReduce、大规模图算法（外部BFS）。SSD vs HDD：SSD随机读写接近顺序，改变算法设计。关键：外部存储算法将I/O-aware设计提升到理论高度，是大数据处理的基础。
...
- [3.5 External Memory Algorithms](#35-external-memory-algorithms)
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [熵](../../CONCEPT_CROSS_INDEX.md#71-熵-entropy-七视角) - 外部存储算法中的信息不确定性
  - [通信复杂度](../../CONCEPT_CROSS_INDEX.md#56-通信复杂度-communication-complexity-七视角) - I/O复杂度与通信复杂度的关系
  - [并行复杂度类](../../CONCEPT_CROSS_INDEX.md#54-并行复杂度类-nc-p-完全性-七视角) - 外部存储并行算法的复杂度

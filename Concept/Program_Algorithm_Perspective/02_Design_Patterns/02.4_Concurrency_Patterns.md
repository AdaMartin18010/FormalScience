...
<details>
<summary><b>⚡🔬 点击展开：并发模式核心洞察</b></summary>

**终极洞察**: 并发编程=非确定性的驯服之道。三大范式：①消息传递（Message Passing）：Actor模型（Erlang/Akka）+CSP（Golang）+π-演算（理论基础），进程通过消息通信，无共享状态，天然避免数据竞争②共享内存（Shared Memory）：Mutex（互斥）+RwLock（读写锁）+Atomic（原子操作）+STM（软件事务内存），需要精心设计同步原语避免死锁/竞态③混合模型：Rust（Send/Sync trait+所有权），编译期保证线程安全。形式化工具：①进程代数（CCS/CSP/π-演算）：建模并发交互②时序逻辑（LTL/CTL）：验证安全性（"坏事不发生"）和活性（"好事终将发生"）③分离逻辑（Concurrent Separation Logic）：推理共享状态④Iris逻辑：Rust并发验证基础。经典问题：哲学家就餐、读者写者、生产者消费者。验证技术：模型检测（TLA+/SPIN）、定理证明（Coq/Iris）、动态分析（ThreadSanitizer/Loom）。关键：并发模式通过结构化并发消除非确定性带来的混乱。
...
- [2.4 Concurrency Patterns](#24-concurrency-patterns)
  ...
  - [7 并发模式的典型问题](#7-并发模式的典型问题)
    - [7.1 哲学家就餐问题](#71-哲学家就餐问题)
      - [1 CSP 解法](#1-csp-解法)
      - [2 Golang 实现（资源排序）](#2-golang-实现资源排序)
    - [7.2 生产者-消费者](#72-生产者-消费者)
      - [1 Actor 实现](#1-actor-实现)
      - [2 Golang 实现（通道）](#2-golang-实现通道)
  - [8 大学课程对应](#8-大学课程对应)
  - [9 教材对应](#9-教材对应)
  - [10 本地项目引用](#10-本地项目引用)
  - [11 总结](#11-总结)
    - [1 核心洞察](#1-核心洞察)
    - [11.2 工具链推荐](#112-工具链推荐)
    - [11.3 实战建议](#113-实战建议)
  - [12 附录 A：并发原语对比](#12-附录-a并发原语对比)
  - [13 附录 B：Wikipedia 概念对照](#13-附录-bwikipedia-概念对照)
  - [跨视角链接](#跨视角链接)
...
| **Mutex** | 是 | 数据待补充 | 临界区保护 |
| **RwLock** | 是 | 数据待补充 | 读多写少 |
| **Atomic** | 否 | 数据待补充 | 无锁计数器 |

---

## 13 附录 B：Wikipedia 概念对照
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [隔离](../../CONCEPT_CROSS_INDEX.md#112-隔离-isolation-七视角) - 并发模式中的隔离机制（Actor、进程隔离）
  - [零开销隔离](../../CONCEPT_CROSS_INDEX.md#174-零开销隔离-zero-overhead-isolation-七视角) - Rust并发模型的编译期隔离
  - [通信复杂度](../../CONCEPT_CROSS_INDEX.md#56-通信复杂度-communication-complexity-七视角) - 并发模式中的消息传递成本

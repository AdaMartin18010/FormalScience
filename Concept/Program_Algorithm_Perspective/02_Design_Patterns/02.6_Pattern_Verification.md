...
<details>
<summary><b>🔍🔬 点击展开：模式验证核心洞察</b></summary>

**终极洞察**: 模式验证=架构守护的自动化。三层验证体系：①静态架构验证（轻量级）：ArchUnit/NDepend/Struktur，在代码层面检查依赖规则、分层约束、命名约定，快速反馈但表达力有限②模型检测（中等强度）：mCRL2/SPIN/TLA+，建立抽象模型验证并发性质（无死锁/活性/公平性），能发现设计缺陷但需要建模③定理证明（最强保证）：Coq/Lean/Isabelle，形式化证明模式性质（Singleton唯一性/Observer一致性/Strategy可替换性），完全正确但成本高。验证维度：①结构正确性：依赖关系、接口契约②行为正确性：状态机性质、时序约束③性能正确性：实时性、资源界限。工具选择矩阵：原型期→静态检查，关键系统→模型检测，极致安全→定理证明。CI/CD集成：make verify作为质量门禁，架构违规自动拒绝合并。案例：CompCert用Coq验证编译器、AWS用TLA+验证分布式算法、JUnit用ArchUnit保护架构。关键：自动化验证将架构约束从文档变为可执行规范。
...
- [2.6 Pattern Verification](#26-pattern-verification)

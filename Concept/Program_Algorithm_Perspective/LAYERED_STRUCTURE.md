- --
topic: "Program-Algorithm-Design Perspective - 分层结构梳理"
dependencies: []
status: "review"
...
- [Program-Algorithm-Design Perspective - 分层结构梳理](#program-algorithm-design-perspective---分层结构梳理)
  - [1. 📋 目录 {#-目录}](#1--目录--目录)
  - [1. 抽象层次分层 {#抽象层次分层}](#1-抽象层次分层-抽象层次分层)
    - [1 1 五层抽象模型 {#1-五层抽象模型}](#1-1-五层抽象模型-1-五层抽象模型)
    - [1 2 层间依赖关系 {#2-层间依赖关系}](#1-2-层间依赖关系-2-层间依赖关系)
  - [2. 学习难度分层 {#学习难度分层}](#2-学习难度分层-学习难度分层)
    - [2 1 按难度分级 {#1-按难度分级}](#2-1-按难度分级-1-按难度分级)
    - [2 2 学习路径推荐 {#2-学习路径推荐}](#2-2-学习路径推荐-2-学习路径推荐)
  - [3. 工程实践分层 {#工程实践分层}](#3-工程实践分层-工程实践分层)
    - [3 1 软件开发生命周期分层 {#1-软件开发生命周期分层}](#3-1-软件开发生命周期分层-1-软件开发生命周期分层)
    - [3 2 不同规模项目的应用 {#2-不同规模项目的应用}](#3-2-不同规模项目的应用-2-不同规模项目的应用)
  - [4. 形式化程度分层 {#形式化程度分层}](#4-形式化程度分层-形式化程度分层)
    - [4 1 按形式化严格程度分层 {#1-按形式化严格程度分层}](#4-1-按形式化严格程度分层-1-按形式化严格程度分层)
    - [4 2 形式化方法投入产出比 {#2-形式化方法投入产出比}](#4-2-形式化方法投入产出比-2-形式化方法投入产出比)
  - [5. 时间发展分层 {#时间发展分层}](#5-时间发展分层-时间发展分层)
    - [5 1 按历史发展分层 {#1-按历史发展分层}](#5-1-按历史发展分层-1-按历史发展分层)
    - [5 2 技术成熟度曲线 {#2-技术成熟度曲线}](#5-2-技术成熟度曲线-2-技术成熟度曲线)
  - [6. 跨领域知识分层 {#跨领域知识分层}](#6-跨领域知识分层-跨领域知识分层)
    - [6 1 知识领域依赖图 {#1-知识领域依赖图}](#6-1-知识领域依赖图-1-知识领域依赖图)
    - [6 2 领域知识需求矩阵 {#2-领域知识需求矩阵}](#6-2-领域知识需求矩阵-2-领域知识需求矩阵)
  - [7. 验证技术分层 {#验证技术分层}](#7-验证技术分层-验证技术分层)
    - [7 1 按验证强度分层 {#1-按验证强度分层}](#7-1-按验证强度分层-1-按验证强度分层)
    - [7 2 验证技术适用场景 {#2-验证技术适用场景}](#7-2-验证技术适用场景-2-验证技术适用场景)
  - [8. 系统架构分层 {#系统架构分层}](#8-系统架构分层-系统架构分层)
    - [8 1 经典五层架构详解 {#1-经典五层架构详解}](#8-1-经典五层架构详解-1-经典五层架构详解)
    - [8 2 跨层验证路径 {#2-跨层验证路径}](#8-2-跨层验证路径-2-跨层验证路径)
  - [9. 使用建议 {#使用建议}](#9-使用建议-使用建议)
    - [9 1 如何使用分层结构 {#1-如何使用分层结构}](#9-1-如何使用分层结构-1-如何使用分层结构)
    - [9 2 层次跃迁的时机 {#2-层次跃迁的时机}](#9-2-层次跃迁的时机-2-层次跃迁的时机)

...
<details>
<summary><b>⚖️🔬 点击展开：公理语义核心洞察</b></summary>

**终极洞察**: 公理语义=程序正确性的逻辑系统。核心思想：用逻辑公式{P}c{Q}描述程序性质，回答"如何证明程序正确"。Hoare Logic（霍尔逻辑）：①前置条件P（执行前必须满足）②程序c（待验证代码）③后置条件Q（执行后保证成立）。推理规则：组合规则（顺序、条件、循环）+结构规则（强化、弱化）+不变式（循环不变量）。关键技术：①最弱前置条件WP（Weakest Precondition）：wp(c,Q)=保证达到Q的最弱条件②最强后置条件SP（Strongest Postcondition）：sp(P,c)=从P出发能达到的最强条件③分离逻辑（Separation Logic）：*分离合取，处理指针和堆内存，形式化"无别名"。完全正确性vs部分正确性：[P]c[Q]保证终止，{P}c{Q}只保证若终止则正确。工具生态：Dafny（自动验证）、Why3（多证明器后端）、Frama-C（C程序验证）、VeriFast（分离逻辑工具）。应用：①关键系统验证（航空航天、医疗）②编译器正确性③密码学协议④智能合约（Solidity验证）。可靠性定理：⊢{P}c{Q} ⇒ ⊨{P}c{Q}（语法推导蕴含语义有效）。关键：公理语义将程序验证变为逻辑推导，是形式化验证的核心方法。
...
- [1.3 Axiomatic Semantics](#13-axiomatic-semantics)
  - [1 📊 核心概念深度分析](#1--核心概念深度分析)
  - [📋 目录](#-目录)
  - [2 概述](#2-概述)
  - [1 Hoare 逻辑推理规则](#1-hoare-逻辑推理规则)
    - [1.1 基本规则](#11-基本规则)
      - [1 赋值规则（Assignment）](#1-赋值规则assignment)
      - [2 顺序组合规则（Sequential Composition）](#2-顺序组合规则sequential-composition)
      - [3 条件规则（Conditional）](#3-条件规则conditional)
      - [4 循环规则（While Loop）](#4-循环规则while-loop)
      - [5 后果规则（Consequence）](#5-后果规则consequence)
  ...
  - [4 分离逻辑（Separation Logic）](#4-分离逻辑separation-logic)
    ...
    - [4.3 分离逻辑规则](#43-分离逻辑规则)
      - [1 帧规则（Frame Rule）](#1-帧规则frame-rule)
      - [2 分配规则](#2-分配规则)
      - [3 释放规则](#3-释放规则)
  ...
  - [10 总结](#10-总结)
    - [1 核心洞察](#1-核心洞察)
    - [10.2 工具链推荐](#102-工具链推荐)
    - [10.3 实战建议](#103-实战建议)
  - [11 附录 A：Wikipedia 概念对照](#11-附录-awikipedia-概念对照)
...
#### 2 顺序组合规则（Sequential Composition）
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [Gödel不完备定理](../../CONCEPT_CROSS_INDEX.md#101-gödel不完备定理-gödels-incompleteness-theorems-七视角) - 公理语义的完备性限制，形式验证的根本约束
  - [停机问题](../../CONCEPT_CROSS_INDEX.md#102-停机问题-halting-problem-七视角) - 完全正确性验证中的终止性判定
  - [Rice定理](../../CONCEPT_CROSS_INDEX.md#171-rice定理-rices-theorem-七视角) - 程序性质验证的不可判定性

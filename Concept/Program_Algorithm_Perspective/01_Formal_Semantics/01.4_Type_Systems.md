...
- [📋 目录](#目录)
- [概述](#概述)
- [1. 简单类型 λ 演算（Simply Typed Lambda Calculus, STLC）](#1-简单类型-演算simply-typed-lambda-calculus-stlc)
  - [1.1 语法](#11-语法)
...
- [4. Affine 类型（Rust 的所有权）](#4-affine-类型rust-的所有权)
  ...
  - [4.3 λRust 形式化（RustBelt）](#43-rust-形式化rustbelt)
...
- [附录 A：类型推导示例](#附录-a类型推导示例)
  - [示例：推导 `(λx:Bool. x) true`](#示例推导-xbool-x-true)

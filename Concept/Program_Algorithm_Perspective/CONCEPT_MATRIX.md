- --
topic: "Program-Algorithm-Design Perspective - 概念矩阵对比"
dependencies: []
status: "review"
...
- [Program-Algorithm-Design Perspective - 概念矩阵对比](#program-algorithm-design-perspective---概念矩阵对比)
  - [1. 📋 目录 {#-目录}](#1--目录--目录)
  - [1. 形式语义对比矩阵 {#形式语义对比矩阵}](#1-形式语义对比矩阵-形式语义对比矩阵)
    - [1 1 三大语义流派对比 {#1-三大语义流派对比}](#1-1-三大语义流派对比-1-三大语义流派对比)
    - [1 2 操作语义的变体对比 {#2-操作语义的变体对比}](#1-2-操作语义的变体对比-2-操作语义的变体对比)
  - [2. 设计模式分类矩阵 {#设计模式分类矩阵}](#2-设计模式分类矩阵-设计模式分类矩阵)
    - [2 1 GoF 模式全景矩阵 {#1-gof-模式全景矩阵}](#2-1-gof-模式全景矩阵-1-gof-模式全景矩阵)
    - [2 2 分布式模式对比矩阵 {#2-分布式模式对比矩阵}](#2-2-分布式模式对比矩阵-2-分布式模式对比矩阵)
  - [3. 复杂度维度矩阵 {#复杂度维度矩阵}](#3-复杂度维度矩阵-复杂度维度矩阵)
    - [3 1 多维复杂度全景矩阵 {#1-多维复杂度全景矩阵}](#3-1-多维复杂度全景矩阵-1-多维复杂度全景矩阵)
    - [3 2 复杂度类层次矩阵 {#2-复杂度类层次矩阵}](#3-2-复杂度类层次矩阵-2-复杂度类层次矩阵)
  - [4. 架构模式对比矩阵 {#架构模式对比矩阵}](#4-架构模式对比矩阵-架构模式对比矩阵)
    - [4 1 六大架构模式对比 {#1-六大架构模式对比}](#4-1-六大架构模式对比-1-六大架构模式对比)
    - [4 2 五层架构模型矩阵 {#2-五层架构模型矩阵}](#4-2-五层架构模型矩阵-2-五层架构模型矩阵)
  - [5. 形式验证工具矩阵 {#形式验证工具矩阵}](#5-形式验证工具矩阵-形式验证工具矩阵)
    - [5 1 定理证明器对比 {#1-定理证明器对比}](#5-1-定理证明器对比-1-定理证明器对比)
    - [5 2 模型检测工具对比 {#2-模型检测工具对比}](#5-2-模型检测工具对比-2-模型检测工具对比)
    - [5 3 符号执行工具对比 {#3-符号执行工具对比}](#5-3-符号执行工具对比-3-符号执行工具对比)
  - [6. 编程语言特性矩阵 {#编程语言特性矩阵}](#6-编程语言特性矩阵-编程语言特性矩阵)
    - [6 1 Rust vs Python vs Golang 对比 {#1-rust-vs-python-vs-golang-对比}](#6-1-rust-vs-python-vs-golang-对比-1-rust-vs-python-vs-golang-对比)
    - [6 2 类型系统特性矩阵 {#2-类型系统特性矩阵}](#6-2-类型系统特性矩阵-2-类型系统特性矩阵)
  - [7. 并发模型对比矩阵 {#并发模型对比矩阵}](#7-并发模型对比矩阵-并发模型对比矩阵)
    - [7 1 主流并发模型对比 {#1-主流并发模型对比}](#7-1-主流并发模型对比-1-主流并发模型对比)
    - [7 2 并发原语对比 {#2-并发原语对比}](#7-2-并发原语对比-2-并发原语对比)
  - [8. 分布式模式矩阵 {#分布式模式矩阵}](#8-分布式模式矩阵-分布式模式矩阵)
    - [8 1 一致性协议对比 {#1-一致性协议对比}](#8-1-一致性协议对比-1-一致性协议对比)
    - [8 2 消息队列对比 {#2-消息队列对比}](#8-2-消息队列对比-2-消息队列对比)
  - [9. 验证技术对比矩阵 {#验证技术对比矩阵}](#9-验证技术对比矩阵-验证技术对比矩阵)
    - [9 1 验证方法对比 {#1-验证方法对比}](#9-1-验证方法对比-1-验证方法对比)
    - [9 2 工业验证案例对比 {#2-工业验证案例对比}](#9-2-工业验证案例对比-2-工业验证案例对比)
  - [10. 工具生态系统矩阵 {#工具生态系统矩阵}](#10-工具生态系统矩阵-工具生态系统矩阵)
    - [10 1 开发工具链对比 {#1-开发工具链对比}](#10-1-开发工具链对比-1-开发工具链对比)
    - [10 2 形式化工具成熟度矩阵 {#2-形式化工具成熟度矩阵}](#10-2-形式化工具成熟度矩阵-2-形式化工具成熟度矩阵)
  - [11. 使用指南 {#使用指南}](#11-使用指南-使用指南)
    - [11 1 如何使用这些矩阵 {#1-如何使用这些矩阵}](#11-1-如何使用这些矩阵-1-如何使用这些矩阵)
    - [11 2 矩阵更新频率 {#2-矩阵更新频率}](#11-2-矩阵更新频率-2-矩阵更新频率)
  - [关联网络](#关联网络)
    - [前向引用](#前向引用)
    - [后向引用](#后向引用)
    - [交叉链接](#交叉链接)
...
### 6 1 Rust vs Python vs Golang 对比 {#1-rust-vs-python-vs-golang-对比}
...
## 10. 工具生态系统矩阵 {#工具生态系统矩阵}

### 10 1 开发工具链对比 {#1-开发工具链对比}

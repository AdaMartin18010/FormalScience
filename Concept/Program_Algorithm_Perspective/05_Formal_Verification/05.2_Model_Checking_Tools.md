...
<details>
<summary><b>🔍🔬 点击展开：模型检测核心洞察</b></summary>

**终极洞察**: 模型检测=穷举搜索状态空间找bug。基本思想：①建立系统的有限状态模型②用时序逻辑表达性质（如"永不死锁"）③自动搜索所有可达状态验证性质④若失败给出反例（bug执行路径）。三大工具：①mCRL2（进程代数）：用ACP/CCS/π-演算建模并发系统，μ-calculus验证性质，适合协议验证②UPPAAL（时间自动机）：建模实时系统，时钟约束+状态迁移，CTL时序逻辑，适合嵌入式验证③TLA+（时序逻辑）：Lamport的规约语言，描述分布式算法，TLC模型检查器，Amazon/Microsoft广泛使用。核心挑战：状态爆炸（状态数随变量指数增长）。应对技术：①符号模型检测（BDD）②部分序约简（并发等价类）③抽象精化④有界模型检测（BMC）。验证性质：①安全性（Safety）：坏事永不发生，如无死锁②活性（Liveness）：好事最终发生，如请求最终响应③公平性（Fairness）：无饥饿。工业应用：Intel（芯片协议）、AWS（分布式算法）、航空航天（飞控系统）。关键：模型检测提供自动化验证+反例调试，是发现并发/分布式系统bug的利器。
...
- [05.2 模型检测工具：mCRL2, UPPAAL, TLA+](#052-模型检测工具mcrl2-uppaal-tla)
  ...
  - [2 . mCRL2：进程代数建模](#2--mcrl2进程代数建模)
    ...
    - [2.4 性质验证（μ-calculus）](#24-性质验证μ-calculus)
  ...
  - [10 附录 B：工具安装指南](#10-附录-b工具安装指南)
    - [1 mCRL2](#1-mcrl2)
    - [10.2 UPPAAL](#102-uppaal)
    - [10.3 TLA+](#103-tla)
  - [跨视角链接](#跨视角链接)
...
- [概念交叉索引（七视角版）](../../CONCEPT_CROSS_INDEX.md) - 查看相关概念的七视角分析：
  - [停机问题](../../CONCEPT_CROSS_INDEX.md#102-停机问题-halting-problem-七视角) - 模型检测中的终止性验证

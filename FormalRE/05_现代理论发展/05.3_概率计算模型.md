...
| 维度 | 内容 |
|------|------|
| **形式化定义** | 概率计算模型作为随机计算系统：概率图灵机在标准图灵机基础上增加随机带，通过随机选择实现概率计算，复杂度类BPP、RP、ZPP在P和PSPACE之间 |
| **直观理解** | 概率计算模型允许算法使用随机性，可以在多项式时间内以高概率解决某些问题，但受限于可计算性边界 |
| **等价定义** | 1. 随机计算系统<br>2. 概率图灵机系统<br>3. BPP复杂度类系统 |
| **历史定义** | 概率计算：随机算法；复杂度类：BPP、RP、ZPP；可计算性分析：从RE理论视角理解概率计算 |
...
1. **具体算法**: 素性测试、多项式恒等测试等具体算法
2. **具体复杂度类**: BPP、RP、ZPP等具体类
3. **应用领域**: 在密码学、算法设计等领域的应用
...
| 概念 | 区别 |
|------|------|
| **确定性计算** | 概率计算使用随机性，确定性计算不使用 |
| **其他计算系统** | 概率计算强调随机性，其他系统可能有不同机制 |
| **其他复杂度类** | 概率计算强调概率性，其他类可能有不同性质 |
...
- 随机计算系统 (Randomized Computational System)
- 概率计算系统 (Probabilistic Computational System)

...
## 1. 不动点的直觉 {#不动点的直觉} {#不动点的直觉-不动点的直觉}

### 1 1 什么是不动点？ {#1-什么是不动点} {#1-什么是不动点-1-什么是不动点}

- _定义1.1 (不动点)**: 设 $f: X \to X$ 是集合 $X$ 上的函数，$x \in X$ 是 $f$ 的**不动点_*，如果：
...
## 1. 0 概念分析：不动点 {#0-概念分析不动点} {#0-概念分析不动点-0-概念分析不动点}
...
### 1 2 为什么不动点重要？ {#2-为什么不动点重要} {#2-为什么不动点重要-2-为什么不动点重要}
...
### 1 3 递归可枚举性与不动点 {#3-递归可枚举性与不动点} {#3-递归可枚举性与不动点-3-递归可枚举性与不动点}
...
## 2. Knaster-Tarski不动点定理 {#knaster-tarski不动点定理} {#knaster-tarski不动点定理-knaster-ta}

### 2 1 完备格基础 {#1-完备格基础} {#1-完备格基础-1-完备格基础}
...
### 2 3 Knaster-Tarski定理 {#3-knaster-tarski定理} {#3-knaster-tarski定理-3-knaster-t}
...
- _引理2.3.1_* (前置不动点集合的性质):

设$P = \{x \in L \mid f(x) \leq x\}$，则$P$在$\bigwedge$下封闭。

- _证明_*: 设$S \subseteq P$，$s = \bigwedge S$。对任意$x \in S$，有$s \leq x$，因此$f(s) \leq f(x) \leq x$。因此$f(s)$是$S$的下界，所以$f(s) \leq s$，即$s \in P$。
...
### 2 4 Kleene不动点定理 {#4-kleene不动点定理} {#4-kleene不动点定理-4-kleene不动点定理}
...
- _步骤3: 证明$\bigvee_n x_n$是最小不动点_*

设$y$是$f$的任意不动点，即$f(y) = y$。

我们需要证明$\bigvee_n x_n \leq y$。

- _归纳证明_* $x_n \leq y$对所有$n$成立：
...
- _引理2.4.2_* (Kleene不动点与Knaster-Tarski不动点的等价性):

在完备格上，如果$f$单调，则Kleene不动点$\bigvee_n f^n(\bot)$等于Knaster-Tarski最小不动点$\bigwedge \{x \mid f(x) \leq x\}$。

- _证明思路_*: 两者都构造最小不动点，只是方法不同。Kleene方法通过迭代，Knaster-Tarski方法通过前置不动点集合。
...
- _直觉_*: 从"空"开始，不断应用$f$，直到达到不动点。这提供了计算最小不动点的算法。

- --

## 3. Scott不动点定理 {#scott不动点定理} {#scott不动点定理-scott不动点定理}

### 3 1 完备偏序集 (CPO) {#1-完备偏序集-cpo} {#1-完备偏序集-cpo-1-完备偏序集-cpo}
...
### 3 2 Scott连续函数 {#2-scott连续函数} {#2-scott连续函数-2-scott连续函数}
...
### 3 3 Scott不动点定理 {#3-scott不动点定理} {#3-scott不动点定理-3-scott不动点定理}
...
- _归纳证明_* $x_n \sqsubseteq y$对所有$n$成立：
...
## 4. Y组合子与递归 {#y组合子与递归} {#y组合子与递归-y组合子与递归}

### 4 1 λ演算中的不动点 {#1-λ演算中的不动点} {#1-λ演算中的不动点-1-λ演算中的不动点}
...
- _解决_*: 用**不动点组合子** (Fixed-Point Combinator)

### 4 2 Y组合子 {#2-y组合子} {#2-y组合子-2-y组合子}
...
### 4 3 不动点组合子的多样性 {#3-不动点组合子的多样性} {#3-不动点组合子的多样性-3-不动点组合子的多样性}
...
## 5. 最小不动点语义 {#最小不动点语义} {#最小不动点语义-最小不动点语义}

### 5 1 为什么要"最小"？ {#1-为什么要最小} {#1-为什么要最小-1-为什么要最小}
...
### 5 2 归纳定义的语义 {#2-归纳定义的语义} {#2-归纳定义的语义-2-归纳定义的语义}

- _原则_*: 归纳定义的含义 = 其生成算子的最小不动点
...
### 5 3 余归纳定义的语义 {#3-余归纳定义的语义} {#3-余归纳定义的语义-3-余归纳定义的语义}

- _对偶_*: 余归纳定义的含义 = 其生成算子的**最大不动点**
...
## 6. 应用实例 {#应用实例} {#应用实例-应用实例}

### 6 1 文法语言生成 {#1-文法语言生成} {#1-文法语言生成-1-文法语言生成}
...
### 6 2 Petri网可达标识 {#2-petri网可达标识} {#2-petri网可达标识-2-petri网可达标识}
...
### 6 3 λ项的范式 {#3-λ项的范式} {#3-λ项的范式-3-λ项的范式}
...
- _范式_*: 如果 Reach(M) 包含范式，则 M 可规约

### 6 4 程序语义 {#4-程序语义} {#4-程序语义-4-程序语义}

- _While语言_*: `while B do C`

- _指称语义_* (Denotational Semantics):
...
### 7 1 递归 = 不动点 {#1-递归--不动点} {#1-递归--不动点-1-递归--不动点}
...
### 7 2 构造性 vs 存在性 {#2-构造性-vs-存在性} {#2-构造性-vs-存在性-2-构造性-vs-存在性}

- _Knaster-Tarski_*: 存在性证明（非构造性）
...
### 7 3 归纳 vs 余归纳 {#3-归纳-vs-余归纳} {#3-归纳-vs-余归纳-3-归纳-vs-余归纳}
...
### 10 1 习题1 (容易) {#习题1-容易} {#1-习题1-容易-习题1-容易}

证明: 在完备格上，f(x) = x ∪ {0} 有最小不动点 {0}。

### 10 2 习题2 (中等) {#习题2-中等} {#2-习题2-中等-习题2-中等}
...
### 10 3 习题3 (困难) {#习题3-困难} {#3-习题3-困难-习题3-困难}
...
## 9. 思维表征：不动点理论 {#思维表征不动点理论} {#思维表征不动点理论-思维表征不动点理论}
...
- _Wikipedia词条_*: [Fixed-point theorem](https://en.wikipedia.org/wiki/Fixed-point_theorem)
...
### 12 3 # 10.2.1 CMU 15-312 (Foundations of Programming Languages) {#-1021-cmu-15-312-foundations-o} {#3--1021-cmu-15-312-foundations}
...
### 12 4 # 10.2.2 MIT 6.820 (Fundamentals of Program Analysis) {#-1022-mit-6820-fundamentals-of} {#4--1022-mit-6820-fundamentals-}
...
### 12 5 # 10.2.3 Oxford Category Theory Course {#-1023-oxford-category-theory-c} {#5--1023-oxford-category-theory}
...
- _建议补充_*: 初始代数与终结余代数的详细理论

### 10 3 权威教材对标 {#3-权威教材对标} {#3-权威教材对标-3-权威教材对标}

### 12 7 # 10.3.1 Stoy, "Denotational Semantics" {#-1031-stoy-denotational-semant} {#7--1031-stoy-denotational-sema}
...
### 12 8 # 10.3.2 Winskel, "The Formal Semantics of Programming Languages" {#-1032-winskel-the-formal-seman} {#8--1032-winskel-the-formal-sem}
...
### 12 9 # 10.3.3 Davey & Priestley, "Introduction to Lattices and Order" {#-1033-davey--priestley-introdu} {#9--1033-davey--priestley-intro}
...
## 15. 📚 参考文献 {#-参考文献} {#-参考文献--参考文献}

### 15 1 不动点理论基础 {#不动点理论基础} {#1-不动点理论基础-不动点理论基础}
...
### 15 2 Lambda演算 {#lambda演算} {#2-lambda演算-lambda演算}

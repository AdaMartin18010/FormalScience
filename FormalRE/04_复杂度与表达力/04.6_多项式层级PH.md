- --
topic: "多项式层级 (Polynomial Hierarchy)"
dependencies: []
status: "review"
...
## 1. 定义与动机 {#定义与动机} {#定义与动机-定义与动机}

### 1 0 概念分析：多项式层级 {#0-概念分析多项式层级} {#0-概念分析多项式层级-0-概念分析多项式层级}
...
### 1 1 从算术谱系到多项式层级 {#1-从算术谱系到多项式层级} {#1-从算术谱系到多项式层级-1-从算术谱系到多项式层级}

- _算术谱系回顾_* (见 [04.2](04.2_算术谱系与Sigma层级.md)):
...
### 1 2 三种等价定义 {#2-三种等价定义} {#2-三种等价定义-2-三种等价定义}
...
## 2. 层级结构 {#层级结构} {#层级结构-层级结构}

### 2 1 Σᵢᴾ和Πᵢᴾ类 {#1-σᵢᴾ和πᵢᴾ类} {#1-σᵢᴾ和πᵢᴾ类-1-σᵢᴾ和πᵢᴾ类}
...
## 3. 重要性质 {#重要性质} {#重要性质-重要性质}

### 3 1 包含关系 {#1-包含关系} {#1-包含关系-1-包含关系}
...
### 3 2 PH坍塌定理 {#2-ph坍塌定理} {#2-ph坍塌定理-2-ph坍塌定理}
...
- _步骤1: 证明$\Sigma_{i+1}^P = \Pi_{i+1}^P$_*
...
- _结论_*: $\Sigma_{i+1}^P = \Pi_{i+1}^P$。

- _步骤2: 归纳证明所有更高层都坍塌_*

- _基础情况_*: 已证明$\Sigma_{i+1}^P = \Pi_{i+1}^P$。

- _归纳步骤_*: 假设$\Sigma_j^P = \Pi_j^P$对某个$j \geq i+1$成立。
...
- _证明思路_*: 由于$\Sigma_i^P = \Pi_i^P$，带$\Sigma_i^P$预言机的计算可以模拟带$\Pi_i^P$预言机的计算（通过补集和对偶性）。
...
## 4. 与其他复杂度类关系 {#与其他复杂度类关系} {#与其他复杂度类关系-与其他复杂度类关系}

### 4 1 PH与PSPACE {#1-ph与pspace} {#1-ph与pspace-1-ph与pspace}
...
### 4 2 PH与BQP (量子) {#2-ph与bqp-量子} {#2-ph与bqp-量子-2-ph与bqp-量子}
...
## 5. 应用实例 {#应用实例} {#应用实例-应用实例}

### 5 1 逻辑与数据库 {#1-逻辑与数据库} {#1-逻辑与数据库-1-逻辑与数据库}
...
### 5 2 游戏与验证 {#2-游戏与验证} {#2-游戏与验证-2-游戏与验证}
...
### 6 2 实践相关性 {#2-实践相关性} {#2-实践相关性-2-实践相关性}
...
### 6 3 未解决问题 {#3-未解决问题} {#3-未解决问题-3-未解决问题}
...
## 7. 思维表征：多项式层级 {#思维表征多项式层级} {#思维表征多项式层级-思维表征多项式层级}
...
- _Wikipedia词条_*: [Polynomial hierarchy](https://en.wikipedia.org/wiki/Polynomial_hierarchy)
...
### 9 3 # 8.2.1 CMU 15-455 (Computational Complexity) {#-821-cmu-15-455-computational-} {#3--821-cmu-15-455-computationa}
...
### 9 4 # 8.2.2 MIT 6.045J (Automata, Computability, and Complexity) {#-822-mit-6045j-automata-comput} {#4--822-mit-6045j-automata-comp}
...
### 9 6 # 8.3.1 Arora & Barak, "Computational Complexity" {#-831-arora--barak-computationa} {#6--831-arora--barak-computatio}
...
### 9 7 # 8.3.2 Papadimitriou, "Computational Complexity" {#-832-papadimitriou-computation} {#7--832-papadimitriou-computati}
...
### 11 3 未解决之美 {#未解决之美} {#3-未解决之美-未解决之美}

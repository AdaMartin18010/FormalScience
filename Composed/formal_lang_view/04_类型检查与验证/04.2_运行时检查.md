...
- [4.2 运行时检查 ↔ 准入控制器](#42-运行时检查--准入控制器)
  ...
  - [📊 思维表征体系](#-思维表征体系)
    ...
    - [📊 2. 多维对比矩阵](#-2-多维对比矩阵)
      - [2.1 运行时检查 vs 准入控制器对比矩阵](#21-运行时检查-vs-准入控制器对比矩阵)
      - [2.2 运行时检查工具对比矩阵](#22-运行时检查工具对比矩阵)
      - [2.3 验证策略对比矩阵](#23-验证策略对比矩阵)
    - [🌲 3. 决策树](#-3-决策树)
      - [3.1 运行时检查应用选择决策树](#31-运行时检查应用选择决策树)
    - [🛤️ 4. 决策逻辑路径](#️-4-决策逻辑路径)
      - [4.1 运行时检查应用路径](#41-运行时检查应用路径)
    - [🕸️ 5. 概念关系网络](#️-5-概念关系网络)
      - [5.1 运行时检查概念关系网络](#51-运行时检查概念关系网络)
    - [🗺️ 6. 知识图谱](#️-6-知识图谱)
      - [6.1 运行时检查知识图谱](#61-运行时检查知识图谱)
  - [📚 理论体系](#-理论体系)
    - [理论基础](#理论基础)
      - [运行时类型检查/动态验证/Kubernetes基础](#运行时类型检查动态验证kubernetes基础)
      - [历史发展](#历史发展)
    - [理论框架](#理论框架)
      - [核心假设](#核心假设)
      - [基本概念体系](#基本概念体系)
...
graph TD
    ...
    B -->|类型断言| C[Admission Webhook]
    B -->|动态检查| D[健康检查]
    B -->|能力检查| E[Seccomp/AppArmor]

    C --> F{检查阶段?}
    F -->|准入| G[Admission Controller]
    F -->|运行时| H[Runtime Hook]
```
...
graph TD
    ...
    B --> B1[Admission Webhook/类型断言/准入控制]
    C --> C1[健康检查/动态验证/运行时监控]
    D --> D1[Seccomp/AppArmor/能力限制]
...
graph LR
    ...
    B --> E((Admission Webhook/准入控制))
    C --> F((健康检查/运行时监控))
    D --> G((Seccomp/AppArmor/能力限制))
...
- **2025年**：Kubernetes 1.28、智能健康检查、eBPF运行时验证
...
|-----------|---------|---------|---------|----------|
| **运行时类型断言** | 类型系统 | 类型断言 | 对应 | 运行时类型断言对应类型断言 |
| **动态检查** | 运行时系统 | 动态验证 | 对应 | 动态检查对应动态验证 |
| **能力类型** | 安全系统 | 能力控制 | 对应 | 能力类型对应能力控制 |
...
|-----------|---------|---------|---------|----------|
| **运行时检查** | 类型系统 | 运行时检查 | 对应 | 运行时检查对应运行时检查 |
| **Admission Webhook** | Kubernetes | 准入控制 | 对应 | Admission Webhook对应准入控制 |
| **健康检查** | Kubernetes | 健康监控 | 对应 | 健康检查对应健康监控 |
| **Seccomp/AppArmor** | 安全系统 | 能力限制 | 对应 | Seccomp/AppArmor对应能力限制 |
...
- **本理论基于**：
  - [04.1_编译期检查.md](04.1_编译期检查.md) ⭐⭐⭐ - 编译期检查
  - 类型系统 ⭐⭐ - 理论基础

- **本理论应用于**：
  - [04.3_形式化验证.md](04.3_形式化验证.md) ⭐⭐⭐ - 形式化验证
  - [06.1_反射.md](../06_动态性与反射/06.1_反射.md) ⭐⭐⭐ - 反射
  - Kubernetes运行时验证 ⭐⭐⭐ - 实际应用
...
|-----------|---------|---------|---------|
| **运行时检查** | Kubernetes | 准入控制 | 成功 |
| **动态检查** | Kubernetes | 健康监控 | 成功 |
| **能力检查** | Kubernetes | 安全控制 | 成功 |
...
| 视角 | 关联文档 | 核心理论 | 关注点 |
|------|---------|---------|--------|
| **运行时检查** | 本文档 | 动态检查 | 运行时检查 |
| **编译期检查** | 04.1 | 静态检查 | 编译期检查 |
...
1. [04.3_形式化验证.md](04.3_形式化验证.md) ⭐⭐⭐ - 形式化验证
2. [06.1_反射.md](../06_动态性与反射/06.1_反射.md) ⭐⭐⭐ - 反射
3. Kubernetes运行时验证实践 ⭐⭐ - 实践应用

...
- [6.1 反射（Reflection）](#61-反射reflection)
  ...
  - [12 核心概念](#12-核心概念)
    - [12.1 Downward API作为反射](#121-downward-api作为反射)
    - [12.2 监控指标暴露作为内省](#122-监控指标暴露作为内省)
    - [12.3 kubectl exec作为运行时反射](#123-kubectl-exec作为运行时反射)
  - [13 反射映射表](#13-反射映射表)
  - [4 技术细节](#4-技术细节)
  ...
  - [📊 思维表征体系](#-思维表征体系)
    ...
    - [📊 2. 多维对比矩阵](#-2-多维对比矩阵)
      - [2.1 反射 vs Kubernetes机制对比矩阵](#21-反射-vs-kubernetes机制对比矩阵)
      - [2.2 内省工具对比矩阵](#22-内省工具对比矩阵)
      - [2.3 反射应用场景对比矩阵](#23-反射应用场景对比矩阵)
    - [🌲 3. 决策树](#-3-决策树)
      - [3.1 反射应用选择决策树](#31-反射应用选择决策树)
    - [🛤️ 4. 决策逻辑路径](#️-4-决策逻辑路径)
      - [4.1 反射应用路径](#41-反射应用路径)
    - [🕸️ 5. 概念关系网络](#️-5-概念关系网络)
      - [5.1 反射概念关系网络](#51-反射概念关系网络)
    - [🗺️ 6. 知识图谱](#️-6-知识图谱)
      - [6.1 反射知识图谱](#61-反射知识图谱)
  - [📚 理论体系](#-理论体系)
    - [理论基础](#理论基础)
      - [反射机制/元编程/Kubernetes基础](#反射机制元编程kubernetes基础)
      - [历史发展](#历史发展)
    - [理论框架](#理论框架)
      - [核心假设](#核心假设)
      - [基本概念体系](#基本概念体系)
...
graph TD
    ...
    B -->|元数据访问| C[Downward API]
    B -->|监控指标| D[Prometheus]
    B -->|运行时操作| E[kubectl exec]

    C --> F{元数据类型?}
    F -->|Pod信息| G[Downward API]
    F -->|Service信息| H[Service API]
```
...
graph TD
    ...
    B --> B1[Downward API/Prometheus/监控指标]
    C --> C1[运行时反射/元数据访问/动态操作]
    D --> D1[RTTI/类型信息/运行时类型]
...
- **2025年**：Downward API增强、Prometheus 2.50、OpenTelemetry 1.30
...
|-----------|---------|---------|---------|----------|
| **内省** | 系统理论 | 内省 | 对应 | 内省对应内省 |
| **元编程** | 编程语言理论 | 元编程 | 对应 | 元编程对应元编程 |
| **运行时类型信息** | 编程语言理论 | RTTI | 对应 | 运行时类型信息对应RTTI |
...
|-----------|---------|---------|---------|----------|
| **反射** | 编程语言 | 反射 | 对应 | 反射对应反射 |
| **Downward API** | Kubernetes | Downward API | 对应 | Downward API对应元数据访问 |
| **Prometheus** | Kubernetes | 监控 | 对应 | Prometheus对应监控指标 |
| **kubectl exec** | Kubernetes | 运行时操作 | 对应 | kubectl exec对应运行时操作 |
...
- **本理论基于**：
  - [04.2_运行时检查.md](../04_类型检查与验证/04.2_运行时检查.md) ⭐⭐⭐ - 运行时检查
  - 编程语言理论 ⭐⭐ - 理论基础

- **本理论应用于**：
  - [06.2_动态类型与弹性伸缩.md](06.2_动态类型与弹性伸缩.md) ⭐⭐⭐ - 动态类型
  - [09.6_工程实践工具链.md](../09_形式化理论/09.6_工程实践工具链.md) ⭐⭐⭐ - 工程实践工具链
  - Kubernetes可观测性 ⭐⭐⭐ - 实际应用
...
|-----------|---------|---------|---------|
| **反射** | Kubernetes | 运行时内省 | 成功 |
| **内省** | Kubernetes | 监控系统 | 成功 |
| **元编程** | Kubernetes | 动态操作 | 成功 |
...
1. [06.2_动态类型与弹性伸缩.md](06.2_动态类型与弹性伸缩.md) ⭐⭐⭐ - 动态类型
2. [09.6_工程实践工具链.md](../09_形式化理论/09.6_工程实践工具链.md) ⭐⭐⭐ - 工程实践工具链
3. Kubernetes可观测性实践 ⭐⭐ - 实践应用

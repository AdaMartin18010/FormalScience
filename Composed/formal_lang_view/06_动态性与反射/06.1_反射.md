...
- [6.1 反射（Reflection）](#61-反射reflection)
  ...
  - [📑 目录](#-目录)
  - [🔑 核心概念](#-核心概念)
    - [Downward API作为反射](#downward-api作为反射)
    - [监控指标暴露作为内省](#监控指标暴露作为内省)
    - [kubectl exec作为运行时反射](#kubectl-exec作为运行时反射)
  - [📊 反射映射表](#-反射映射表)
  - [🔍 技术细节](#-技术细节)
    - [Downward API实现](#downward-api实现)
    - [Prometheus监控指标](#prometheus监控指标)
    - [kubectl exec运行时检查](#kubectl-exec运行时检查)
  - [💡 实际应用](#-实际应用)
    - [容器元数据获取](#容器元数据获取)
    - [监控指标收集](#监控指标收集)
    - [容器调试](#容器调试)

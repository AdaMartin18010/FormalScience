...
### 10.1 Kubernetes Pod调度优化（2025年新增）

**Pod调度策略**：

- **节点选择**：根据资源需求和节点标签选择节点
- **亲和性调度**：Pod亲和性和反亲和性配置
- **污点容忍**：容忍节点污点，实现节点隔离
...
$$
\text{Schedule}(pod, node) \iff \text{ResourceAvailable}(node) \land \text{AffinityMatch}(pod, node) \land \text{TaintTolerated}(pod, node)
$$

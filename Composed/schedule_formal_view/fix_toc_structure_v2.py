...
# -*- coding: utf-8 -*-
"""
统一Markdown文件的目录结构（改进版）
确保：
1. 每个文件有且只有一个目录
2. 主题与子主题编号一致
3. 子主题编号从文件编号开始（如文件6.2，子主题从6.2.1开始）
"""
...
def fix_toc_numbering(toc_lines, file_num):
    ...
    for line in toc_lines:
        ...
        # 或 - [6.2.1 CFS调度器形式化](#621-cfs调度器形式化)
        ...
        if match:
            ...
            # 判断缩进级别
            indent_level = len(indent) // 2  # 假设每级缩进2个空格
            ...
            if indent_level == 0:
                ...
                new_line = f"{indent}- [{new_num} {title}](#{anchor})"
                fixed_lines.append(new_line)
                sub_index += 1
            elif indent_level == 2:
                # 二级子主题，应该是 file_series.file_sub.parent_sub.subsub_index
                # 需要找到父主题的编号
                parent_sub = sub_index - 1
                subsub_index = int(match.group(4)) if match.group(4) else 1
...
def fix_content_headers(content, file_num):
    ...
    for i, line in enumerate(lines):
        # 匹配标题，如 ## 6.1 CFS调度器形式化 或 ## 6.2.1 CFS调度器形式化
        ...
        if match:
            ...
            if level_count == 2:
                ...
                if header_series != file_series or header_sub != file_sub:
                    new_num = f"{file_series}.{file_sub}.{sub_index}"

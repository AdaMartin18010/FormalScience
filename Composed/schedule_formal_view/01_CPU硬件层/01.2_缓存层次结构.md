...
- [1.2 缓存层次结构](#12-缓存层次结构)
  ...
  - [10 2025年最新技术（已整合view文件夹内容）](#10-2025年最新技术已整合view文件夹内容)
    - [10.1 缓存替换策略优化（2025年新增）](#101-缓存替换策略优化2025年新增)
    - [10.2 缓存一致性协议优化（2025年新增）](#102-缓存一致性协议优化2025年新增)
    - [10.3 伪共享问题优化（2025年新增）](#103-伪共享问题优化2025年新增)
...
- **LRU（Least Recently Used）**：替换最久未使用的缓存行，命中率高
- **LFU（Least Frequently Used）**：替换使用频率最低的缓存行，适合频率敏感场景
- **FIFO（First In First Out）**：替换最早进入的缓存行，实现简单
- **随机替换**：随机选择缓存行替换，实现最简单
...
- **总线监听延迟**：~10ns（同Socket）
- **目录协议延迟**：~20ns（跨Socket）
- **一致性消息带宽**：> 100GB/s（现代CPU）

### 10.3 伪共享问题优化（2025年新增）
...
$$
\text{FalseSharing}(cacheline, cores) \iff \exists c_1, c_2 \in cores: \text{Access}(c_1, data_1) \land \text{Access}(c_2, data_2) \land \text{SameCacheline}(data_1, data_2)
$$

**优化策略**：

- **数据结构对齐**：对齐到缓存行边界（64字节），避免跨缓存行访问
- **线程局部存储（TLS）**：使用线程局部变量，避免共享热点数据
- **分离热点数据**：将频繁访问的数据分离到不同缓存行
- **填充字节**：使用padding填充，确保数据结构对齐

**性能提升**：

- **伪共享消除**：性能提升20-50%（多线程场景）
- **缓存命中率**：提升5-10%
- **内存带宽利用率**：提升10-20%

**缓存调度优化**：

- **预取优化**：提前加载可能访问的数据，减少缓存未命中
- **缓存对齐**：数据结构对齐到缓存行边界，提升访问效率
- **NUMA感知**：优先使用本地NUMA节点的缓存，减少跨节点访问

...
related: ["03.1 进程调度", "03.3 内存管理", "03.4 设备调度", "04.3 任务调度", "03.1 进程调度"]
math: true
code: true
...
本文档深入探讨线程调度的核心原理和关键方法。内容涵盖OS调度领域的主要知识点，包括调度算法, 调度, 资源分配, CPU调度等关键主题。适合有一定基础的学习者系统学习。
...
impl AsyncScheduler {
    ...
    fn create_waker(&self) -> Waker {
        ...
        unsafe fn wake(data: *const ()) {
            // 将任务重新加入就绪队列
            let queue = &*(data as *const Mutex<VecDeque<Task>>);
            // 实际实现中需要获取当前任务并重新加入队列
        }
        ...
        // 创建基于就绪队列的 waker
        let raw_waker = unsafe {
            RawWaker::new(
                Arc::into_raw(ready_queue) as *const (),
                &WAKER_VTABLE
            )
        };

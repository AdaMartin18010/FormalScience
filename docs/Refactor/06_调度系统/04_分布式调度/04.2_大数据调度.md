...
related: ["Matter: Spark调度", "Matter: Flink调度", "FormalRE: 大数据调度理论", "04.1 集群调度", "04.3 跨层调度协同"]
math: false
code: true
...
> **交叉引用**: 源Matter中的大数据调度文档
>
> - [Matter: Spark调度](../../Matter/02_分布式系统/02.6_Spark调度.md)
> - [Matter: Flink调度](../../Matter/02_分布式系统/02.7_Flink调度.md)
> - [FormalRE: 大数据调度理论](../../FormalRE/分布式系统/大数据调度理论.md)
    ...
    void on_stage_completion(int stage_id) {
        ...
        for (int waiting : waiting_stages_) {
            ...
            if (std::all_of(stage.parent_stages.begin(),
                           ...
                           [this](int p) { return completed_stages_.count(p); })) {

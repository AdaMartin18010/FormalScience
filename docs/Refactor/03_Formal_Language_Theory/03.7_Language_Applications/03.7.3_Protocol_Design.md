...
- 形式语义视角：状态机/时序逻辑/过程代数提供可验证规范。
- 安全视角：攻击者模型（Dolev–Yao/实作侧信道）决定证明强度。
- 分布式视角：一致性/容错假设直接影响正确性与性能。

### 局限性

- 规格—实现差距：实现细节（序列号、重试、超时）常成漏洞根源。
- 可组合性：多协议叠加后的交互效应难以完全验证。
...
- 零信任网络、车联网/工控协议、安全支付与区块链共识。

### 改进建议

- 规格驱动生成与精化（refinement）工具链；
- 协议fuzzer与形式化模型的联动测试；
- 运行时监控与证据化审计闭环。

...
- 与操作语义：公理语义以断言与规则推理为中心，擅长证明全局性质；需保证与小步/大步规则的健全性与完备性（相对完备）。
- 与指称语义：通过语义函数刻画断言真值，可用充分性/充足性链接逻辑与语义；在不动点与递归上需精心处理。
- 与代数语义：逻辑公理可视作代数定律，代数语义提供规则的模型化解释。

### 局限与争议

- 循环不变式：发现与维护不变式成本高、自动化难度大。
- 部分正确性：经典霍尔逻辑强调部分正确性；终止性需额外手段（变参法、良基递归）。
- 现实特性：并发、异常、I/O、安全策略等需扩展（如 Rely/Guarantee、分离逻辑、程序逻辑的效果系统）。

### 应用前景

- 形式验证：嵌入式/安全关键系统的规范化验证与证据生成。
- 合同式设计：以前/后置条件和不变式驱动设计、测试与运行时监测。
- 工具生态：与 SMT/定理证明器结合，实现半自动到全自动验证流程。

### 实践建议

- 逻辑选型：按场景选择分离逻辑、时序逻辑、概率逻辑等变体。
- 战术化：沉淀不变式发现模板与证明战术库，提升复用度。
- 与语义合流：与操作/指称语义建立对齐的参考模型，形成可信闭环。

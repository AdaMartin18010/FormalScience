...
class BeliefModel:
    ...
    def satisfies(self, world: World, formula: str, agent: str = None) -> bool:
        ...
        if formula.startswith('B['):
            ...
            ag = formula[2:end]
            inner = formula[end+2:-1]  # 去掉 B[agent]( 和 )
...
if __name__ == "__main__":
    ...
    # 验证公式
    print(f"w1 ⊨ B[alice](p): {model.satisfies(w1, 'B[alice](p)')}")
    print(f"w3 ⊨ B[alice](p): {model.satisfies(w3, 'B[alice](p)')}")
    print(f"w1 ⊨ B[alice](!p): {model.satisfies(w1, 'B[alice](!p)')}")

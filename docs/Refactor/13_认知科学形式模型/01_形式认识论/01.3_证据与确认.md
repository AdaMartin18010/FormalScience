...
class BayesianLearning:
    ...
    def update(self, data: str):
        ...
        # 计算证据的边缘概率
        P_data = sum(
            self.likelihood_model[h](data) * self.posteriors[h]
        ...
        new_posteriors = {}
        for h in self.hypotheses:
            P_data_given_h = self.likelihood_model[h](data)
    ...
    def posterior_predictive(self, new_data: str) -> float:
        ...
        """
        return sum(
            self.likelihood_model[h](new_data) * self.posteriors[h]
            for h in self.hypotheses
        )

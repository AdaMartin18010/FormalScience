...
difficulty: "初级"
status: "已完成"
tags: ["λ演算", "图灵机", "可计算性", "函数式", "递归论", "元数学"]
related: ["1.3 递归论与可计算性", "目录", "1.3.1 引言", "1.3.2 图灵机", "1.3.2.1 形式化定义"]
math: true
code: true
...
本文档深入探讨递归论与可计算性的核心原理和关键方法。内容涵盖元数学领域的主要知识点，包括λ演算, 图灵机, 可计算性, 函数式, 递归论等关键主题。适合初学者建立基础知识体系。
...
| $\Sigma_{n+1}^0$ | $\exists x_1 \cdots \exists x_n \forall y_1 \cdots \forall y_m \varphi$ | 高阶存在算术层级 |
| $\Pi_{n+1}^0$ | $\forall x_1 \cdots \forall x_n \exists y_1 \cdots \exists y_m \varphi$ | 高阶全称算术层级 |
| $\Delta_n^0$ | $\Sigma_n^0 \cap \Pi_n^0$ | 同时可表示为 $\Sigma_n^0$ 和 $\Pi_n^0$ 的集合 |

...
- [05.1.4 系统辨识理论](#0514-系统辨识理论)
  ...
  - [📚 目录](#-目录-1)
  - [3 基本概念](#3-基本概念)
    - [3.1 系统辨识定义](#31-系统辨识定义)
    - [3.2 辨识问题分类](#32-辨识问题分类)
    - [3.3 辨识准则](#33-辨识准则)
  - [4 参数辨识](#4-参数辨识)
    - [4.1 最小二乘辨识](#41-最小二乘辨识)
    - [4.2 递推最小二乘](#42-递推最小二乘)
    - [4.3 最大似然辨识](#43-最大似然辨识)
  - [5 状态估计](#5-状态估计)
    - [5.1 卡尔曼滤波](#51-卡尔曼滤波)
    - [5.2 扩展卡尔曼滤波](#52-扩展卡尔曼滤波)
    - [5.3 粒子滤波](#53-粒子滤波)
  - [6 模型结构辨识](#6-模型结构辨识)
    - [6.1 模型阶数辨识](#61-模型阶数辨识)
    - [6.2 模型结构选择](#62-模型结构选择)
  - [7 递归辨识](#7-递归辨识)
    - [7.1 递归最小二乘](#71-递归最小二乘)
    - [7.2 递归最大似然](#72-递归最大似然)
  - [8 鲁棒辨识](#8-鲁棒辨识)
    - [8.1 鲁棒辨识问题](#81-鲁棒辨识问题)
    - [8.2 H辨识](#82-h辨识)
    - [8.3 集员辨识](#83-集员辨识)
  - [9 非线性系统辨识](#9-非线性系统辨识)
    - [9.1 非线性模型结构](#91-非线性模型结构)
    - [9.2 非线性参数辨识](#92-非线性参数辨识)
  - [10 模型验证](#10-模型验证)
    - [10.1 残差分析](#101-残差分析)
    - [10.2 交叉验证](#102-交叉验证)
    - [10.3 模型比较](#103-模型比较)
  - [11 代码实现](#11-代码实现)
    - [11.1 系统辨识器](#111-系统辨识器)
    - [11.2 测试代码](#112-测试代码)
  - [12 应用示例](#12-应用示例)
    - [12.1 线性系统辨识](#121-线性系统辨识)
    - [12.2 非线性系统辨识](#122-非线性系统辨识)
    - [12.3 状态估计示例](#123-状态估计示例)
  - [13 相关理论](#13-相关理论)
    - [13.1 控制理论](#131-控制理论)
    - [13.2 状态估计理论](#132-状态估计理论)
    - [13.3 自适应控制理论](#133-自适应控制理论)
    - [13.4 鲁棒控制理论](#134-鲁棒控制理论)
  - [12. 参考文献](#12-参考文献)
  - [14 批判性分析](#14-批判性分析)
...
1. **Ljung, L.** (1999). _System Identification: Theory for the User_. Prentice Hall.
2. **Söderström, T., & Stoica, P.** (1989). _System Identification_. Prentice Hall.
3. **Nelles, O.** (2001). _Nonlinear System Identification: From Classical Approaches to Neural Networks and Fuzzy Models_. Springer.
4. **Anderson, B. D. O., & Moore, J. B.** (1979). _Optimal Filtering_. Prentice Hall.
5. **Haykin, S.** (2001). _Kalman Filtering and Neural Networks_. Wiley.
6. **Doucet, A., de Freitas, N., & Gordon, N.** (2001). _Sequential Monte Carlo Methods in Practice_. Springer.
7. **Akaike, H.** (1974). _A New Look at the Statistical Model Identification_. IEEE Transactions on Automatic Control.
8. **Schwarz, G.** (1978). _Estimating the Dimension of a Model_. The Annals of Statistics.
9. **Rissanen, J.** (1978). _Modeling by Shortest Data Description_. Automatica.
10. **Zhou, K., & Doyle, J. C.** (1998). _Essentials of Robust Control_. Prentice Hall.
...
**统计视角**：以估计量无偏性、一致性、方差最小为目标，理论严谨但假设（独立同分布、噪声高斯性）在工程中常被破坏。

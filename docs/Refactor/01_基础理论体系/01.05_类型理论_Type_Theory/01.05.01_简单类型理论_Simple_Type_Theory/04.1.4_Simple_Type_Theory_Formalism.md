...
- [04.1.4 简单类型理论（形式化）](#0414-简单类型理论形式化)
  - [📋 目录](#-目录)
  - [1 概述](#1-概述)
  - [2 核心目标](#2-核心目标)
  - [3 基本概念](#3-基本概念)
    - [3.1 简单类型理论的定义](#31-简单类型理论的定义)
    - [3.2 基本类型](#32-基本类型)
    - [3.3 类型系统的作用](#33-类型系统的作用)
  - [4 形式化定义](#4-形式化定义)
    - [4.1 类型语法](#41-类型语法)
    - [4.2 项语法](#42-项语法)
    - [4.3 类型环境](#43-类型环境)
    - [4.4 类型推导规则](#44-类型推导规则)
    - [4.5 类型安全](#45-类型安全)
  - [5 定理与证明](#5-定理与证明)
    - [5.1 类型保持性](#51-类型保持性)
    - [5.2 进展性](#52-进展性)
    ...
    - [5.4 主体归约](#54-主体归约)
  - [6 代码实现](#6-代码实现)
    - [6.1 简单类型理论实现](#61-简单类型理论实现)
  - [7 应用示例](#7-应用示例)
    - [7.1 基本类型推导](#71-基本类型推导)
    - [7.2 复杂类型分析](#72-复杂类型分析)
    - [7.3 类型等价检查](#73-类型等价检查)
  - [8 相关理论](#8-相关理论)
    - [8.1 与λ演算的关系](#81-与λ演算的关系)
    - [8.2 与逻辑的关系](#82-与逻辑的关系)
    - [8.3 与编程语言的关系](#83-与编程语言的关系)
    - [8.4 与范畴论的关系](#84-与范畴论的关系)
  - [7. 参考文献](#7-参考文献)
  - [9 批判性分析](#9-批判性分析)
...
1. **Barendregt, H. P.** (1992). _Lambda Calculi with Types_. Oxford University Press.
2. **Pierce, B. C.** (2002). _Types and Programming Languages_. MIT Press.
3. **Girard, J.-Y., Lafont, Y., Taylor, P.** (1989). _Proofs and Types_. Cambridge University Press.
4. **Thompson, S.** (1991). _Type Theory and Functional Programming_. Addison-Wesley.
5. **Cardelli, L., Wegner, P.** (1985). "On Understanding Types, Data Abstraction, and Polymorphism". _Computing Surveys_.
6. **Reynolds, J. C.** (1983). "Types, Abstraction and Parametric Polymorphism". _Information Processing_.
7. **Mitchell, J. C.** (1996). _Foundations for Programming Languages_. MIT Press.
8. **Winskel, G.** (1993). _The Formal Semantics of Programming Languages_. MIT Press.

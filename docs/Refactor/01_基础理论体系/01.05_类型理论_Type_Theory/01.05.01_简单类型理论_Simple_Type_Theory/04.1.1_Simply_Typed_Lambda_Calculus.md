...
- [04.1.1 简单类型λ演算](#0411-简单类型λ演算)
  - [📋 目录](#-目录)
  - [1 概述](#1-概述)
  - [2 核心目标](#2-核心目标)
  - [3 基本概念](#3-基本概念)
    - [3.1 简单类型λ演算的直观理解](#31-简单类型λ演算的直观理解)
    - [3.2 STLC的基本构造](#32-stlc的基本构造)
  - [4 形式化定义](#4-形式化定义)
    - [4.1 类型定义](#41-类型定义)
    - [4.2 项定义](#42-项定义)
    - [4.3 类型环境](#43-类型环境)
    - [4.4 类型判断](#44-类型判断)
  - [5 定理与证明](#5-定理与证明)
    - [5.1 类型保持定理](#51-类型保持定理)
    - [5.2 类型安全定理](#52-类型安全定理)
    - [5.3 强正规化定理](#53-强正规化定理)
    - [5.4 主体归约定理](#54-主体归约定理)
  - [6 类型推导](#6-类型推导)
    - [6.1 类型推导算法](#61-类型推导算法)
    - [6.2 类型检查算法](#62-类型检查算法)
    - [6.3 类型推断算法](#63-类型推断算法)
  - [7 代码实现](#7-代码实现)
  ...
  - [8 应用示例](#8-应用示例)
    ...
    - [8.3 柯里化](#83-柯里化)
  - [9 相关理论](#9-相关理论)
    - [9.1 与无类型λ演算的关系](#91-与无类型λ演算的关系)
    - [9.2 与逻辑的关系](#92-与逻辑的关系)
    - [9.3 与编程语言的关系](#93-与编程语言的关系)
  - [8. 参考文献](#8-参考文献)
  - [10 批判性分析](#10-批判性分析)
...
5. Sørensen, M. H., & Urzyczyn, P. (2006). _Lectures on the Curry-Howard Isomorphism_. Elsevier.

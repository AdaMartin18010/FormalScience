...
- [04.4.4 依赖类型系统](#0444-依赖类型系统)
  ...
  - [3 基本概念](#3-基本概念)
    ...
    - [3.2 依赖类型系统特征](#32-依赖类型系统特征)
  - [4 形式化定义](#4-形式化定义)
    - [4.1 依赖类型系统](#41-依赖类型系统)
    - [4.2 类型族系统](#42-类型族系统)
  - [5 定理与证明](#5-定理与证明)
    - [5.1 依赖类型系统基本定理](#51-依赖类型系统基本定理)
    - [5.2 类型族定理](#52-类型族定理)
  - [6 代码实现](#6-代码实现)
    - [6.1 依赖类型系统基础实现](#61-依赖类型系统基础实现)
    - [6.2 依赖模式匹配实现](#62-依赖模式匹配实现)
    - [6.3 依赖类型推理实现](#63-依赖类型推理实现)
  - [7 应用示例](#7-应用示例)
    - [7.1 依赖类型系统使用](#71-依赖类型系统使用)
  - [8 相关理论](#8-相关理论)
...
- 哲学维度：依赖类型系统作为逻辑与计算的统一框架；类型作为命题的构造性解释。
- 方法论维度：依赖类型系统的设计复杂性；类型检查与相等判定的算法。
- 工程维度：依赖类型系统在定理证明中的应用；与经典编程语言的互操作。
- 社会技术维度：依赖类型系统的学习曲线；在工业级软件中的采用策略。

...
基础控制理论是控制科学的核心，研究如何设计控制器使系统达到期望的行为。它包括系统建模、稳定性分析、控制器设计等基本概念，为现代控制理论奠定基础。
...
控制系统是由被控对象、控制器、传感器和执行器组成的闭环系统，目的是使系统输出跟踪期望的参考信号。
...
**定理 3.1.1** (李雅普诺夫稳定性)
系统 $\dot{x} = f(x)$ 在平衡点 $x_e$ 处渐近稳定，如果存在正定函数 $V(x)$ 使得：
$$\dot{V}(x) = \frac{\partial V}{\partial x} f(x) < 0$$

**证明**：
由李雅普诺夫稳定性理论，如果存在李雅普诺夫函数 $V(x)$ 满足条件，则系统渐近稳定。

**定理 3.1.2** (线性系统稳定性)
线性系统 $\dot{x} = Ax$ 渐近稳定当且仅当 $A$ 的所有特征值都有负实部。

**证明**：

1. **必要性**：如果系统稳定，则状态响应 $x(t) = e^{At}x(0)$ 趋于零，要求 $A$ 的特征值有负实部
2. **充分性**：如果 $A$ 的特征值有负实部，则 $e^{At}$ 趋于零，系统稳定

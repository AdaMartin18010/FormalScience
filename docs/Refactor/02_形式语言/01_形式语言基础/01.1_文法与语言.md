...
status: "已完成"
tags: ["形式语言基础", "形式语言"]
related: ["01.2 有限自动机", "01.3 下推自动机", "01.4 图灵机与计算", "04.1 自动机理论", "14.1.1 Chomsky 层次"]
math: true
code: true
...
本文档深入探讨文法与语言的核心原理和关键方法。内容涵盖形式语言基础领域的主要知识点，包括相关理论、方法及应用。适合初学者建立基础知识体系。
...
形式语言理论相关的Lean形式化代码，参见：

- 📄 [`examples/lean/FiniteAutomaton.lean`](../../../examples/lean/FiniteAutomaton.lean) - 有限自动机与正则语言
- 📄 [`examples/lean/LambdaCalculus.lean`](../../../examples/lean/LambdaCalculus.lean) - λ演算与Chomsky层次
...
theorem regular_union {A : Type} (L1 L2 : List A → Bool)
    ...
    IsRegularLanguage (λ w => L1 w || L2 w)

-- Myhill-Nerode定理
theorem myhill_nerode {A : Type} (L : List A → Bool) :
  IsRegularLanguage L ↔
  ∃ (n : Nat), ∀ (w : List A),
    (List.filter (λ x => Indistinguishable L x w) (allStringsUpTo n)).length ≤ n
```
...
- [14.1.1 Chomsky 层次](./14_形式语言学/01_形式语法/01.1_Chomsky层次.md)
- [01 形式语法](./14_形式语言学/01_形式语法.md)
- [01.2 有限自动机](../01_形式语言基础/01.2_有限自动机.md)

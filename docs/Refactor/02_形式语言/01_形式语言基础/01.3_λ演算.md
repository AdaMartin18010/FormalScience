...
related: ["1.3 λ演算 (Lambda Calculus)", "目录", "1.3.1 引言", "1.3.2 无类型λ演算", "1.3.2.1 语法定义"]
math: true
code: true
...
本文档深入探讨λ演算的核心原理和关键方法。内容涵盖形式语言基础领域的主要知识点，包括函数式, λ演算, 归约等关键主题。适合初学者建立基础知识体系。
...
- [1.3 λ演算 (Lambda Calculus)](#13-λ演算-lambda-calculus)
  ...
  - [1.3.7 总结](#137-总结)
  - [1.3.8 代码示例](#138-代码示例)
    - [Lean 4：λ演算的完整形式化](#lean-4λ演算的完整形式化)
...
完整的λ演算形式化定义，包括Church-Rosser定理和Church编码，参见：
📄 [`examples/lean/LambdaCalculus.lean`](../../../examples/lean/LambdaCalculus.lean)
...
inductive Beta1 : Term → Term → Prop where
  | beta (x : String) (t s : Term) :
      Beta1 (.app (.abs x t) s) (subst t x s)
  | appL {t₁ t₁' t₂ : Term} :
...
完整的λ演算形式化代码（包含语法定义、β归约、Church-Rosser定理框架）：
...
theorem church_rosser : Confluence Beta1 := by
  sorry  -- 证明涉及并行归约和Tait-Martin-Löf方法
```

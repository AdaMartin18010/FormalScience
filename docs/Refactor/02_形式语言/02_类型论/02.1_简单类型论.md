---
title: "2.1 简单类型论 (Simply Typed Lambda Calculus)"
category: "形式语言"
subcategory: "类型论"
order: 2
difficulty: "初级"
status: "已完成"
tags: ["类型论", "λ演算", "简单类型", "类型安全", "形式语言"]
related: ["2.1 简单类型论 (Simply Typed Lambda Calculus)", "目录", "2.1.1 引言", "2.1.2 STLC的语法", "2.1.2.1 类型语法"]
math: true
code: true
...
本文档深入探讨简单类型论的核心原理和关键方法。内容涵盖类型论领域的主要知识点，包括λ演算, 简单类型, 类型安全等关键主题。适合初学者建立基础知识体系。
...
完整的简单类型λ演算形式化代码（包含类型推导、类型安全性定理）：

📄 [`examples/lean/SimplyTypedLambda.lean`](../../../examples/lean/SimplyTypedLambda.lean)
...
inductive Typing : Context → Term → Ty → Prop where
  | var {Γ x τ} :
      Γ.lookup x = some τ → Typing Γ (.var x) τ
  | abs {Γ x τ₁ t τ₂} :
      Typing ((x, τ₁) :: Γ) t τ₂ →
...
theorem typing_uniqueness {Γ : Context} {t : Term} {τ₁ τ₂ : Ty}
  ...
  induction h₁ generalizing τ₂ with
  | var h => cases h₂; simp_all
  | abs _ ih => cases h₂ with | abs h₂' => simp [ih h₂']
  | app _ _ ih₁ ih₂ => cases h₂ with | app h₃ h₄ => simp [ih₁ h₃]

-- 类型安全性：进展性 + 类型保持
theorem progress {t τ} (ht : [] ⊢ t : τ) :
    Value t ∨ ∃ t', t →₁ t' := by sorry
...
- [1. 惰性求值](./03_编程范式/04_函数式编程/04.3_惰性求值.md)
- [04.4 惰性求值](./03_编程范式/04_函数式编程/04.4_惰性求值.md)

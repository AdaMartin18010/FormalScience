...
difficulty: "中级"
status: "已完成"
tags: ["类型抽象", "多态类型", "类型论", "System F", "形式语言"]
related: ["02.1 简单类型系统", "02.3 依赖类型", "02.4 类型论与逻辑", "04.3 伴随与单子", "02.1 简单类型系统"]
math: true
code: true
...
本文档深入探讨多态类型的核心原理和关键方法。内容涵盖类型论领域的主要知识点，包括类型抽象, 多态类型, System F等关键主题。适合有一定基础的学习者系统学习。
...
- $R_{\forall X. A}(t, u) := \forall B, C, R \subseteq B \times C. R_AX:=R$

## 6. 代码示例

### Lean形式化代码

完整的系统F（多态λ演算）形式化定义，参见：
📄 `examples/lean/PolymorphicTypes.lean`
...
inductive Term where
  ...
  | app : Term → Term → Term
  | tabs : TyVar → Term → Term   -- Λα. M
  | tapp : Term → Ty → Term      -- M [τ]

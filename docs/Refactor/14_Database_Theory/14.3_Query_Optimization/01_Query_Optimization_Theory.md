...
- 多元理论视角：
  - 组合爆炸与近似：优化本质为组合搜索（NP难），需启发式、代价估计与规则/代价混合；语义/代数重写与统计学习可协同。
  - 概率与不确定性：统计失真、相关性与倾斜导致代价模型偏差；贝叶斯/学习式代价模型缓解估计误差。
- 局限性分析：
  - 统计维护成本：直方图/多维统计与收集代价、实时性矛盾；采样与自适应执行是折中。
  - 规则过拟合：规则过多易耦合底层执行器细节，降低可移植性。
  - 黑盒执行器：优化与执行解耦导致反馈不足，难以在线修正错误计划。
- 争议与分歧：
  - 规则优先还是代价优先：工业界多采用混合；但优先级与冲突解决策略存在分歧。
  - 学习优化器：可解释性、冷启动与回退机制是核心争议点。
- 应用前景：
  - 自适应查询：运行时重优化、阶段性重新选择连接顺序与算子并行度。
  - 学习增强：用学习模型提供先验分布与置信区间，优化器在高不确定性区域保守选取。
  - 异构执行：GPU/FPGA/存储内计算纳入代价模型与物理算子库。
- 改进建议：
  - 反馈回路：采集实际代价与选择率，在线校准统计与代价模型。
  - 鲁棒计划：在统计不确定时选择方差较小的鲁棒计划，而非仅最小期望值。
  - 证据与基准：输出计划、统计、实际代价对照证据；维护基准与回归套件。

...
- 多元理论视角：
  - 形式模型对齐：把事务调度/锁/日志在形式模型上与冲突图、等待图、可串行化理论、线性时态逻辑性质对应，建立“规格—实现—验证”链路。
  - 工程与理论统一：ARIES/2PL/MVCC 等工程方案可被抽象为可验证协议族，便于跨存储引擎比较与复验。
- 局限性分析：
  - 可串行化代价：严格可串行化在大规模分布式场景下成本高；实际多采用快照隔离等弱化模型，带来异常与权衡。
  - 跨区域与异构：跨机房时钟漂移、网络分区、存储异构使得全局一致性难以维持；需结合共识与因果一致性设计。
  - 日志与恢复开销：写前日志、检查点频率与IO放大、尾延迟之间存在张力。
- 争议与分歧：
  - 一致性级别选择：强一致/快照隔离/读已提交的选择依业务而定；“默认级别”具有争议且易被误用。
  - 悲观 vs. 乐观控制：高冲突下悲观优；低冲突/读多写少下乐观优，需有自适应切换策略。
- 应用前景：
  - 硬件卸载与加速：NVRAM/RDMA/可编程网卡可降低日志与一致性成本。
  - 共识融合：将事务与共识层（Paxos/Raft）紧耦合，形成原生分布式事务方案。
  - 可验证实现：形式化规格 + 模型检查/定理证明，输出可复验证书。
- 改进建议：
  - 明确级别与异常矩阵：为各隔离级别列出异常/保证矩阵与使用指引。
  - 自适应并发控制：基于冲突/热点检测在2PL、OCC、MVCC间切换或混合。
  - 证据产物化：产出恢复/回放脚本、崩溃一致性测试与证书，纳入CI。

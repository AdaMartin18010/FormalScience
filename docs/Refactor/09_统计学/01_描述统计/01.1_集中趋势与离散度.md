...
描述统计学的核心任务是通过少量数值指标概括数据的基本特征。**集中趋势**（Central Tendency）度量数据的中心位置，而**离散度**（Dispersion）度量数据的分散程度。本章基于Casella & Berger (2002) 和 Gelman et al. (2013) 的形式化框架，给出这些统计量的严格数学定义。

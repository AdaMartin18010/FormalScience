...
- [1. 加密、认证与授权的深度分析](#1-加密认证与授权的深度分析)
  - [目录](#目录)
  - [1.1 引言](#11-引言)
  - [1.2 核心概念](#12-核心概念)
    - [1.2.1 加密 (Encryption)](#121-加密-encryption)
    - [1.2.2 认证 (Authentication - Auth)](#122-认证-authentication---auth)
    - [1.2.3 授权 (Authorization - 鉴权)](#123-授权-authorization---鉴权)
  - [1.3 编程语言视角分析](#13-编程语言视角分析)
    - [1.3.1 变量与类型系统](#131-变量与类型系统)
      - [1.3.1.1 定义与概念 (变量类型)](#1311-定义与概念-变量类型)
      - [1.3.1.2 在安全机制中的应用 (变量类型)](#1312-在安全机制中的应用-变量类型)
      - [1.3.1.3 形式化考量 (变量类型)](#1313-形式化考量-变量类型)
      - [1.3.1.4 代码示例 (Rust/Go) (变量类型)](#1314-代码示例-rustgo-变量类型)
    - [1.3.2 控制结构与语法语义](#132-控制结构与语法语义)
      - [1.3.2.1 定义与概念 (控制结构)](#1321-定义与概念-控制结构)
      - [1.3.2.2 对安全逻辑的影响 (控制结构)](#1322-对安全逻辑的影响-控制结构)
      - [1.3.2.3 形式化考量 (控制结构)](#1323-形式化考量-控制结构)
      - [1.3.2.4 代码示例 (Rust/Go) (控制结构)](#1324-代码示例-rustgo-控制结构)
    - [1.3.3 作用域 (Scope)](#133-作用域-scope)
      - [1.3.3.1 定义与概念 (作用域)](#1331-定义与概念-作用域)
      - [1.3.3.2 与安全上下文的关系 (作用域)](#1332-与安全上下文的关系-作用域)
      - [1.3.3.3 形式化考量 (作用域)](#1333-形式化考量-作用域)
  - [1.4 流分析视角](#14-流分析视角)
    - [1.4.1 控制流分析 (Control Flow Analysis - CFA)](#141-控制流分析-control-flow-analysis---cfa)
      - [1.4.1.1 定义与概念 (CFA)](#1411-定义与概念-cfa)
      - [1.4.1.2 在安全策略执行中的作用 (CFA)](#1412-在安全策略执行中的作用-cfa)
      - [1.4.1.3 形式化验证思路 (CFA)](#1413-形式化验证思路-cfa)
      - [1.4.1.4 代码示例 (Rust/Go) (CFA)](#1414-代码示例-rustgo-cfa)
    - [1.4.2 数据流分析 (Data Flow Analysis - DFA)](#142-数据流分析-data-flow-analysis---dfa)
      - [1.4.2.1 定义与概念 (DFA)](#1421-定义与概念-dfa)
      - [1.4.2.2 追踪敏感数据与权限 (DFA)](#1422-追踪敏感数据与权限-dfa)
      - [1.4.2.3 形式化验证思路 (DFA)](#1423-形式化验证思路-dfa)
      - [1.4.2.4 代码示例 (Rust/Go) (DFA - 污点分析概念)](#1424-代码示例-rustgo-dfa---污点分析概念)
    - [1.4.3 执行流分析 (Execution Flow Analysis - Sync/Async)](#143-执行流分析-execution-flow-analysis---syncasync)
      - [1.4.3.1 定义与概念 (执行流)](#1431-定义与概念-执行流)
      - [1.4.3.2 并发/异步对安全的影响 (执行流)](#1432-并发异步对安全的影响-执行流)
      - [1.4.3.3 形式化考量 (执行流)](#1433-形式化考量-执行流)
      - [1.4.3.4 代码示例 (Rust/Go) (执行流 - 竞争条件)](#1434-代码示例-rustgo-执行流---竞争条件)
  - [1.5 形式化方法与验证](#15-形式化方法与验证)
    - [1.5.1 概念与定义 (形式化)](#151-概念与定义-形式化)
    - [1.5.2 在安全领域的应用 (形式化)](#152-在安全领域的应用-形式化)
    - [1.5.3 形式化证明概览 (形式化)](#153-形式化证明概览-形式化)
    - [1.5.4 局限性 (形式化)](#154-局限性-形式化)
  - [1.6 元理论与元模型](#16-元理论与元模型)
    - [1.6.1 概念解释 (元)](#161-概念解释-元)
    - [1.6.2 在安全建模中的意义 (元)](#162-在安全建模中的意义-元)
  - [1.7 关联性与层次分析](#17-关联性与层次分析)
    - [1.7.1 各视角之间的关联](#171-各视角之间的关联)
    - [1.7.2 不同抽象层次的模型](#172-不同抽象层次的模型)
  - [1.8 总结](#18-总结)
  - [1.9 思维导图 (Text)](#19-思维导图-text)
...
### 1.4.1 控制流分析 (Control Flow Analysis - CFA)
...
#### 1.4.1.4 代码示例 (Rust/Go) (CFA)

这里的代码示例主要是为了 _说明_ CFA 的概念，实际的 CFA 通常由静态分析工具执行，而不是手写代码。
...
### 1.4.2 数据流分析 (Data Flow Analysis - DFA)
...
#### 1.4.2.4 代码示例 (Rust/Go) (DFA - 污点分析概念)
...
### 1.4.3 执行流分析 (Execution Flow Analysis - Sync/Async)

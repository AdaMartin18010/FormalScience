...
- [加密](#加密)
  - [1 . 引言：核心概念](#1-引言核心概念)
  - [1. 引言：核心概念](#1-引言核心概念)
    - [1 加密 (Cryptography)](#1-加密-cryptography)
    - [1.2 验证 (Authentication)](#12-验证-authentication)
    - [1.3 授权 (Authorization)](#13-授权-authorization)
  ...
  - [3 . 程序执行流与安全分析](#3-程序执行流与安全分析)
    ...
    - [3.4 同步与异步 (Sync/Async)](#34-同步与异步-syncasync)
  - [4 . 形式化方法与证明](#4-形式化方法与证明)
    - [1 概念与定义](#1-概念与定义)
    - [4.2 在安全领域的应用](#42-在安全领域的应用)
    - [4.3 形式化推理与证明](#43-形式化推理与证明)
  - [5 . 元理论与分层模型](#5-元理论与分层模型)
    - [1 元模型 (Metamodel) 与模型 (Model)](#1-元模型-metamodel-与模型-model)
    - [5.2 元理论 (Metatheory) 与理论 (Theory)](#52-元理论-metatheory-与理论-theory)
    - [5.3 分层分析](#53-分层分析)
  - [6 . 思维导图 (Text)](#6-思维导图-text)
  - [7 . 总结](#7-总结)
  - [8 . 语言特性与具体漏洞类型的关联](#8-语言特性与具体漏洞类型的关联)
    - [1 内存安全与缓冲区溢出/Use-After-Free](#1-内存安全与缓冲区溢出use-after-free)
    - [8.2 类型系统与类型混淆/注入](#82-类型系统与类型混淆注入)
    - [8.3 控制流与逻辑漏洞/旁路](#83-控制流与逻辑漏洞旁路)
    - [8.4 并发与竞态条件 (TOCTOU)](#84-并发与竞态条件-toctou)
  - [9 . 安全工程实践：整合分析](#9-安全工程实践整合分析)
    - [1 安全开发生命周期 (SDL)](#1-安全开发生命周期-sdl)
    - [9.2 威胁建模 (Threat Modeling)](#92-威胁建模-threat-modeling)
    - [9.3 安全编码规范](#93-安全编码规范)
    - [9.4 代码审计与静态/动态分析](#94-代码审计与静态动态分析)
  - [10 . 形式化方法的深化与挑战](#10-形式化方法的深化与挑战)
    - [1 规约的挑战](#1-规约的挑战)
    - [10.2 验证的范围与成本](#102-验证的范围与成本)
    - [10.3 假设与现实世界](#103-假设与现实世界)

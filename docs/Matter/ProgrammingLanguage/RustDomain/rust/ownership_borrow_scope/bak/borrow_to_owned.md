...
- [ToOwned](#toowned)
  ...
  - [3 使用示例](#3-使用示例)
  - [4 总结](#4-总结)
  - [5 ToOwned Trait 的定义](#5-toowned-trait-的定义)
  - [6 为什么不区分可变和不可变借用](#6-为什么不区分可变和不可变借用)
  - [7 代码示例](#7-代码示例)
    - [7.1 示例 1：直接从不可变借用生成拥有数据](#71-示例-1直接从不可变借用生成拥有数据)
    - [7.2 示例 2：从可变引用调用 to\_owned()](#72-示例-2从可变引用调用-to_owned)
  - [8 总结](#8-总结)

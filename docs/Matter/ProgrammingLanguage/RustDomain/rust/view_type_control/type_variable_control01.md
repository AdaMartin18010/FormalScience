# 1. 从形式理论视角看程序语言的类型、变量与控制

## 目录

- [1. 从形式理论视角看程序语言的类型、变量与控制](#1-从形式理论视角看程序语言的类型变量与控制)
  - [目录](#目录)
  - [1.1 引言：形式理论与程序设计语言](#11-引言形式理论与程序设计语言)
    - [1.1.1 形式系统的本质](#111-形式系统的本质)
    - [1.1.2 程序语言作为形式系统的实现](#112-程序语言作为形式系统的实现)
  - [1.2 类型系统的形式理论基础](#12-类型系统的形式理论基础)
    - [1.2.1 类型论视角：命题即类型，证明即程序](#121-类型论视角命题即类型证明即程序)
    - [1.2.2 依值类型论与Rust类型系统](#122-依值类型论与rust类型系统)
    - [1.2.3 代数数据类型的范畴论解释](#123-代数数据类型的范畴论解释)
    - [1.2.4 参数多态与通用量化](#124-参数多态与通用量化)
    - [1.2.5 子类型与存在量化](#125-子类型与存在量化)
  - [1.3 变量与状态管理的形式化](#13-变量与状态管理的形式化)
    - [1.3.1 线性逻辑与资源管理](#131-线性逻辑与资源管理)
    - [1.3.2 所有权模型的线性逻辑解释](#132-所有权模型的线性逻辑解释)
    - [1.3.3 借用系统的分离逻辑模型](#133-借用系统的分离逻辑模型)
    - [1.3.4 生命周期：时态逻辑视角](#134-生命周期时态逻辑视角)
  - [1.4 控制流的形式理论表达](#14-控制流的形式理论表达)
    - [1.4.1 结构化程序定理与控制流](#141-结构化程序定理与控制流)
    - [1.4.2 霍尔逻辑与程序验证](#142-霍尔逻辑与程序验证)
    - [1.4.3 模式匹配作为构造性证明](#143-模式匹配作为构造性证明)
    - [1.4.4 迭代与不动点理论](#144-迭代与不动点理论)
    - [1.4.5 异常处理的集成理论](#145-异常处理的集成理论)
  - [1.5 三位一体：形式系统中的统一视角](#15-三位一体形式系统中的统一视角)
    - [1.5.1 柯里-霍华德同构：类型与证明](#151-柯里-霍华德同构类型与证明)
    - [1.5.2 状态管线：变量转换与演算](#152-状态管线变量转换与演算)
    - [1.5.3 计算即推理：控制流与逻辑推导](#153-计算即推理控制流与逻辑推导)
  - [1.6 实例研究：Rust中的形式化思考](#16-实例研究rust中的形式化思考)
    - [1.6.1 类型安全与借用检查器的形式基础](#161-类型安全与借用检查器的形式基础)
    - [1.6.2 不变量维护与程序正确性](#162-不变量维护与程序正确性)
    - [1.6.3 高级抽象的形式语义](#163-高级抽象的形式语义)
  - [1.7 结论：形式理论指导下的语言设计](#17-结论形式理论指导下的语言设计)
...
## 1.6 实例研究：Rust中的形式化思考

### 1.6.1 类型安全与借用检查器的形式基础

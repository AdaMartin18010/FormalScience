# 1. 堆（Heap）和栈（Stack）是计算机内存管理的基本概念，它们既与硬件体系有关，也与编程语言的实现有关

## 目录

- [1. 堆（Heap）和栈（Stack）是计算机内存管理的基本概念，它们既与硬件体系有关，也与编程语言的实现有关](#堆heap和栈stack是计算机内存管理的基本概念它们既与硬件体系有关也与编程语言的实现有关)
  - [1.1 一般定义](#1-一般定义)
    - [1.1.1 区分](#11-区分)
    - [1.1.2 联系](#12-联系)
    - [1.1.3 解释](#13-解释)
  - [1.2 所有权（Ownership）](#2-所有权ownership)
  - [1.3 借用（Borrowing）](#3-借用borrowing)
  - [1.4 生命周期（Lifetimes）](#4-生命周期lifetimes)
  - [1.5 `Drop` Trait](#5-drop-trait)
  - [1.6 `Clone` Trait](#6-clone-trait)
  - [1.7 `Copy` Trait](#7-copy-trait)
  - [1.8 智能指针（Smart Pointers）](#8-智能指针smart-pointers)
  - [1.9 `Deref` Trait](#9-deref-trait)
  - [1.10 `DerefMut` Trait](#10-derefmut-trait)
  - [1.11 裸指针（Raw Pointers）](#11-裸指针raw-pointers)
  - [1.12 内存分配器（Allocators）](#12-内存分配器allocators)
  - [1.13 栈和堆（Stack and Heap）](#13-栈和堆stack-and-heap)
  - [1.14 RUST内存管理定义](#14-rust内存管理定义)
    - [1.14.1 分类](#141-分类)
  - [1.15 RUST内存管理基本原理与核心概念](#15-rust内存管理基本原理与核心概念)
    - [1.15.1 *所有权（Ownership）*](#151-所有权ownership)
    - [1.15.2 *借用（Borrowing）*](#152-借用borrowing)
    - [1.15.3 *生命周期（Lifetimes）*](#153-生命周期lifetimes)
    - [1.15.4 *RAII 与 Drop 机制*](#154-raii-与-drop-机制)
    - [1.15.5 *智能指针*](#155-智能指针)
  - [1.16 RUST内存管理详细用例说明](#16-rust内存管理详细用例说明)
    - [1.16.1 *所有权与自动释放*](#161-所有权与自动释放)
    - [1.16.2 *借用与生命周期*](#162-借用与生命周期)
    - [1.16.3 *移动语义*](#163-移动语义)
  - [1.17 *思维导图*](#17-思维导图)
  - [1.18 `Box<T>`](#18-boxt)
    - [1.18.1 定义与解释](#181-定义与解释)
    - [1.18.2 用例](#182-用例)
  - [1.19 `Rc<T>`](#19-rct)
  - [1.20 `Arc<T>`](#20-arct)
  - [1.21 思维导图](#21-思维导图)
  - [1.22 栈内存碎片化](#22-栈内存碎片化)
    - [1.22.1 避免栈碎片化](#221-避免栈碎片化)
  - [1.23 堆内存碎片化](#23-堆内存碎片化)
    - [1.23.1 避免堆溢出](#231-避免堆溢出)
    - [1.23.2 避免堆碎片化](#232-避免堆碎片化)
## 1.1 一般定义

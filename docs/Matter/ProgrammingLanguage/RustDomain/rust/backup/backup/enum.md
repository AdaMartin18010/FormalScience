...
- [1. Rust 的 `enum` 支持泛型吗？](#rust-的-enum-支持泛型吗)
  - [1.1 示例](#1-示例)
  - [1.2 enum 是否可以像结构体一样实现某个 trait](#2-enum-是否可以像结构体一样实现某个-trait)
  - [1.3 示例](#3-示例)
  - [1.4 总结](#4-总结)
  - [1.5 enum 支持泛型吗](#5-enum-支持泛型吗)
  - [1.6 支持 impl Trait 吗](#6-支持-impl-trait-吗)
  - [1.7 总结](#7-总结)
  - [1.8 enum 的变体成员能否是 trait 类型](#8-enum-的变体成员能否是-trait-类型)
    - [1.8.1 示例](#81-示例)
    - [1.8.2 关键点](#82-关键点)
  - [1.9 Rust 的 Enum 支持泛型吗](#9-rust-的-enum-支持泛型吗)
    - [1.9.1 例子](#91-例子)
      - [1.9.1.1 泛型枚举](#911-泛型枚举)
      - [1.9.1.2 自定义的泛型枚举](#912-自定义的泛型枚举)
    - [1.9.2 使用](#92-使用)
  - [1.10 总结](#10-总结)
  - [1.11 如何创建递归的 trait](#11-如何创建递归的-trait)
    - [1.11.1 Rust 中的 trait 有哪些相关的限制](#111-rust-中的-trait-有哪些相关的限制)
    - [1.11.2 Trait 与 Struct 和 Enum 的对比](#112-trait-与-struct-和-enum-的对比)
      - [1.11.2.1 Trait 与 Struct 的对比](#1121-trait-与-struct-的对比)
      - [1.11.2.2 Trait 与 Enum 的对比](#1122-trait-与-enum-的对比)
  - [1.12 总结](#12-总结)
  - [1.13 Rust 中的集合类型](#13-rust-中的集合类型)
    - [1.13.1 遵守的 trait](#131-遵守的-trait)
    - [1.13.2 如何统一看待这些类型](#132-如何统一看待这些类型)
    - [1.13.3 Rust 的递归类型表示方法](#133-rust-的递归类型表示方法)
      - [1.13.3.1 使用 BoxT 实现递归类型](#1331-使用-boxt-实现递归类型)
        - [1.13.3.1.1 示例：链表](#13311-示例链表)
      - [1.13.3.2 使用 RcT 和 RefCellT 实现递归类型](#1332-使用-rct-和-refcellt-实现递归类型)
        - [1.13.3.2.1 示例：链表](#13321-示例链表)
      - [1.13.3.3 使用 ArcT 实现线程安全的递归类型](#1333-使用-arct-实现线程安全的递归类型)
        - [1.13.3.3.1 示例：链表](#13331-示例链表)
    - [1.13.4 递归类型的原理](#134-递归类型的原理)
    - [1.13.5 如何有效利用递归类型](#135-如何有效利用递归类型)
      - [1.13.5.1 使用 BoxT 避免栈溢出](#1351-使用-boxt-避免栈溢出)
      - [1.13.5.2 使用 RcT 和 RefCellT 实现共享和内部可变性](#1352-使用-rct-和-refcellt-实现共享和内部可变性)
      - [1.13.5.3 使用 ArcT 实现线程安全的递归类型](#1353-使用-arct-实现线程安全的递归类型)
    - [1.13.6 如何提高递归类型的效率](#136-如何提高递归类型的效率)
      - [1.13.6.1 使用尾递归优化](#1361-使用尾递归优化)
        - [1.13.6.1.1 示例：阶乘计算](#13611-示例阶乘计算)
      - [1.13.6.2 使用迭代代替递归](#1362-使用迭代代替递归)
        - [1.13.6.2.1 示例：斐波那契数列](#13621-示例斐波那契数列)
      - [1.13.6.3 使用缓存和记忆化搜索](#1363-使用缓存和记忆化搜索)
        - [1.13.6.3.1 示例：斐波那契数列](#13631-示例斐波那契数列)
    - [1.13.7 总结](#137-总结)
## 1.1 示例
...
#### 1.13.3.2 使用 RcT 和 RefCellT 实现递归类型

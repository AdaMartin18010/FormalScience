...
- [1. Rust 的递归类型表示方法](#1-rust-的递归类型表示方法)
  - [目录](#目录)
  - [1.1 使用 `Box<T>` 实现递归类型](#11-使用-boxt-实现递归类型)
  - [1.2 使用 `Rc<T>` 和 `RefCell<T>` 实现递归类型](#12-使用-rct-和-refcellt-实现递归类型)
  - [1.3 使用 `Arc<T>` 实现线程安全的递归类型](#13-使用-arct-实现线程安全的递归类型)
  - [1.4 递归类型的原理](#14-递归类型的原理)
  - [1.5 如何有效利用递归类型](#15-如何有效利用递归类型)
    - [1.5.1 **使用 `Box<T>` 避免栈溢出**](#151-使用-boxt-避免栈溢出)
    - [1.5.2 **使用 `Rc<T>` 和 `RefCell<T>` 实现共享和内部可变性**](#152-使用-rct-和-refcellt-实现共享和内部可变性)
    - [1.5.3 **使用 `Arc<T>` 实现线程安全的递归类型**](#153-使用-arct-实现线程安全的递归类型)
  - [1.6 如何提高递归类型的效率](#16-如何提高递归类型的效率)
    - [1.6.1 **使用尾递归优化**](#161-使用尾递归优化)
    - [1.6.2 **使用迭代代替递归**](#162-使用迭代代替递归)
    - [1.6.3 **使用缓存和记忆化搜索**](#163-使用缓存和记忆化搜索)
  - [1.7 总结](#17-总结)

# 1. 在 Rust 中，泛型参数的使用存在以下限制

## 目录

- [1. 在 Rust 中，泛型参数的使用存在以下限制](#1-在-rust-中泛型参数的使用存在以下限制)
  - [目录](#目录)
  - [1.1 **类型限制**](#11-类型限制)
    - [1.1.1 **生命周期限制**](#111-生命周期限制)
    - [1.1.2 **`Sized` 限制**](#112-sized-限制)
    - [1.1.3 **实例化限制**](#113-实例化限制)
    - [1.1.4 **依赖类型和资源管理**](#114-依赖类型和资源管理)
    - [1.1.5 **编译器实现限制**](#115-编译器实现限制)

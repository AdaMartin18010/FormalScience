...
- [1. rust threads](#1-rust-threads)
  - [目录](#目录)
  - [1.1 使用 thread\_local 宏](#11-使用-thread_local-宏)
    - [1.1.1 示例代码](#111-示例代码)
    - [1.1.2 访问线程局部存储](#112-访问线程局部存储)
      - [1.1.2.1 -示例代码](#1121--示例代码)
    - [1.1.3 线程局部存储的生命周期](#113-线程局部存储的生命周期)
    - [1.1.4 线程局部存储的优势](#114-线程局部存储的优势)
    - [1.1.5 线程局部存储的使用场景](#115-线程局部存储的使用场景)
    - [1.1.6 线程局部存储的限制](#116-线程局部存储的限制)
    - [1.1.7 第三方库支持](#117-第三方库支持)
    - [1.1.8 总结](#118-总结)
  - [1.2 PoisonError](#12-poisonerror)
    - [1.2.1 处理 PoisonError](#121-处理-poisonerror)
      - [1.2.1.1 into\_inner 方法](#1211-into_inner-方法)
      - [1.2.1.2 get\_ref 方法](#1212-get_ref-方法)
      - [1.2.1.3 get\_mut 方法](#1213-get_mut-方法)
    - [1.2.2 PoisonError 的机制](#122-poisonerror-的机制)
    - [1.2.3 避免 PoisonError](#123-避免-poisonerror)
  - [1.3 mutex or rwlock](#13-mutex-or-rwlock)
    - [1.3.1 使用 Mutex 保证数据一致性](#131-使用-mutex-保证数据一致性)
      - [1.3.1.1 示例代码：使用 Mutex 保护全局状态](#1311-示例代码使用-mutex-保护全局状态)
    - [1.3.2 使用 RwLock 提高并发性能](#132-使用-rwlock-提高并发性能)
      - [1.3.2.1 示例代码：使用 RwLock 保护共享数据](#1321-示例代码使用-rwlock-保护共享数据)
    - [1.3.3 使用 Arc 实现多线程共享](#133-使用-arc-实现多线程共享)
      - [1.3.3.1 示例代码：使用 Arc 和 Mutex](#1331-示例代码使用-arc-和-mutex)
    - [1.3.4 处理锁竞争和死锁](#134-处理锁竞争和死锁)
    - [1.3.5 使用 PoisonError 处理毒化锁](#135-使用-poisonerror-处理毒化锁)
      - [1.3.5.1 示例代码：处理毒化锁](#1351-示例代码处理毒化锁)
    - [1.3.6 总结](#136-总结)
    - [1.3.7 设计策略](#137-设计策略)
    - [1.3.8 Rust 的保护性机制](#138-rust-的保护性机制)
    - [1.3.9 示例](#139-示例)
      - [1.3.9.1 使用 catch\_unwind 捕获 panic](#1391-使用-catch_unwind-捕获-panic)
      - [1.3.9.2 使用 Mutex 和 PoisonError](#1392-使用-mutex-和-poisonerror)
    - [1.3.10 总结](#1310-总结)
    - [1.3.11 线程取消Thread Cancellation](#1311-线程取消thread-cancellation)
    - [1.3.12 堆栈展开Stack Unwinding](#1312-堆栈展开stack-unwinding)
    - [1.3.13 程序终止](#1313-程序终止)
    - [1.3.14 跨线程影响](#1314-跨线程影响)
    - [1.3.15 如何捕获 panic](#1315-如何捕获-panic)
    - [1.3.16 -总结-](#1316--总结-)
    - [1.3.17 使用 Result 和 Option](#1317-使用-result-和-option)
      - [1.3.17.1 使用 Result 处理错误](#13171-使用-result-处理错误)
      - [1.3.17.2 使用 Option 处理可能缺失的值](#13172-使用-option-处理可能缺失的值)
    - [1.3.18 避免直接调用 unwrap 和 expect](#1318-避免直接调用-unwrap-和-expect)
      - [1.3.18.1 使用 if let 处理 Option](#13181-使用-if-let-处理-option)
      - [1.3.18.2 使用 match 处理 Result](#13182-使用-match-处理-result)
    - [1.3.19 单元测试和模糊测试](#1319-单元测试和模糊测试)
      - [1.3.19.1 单元测试](#13191-单元测试)
      - [1.3.19.2 模糊测试使用 libfuzzer-sys](#13192-模糊测试使用-libfuzzer-sys)
    - [1.3.20 使用封装和抽象](#1320-使用封装和抽象)
      - [1.3.20.1 封装文件读取逻辑](#13201-封装文件读取逻辑)
    - [1.3.21 使用防御性编程](#1321-使用防御性编程)
      - [1.3.21.1 防御性检查示例](#13211-防御性检查示例)
    - [1.3.22 使用智能指针](#1322-使用智能指针)
      - [1.3.22.1 使用 RefCell](#13221-使用-refcell)
    - [1.3.23 审查第三方依赖](#1323-审查第三方依赖)
      - [1.3.23.1 审查依赖的 Cargotoml](#13231-审查依赖的-cargotoml)
    - [1.3.24 -总结](#1324--总结)
    - [1.3.25 Rust 错误处理惯用法](#1325-rust-错误处理惯用法)
    - [1.3.26 unwrap\_or 的解释和示例](#1326-unwrap_or-的解释和示例)
      - [1.3.26.1 解释](#13261-解释)
      - [1.3.26.2 -示例](#13262--示例)
    - [1.3.27 unwrap\_or\_else 的解释和示例](#1327-unwrap_or_else-的解释和示例)
...
### 1.3.11 线程取消Thread Cancellation

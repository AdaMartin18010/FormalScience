...
- [1. Rust语言语法语义特性全面解析](#rust语言语法语义特性全面解析)
  - [1.1 基础语法与表达式](#1-基础语法与表达式)
    - [1.1.1 变量绑定与声明](#11-变量绑定与声明)
      - [1.1.1.1 let绑定与可变性（mut）](#111-let绑定与可变性mut)
      - [1.1.1.2 常量（const）与静态变量（static）](#112-常量const与静态变量static)
      - [1.1.1.3 变量遮蔽（shadowing）](#113-变量遮蔽shadowing)
      - [1.1.1.4 解构模式（destructuring）](#114-解构模式destructuring)
    - [1.1.2 基本数据类型](#12-基本数据类型)
      - [1.1.2.1 整数类型（i8/u8 至 i128/u128, isize/usize）](#121-整数类型i8u8-至-i128u128-isizeusize)
      - [1.1.2.2 浮点类型（f32, f64）](#122-浮点类型f32-f64)
      - [1.1.2.3 布尔类型（bool）](#123-布尔类型bool)
      - [1.1.2.4 字符类型（char）](#124-字符类型char)
      - [1.1.2.5 单元类型（unit type: ()）](#125-单元类型unit-type)
    - [1.1.3 复合数据类型](#13-复合数据类型)
      - [1.1.3.1 元组（tuple）](#131-元组tuple)
      - [1.1.3.2 数组（array）与切片（slice）](#132-数组array与切片slice)
      - [1.1.3.3 字符串（String 与 &str）](#133-字符串string-与-str)
      - [1.1.3.4 向量（`Vec<T>`）](#134-向量vect)
    - [1.1.4 函数与闭包](#14-函数与闭包)
      - [1.1.4.1 函数声明与调用](#141-函数声明与调用)
      - [1.1.4.2 匿名函数与闭包](#142-匿名函数与闭包)
      - [1.1.4.3 高阶函数](#143-高阶函数)
      - [1.1.4.4 发散函数（never type: !）](#144-发散函数never-type)
    - [1.1.5 控制流结构](#15-控制流结构)
      - [1.1.5.1 条件表达式（if/else）](#151-条件表达式ifelse)
      - [1.1.5.2 循环结构（loop, while, for）](#152-循环结构loop-while-for)
      - [1.1.5.3 提前返回（return）与循环控制（break, continue）](#153-提前返回return与循环控制break-continue)
      - [1.1.5.4 模式匹配（match）](#154-模式匹配match)
      - [1.1.5.5 if let 与 while let 语法](#155-if-let-与-while-let-语法)
      - [1.1.5.6 问号操作符（?）的错误传播](#156-问号操作符的错误传播)
  - [1.2 类型系统与抽象](#2-类型系统与抽象)
    - [1.2.1 自定义数据类型](#21-自定义数据类型)
      - [1.2.1.1 结构体（struct）](#211-结构体struct)
      - [1.2.1.2 枚举（enum）](#212-枚举enum)
      - [1.2.1.3 联合体（union）](#213-联合体union)
      - [1.2.1.4 类型别名（type）](#214-类型别名type)
      - [1.2.1.5 新类型模式（newtype pattern）](#215-新类型模式newtype-pattern)
    - [1.2.2 泛型与多态](#22-泛型与多态)
      - [1.2.2.1 泛型参数](#221-泛型参数)
      - [1.2.2.2 泛型函数与方法](#222-泛型函数与方法)
      - [1.2.2.3 泛型结构体与枚举](#223-泛型结构体与枚举)
      - [1.2.2.4 泛型约束](#224-泛型约束)
      - [1.2.2.5 零大小类型（ZST）](#225-零大小类型zst)
    - [1.2.3 特征系统](#23-特征系统)
      - [1.2.3.1 特征（trait）定义与实现](#231-特征trait定义与实现)
      - [1.2.3.2 特征作为参数](#232-特征作为参数)
      - [1.2.3.3 特征对象与动态分发](#233-特征对象与动态分发)
      - [1.2.3.4 特征继承（supertraits）](#234-特征继承supertraits)
      - [1.2.3.5 关联类型与关联常量](#235-关联类型与关联常量)
      - [1.2.3.6 默认实现与特征方法覆盖](#236-默认实现与特征方法覆盖)
      - [1.2.3.7 孤儿规则（orphan rule）](#237-孤儿规则orphan-rule)
    - [1.2.4 类型转换](#24-类型转换)
      - [1.2.4.1 强制类型转换（coercion）](#241-强制类型转换coercion)
      - [1.2.4.2 as运算符](#242-as运算符)
      - [1.2.4.3 From/Into特征](#243-frominto特征)
      - [1.2.4.4 TryFrom/TryInto特征](#244-tryfromtryinto特征)
      - [1.2.4.5 Deref强制转换](#245-deref强制转换)
    - [1.2.5 高级类型系统特性](#25-高级类型系统特性)
      - [1.2.5.1 高级特征约束](#251-高级特征约束)
      - [1.2.5.2 类型变型（variance）](#252-类型变型variance)
      - [1.2.5.3 存在类型（impl Trait）](#253-存在类型impl-trait)
      - [1.2.5.4 高级类型别名](#254-高级类型别名)
      - [1.2.5.5 高级泛型约束（where子句）](#255-高级泛型约束where子句)
      - [1.2.5.6 泛型关联类型（GAT）](#256-泛型关联类型gat)
      - [1.2.5.7 特征别名（trait aliases）](#257-特征别名trait-aliases)
  - [1.3 所有权系统与内存管理](#3-所有权系统与内存管理)
    - [1.3.1 所有权基本原则](#31-所有权基本原则)
      - [1.3.1.1 所有权规则](#311-所有权规则)
      - [1.3.1.2 移动语义（move semantics）](#312-移动语义move-semantics)
      - [1.3.1.3 复制语义（copy semantics）](#313-复制语义copy-semantics)
      - [1.3.1.4 所有权转移的时机与影响](#314-所有权转移的时机与影响)
    - [1.3.2 借用系统](#32-借用系统)
      - [1.3.2.1 不可变借用（&T）](#321-不可变借用t)
      - [1.3.2.2 可变借用（&mut T）](#322-可变借用mut-t)
      - [1.3.2.3 借用规则与借用检查器](#323-借用规则与借用检查器)
      - [1.3.2.4 多重借用与借用冲突](#324-多重借用与借用冲突)
      - [1.3.2.5 自引用结构的挑战](#325-自引用结构的挑战)
    - [1.3.3 生命周期](#33-生命周期)
      - [1.3.3.1 生命周期标注（'a）](#331-生命周期标注a)
      - [1.3.3.2 函数中的生命周期](#332-函数中的生命周期)
      - [1.3.3.3 结构体与枚举中的生命周期](#333-结构体与枚举中的生命周期)
      - [1.3.3.4 生命周期省略规则](#334-生命周期省略规则)
      - [1.3.3.5 'static 生命周期](#335-static-生命周期)
      - [1.3.3.6 生命周期边界与约束](#336-生命周期边界与约束)
      - [1.3.3.7 非词法生命周期（NLL）](#337-非词法生命周期nll)
    - [1.3.4 内存管理模式](#34-内存管理模式)
      - [1.3.4.1 RAII模式](#341-raii模式)
      - [1.3.4.2 Drop特征与资源释放](#342-drop特征与资源释放)
      - [1.3.4.3 智能指针模式](#343-智能指针模式)
      - [1.3.4.4 内存布局和对齐](#344-内存布局和对齐)
      - [1.3.4.5 内存泄漏与防范](#345-内存泄漏与防范)
  - [1.4 错误处理](#4-错误处理)
    - [1.4.1 错误处理策略](#41-错误处理策略)
      - [1.4.1.1 可恢复错误与Result](#411-可恢复错误与result)
      - [1.4.1.2 不可恢复错误与panic](#412-不可恢复错误与panic)
      - [1.4.1.3 Option与空值处理](#413-option与空值处理)
      - [1.4.1.4 自定义错误类型](#414-自定义错误类型)
    - [1.4.2 高级错误处理模式](#42-高级错误处理模式)
      - [1.4.2.1 错误上下文和故障传播](#421-错误上下文和故障传播)
      - [1.4.2.2 错误边界与恢复策略](#422-错误边界与恢复策略)
      - [1.4.2.3 错误日志与监控](#423-错误日志与监控)
  - [1.5 模块与包管理](#5-模块与包管理)
    - [1.5.1 模块系统](#51-模块系统)
      - [1.5.1.1 模块基础](#511-模块基础)
      - [1.5.1.2 可见性规则](#512-可见性规则)
      - [1.5.1.3 模块组织与文件系统](#513-模块组织与文件系统)
      - [1.5.1.4 路径引用和相对路径](#514-路径引用和相对路径)
    - [1.5.2 包与Crate系统](#52-包与crate系统)
      - [1.5.2.1 包与Crate基础](#521-包与crate基础)
      - [1.5.2.2 Cargo包管理器](#522-cargo包管理器)
- [2. 基本依赖指定](#基本依赖指定)
- [3. 创建新项目](#创建新项目)
- [4. 构建项目](#构建项目)
- [5. 运行项目](#运行项目)
...
- [9. 发布](#发布)
- [10. 工作空间](#工作空间)
      - [10.0.0.1 发布与使用crate](#001-发布与使用crate)
- [11. 登录 crates.io](#登录-cratesio)
- [12. 检查包](#检查包)
- [13. 发布包](#发布包)
- [14. 更新版本后再次发布](#更新版本后再次发布)
- [15. 修改 Cargo.toml 中的版本号](#修改-cargotoml-中的版本号)
- [16. cargo publish](#cargo-publish)
- [17. 精确版本](#精确版本)
- [18. 兼容版本（接受1.2.3到1.3.0之前的任何版本）](#兼容版本接受123到130之前的任何版本)
- [19. 主版本兼容（接受1.2.3到2.0.0之前的任何版本）](#主版本兼容接受123到200之前的任何版本)
- [20. 范围版本](#范围版本)
- [21. 通配符版本](#通配符版本)
- [22. 最新版本](#最新版本)
  - [22.1 并发](#1-并发)
    - [22.1.1 线程与并发基础](#11-线程与并发基础)
      - [22.1.1.1 线程创建与管理](#111-线程创建与管理)
      - [22.1.1.2 线程间通信](#112-线程间通信)
      - [22.1.1.3 线程同步原语](#113-线程同步原语)
    - [22.1.2 Rayon并行迭代器](#12-rayon并行迭代器)
    - [22.1.3 异步编程](#13-异步编程)
      - [22.1.3.1 Future与异步函数](#131-future与异步函数)
      - [22.1.3.2 异步运行时](#132-异步运行时)
  - [22.2 并发（续）](#2-并发续)
      - [22.2.0.1 异步运行时（续）](#201-异步运行时续)
      - [22.2.0.2 Async/Await模式](#202-asyncawait模式)
    - [22.2.1 并发设计模式](#21-并发设计模式)
      - [22.2.1.1 Actor模型](#211-actor模型)
      - [22.2.1.2 工作池与任务分发](#212-工作池与任务分发)
      - [22.2.1.3 并发组合模式](#213-并发组合模式)
  - [22.3 元编程](#3-元编程)
    - [22.3.1 宏系统](#31-宏系统)
      - [22.3.1.1 声明宏](#311-声明宏)
      - [22.3.1.2 过程宏](#312-过程宏)
      - [22.3.1.3 常见宏模式与技巧](#313-常见宏模式与技巧)
    - [22.3.2 编译时反射](#32-编译时反射)
      - [22.3.2.1 编译时类型信息](#321-编译时类型信息)
      - [22.3.2.2 编译时代码生成](#322-编译时代码生成)
      - [22.3.2.3 类型级编程](#323-类型级编程)
    - [22.3.3 构建时配置](#33-构建时配置)
      - [22.3.3.1 条件编译](#331-条件编译)
      - [22.3.3.2 自定义构建脚本](#332-自定义构建脚本)
  - [22.4 高级特性](#4-高级特性)
    - [22.4.1 Unsafe Rust](#41-unsafe-rust)
      - [22.4.1.1 Unsafe基础](#411-unsafe基础)
      - [22.4.1.2 内存管理与原始指针](#412-内存管理与原始指针)
      - [22.4.1.3 安全抽象构建](#413-安全抽象构建)
    - [22.4.2 高级特征](#42-高级特征)
      - [22.4.2.1 关联类型与类型族](#421-关联类型与类型族)
      - [22.4.2.2 高级特征边界](#422-高级特征边界)
      - [22.4.2.3 GAT与复杂泛型](#423-gat与复杂泛型)
      - [22.4.2.4 特征对象与动态分发](#424-特征对象与动态分发)
      - [22.4.2.5 零成本抽象](#425-零成本抽象)
    - [22.4.3 高级类型系统特性](#43-高级类型系统特性)
      - [22.4.3.1 异质集合与 `Any` 类型](#431-异质集合与-any-类型)
      - [22.4.3.2 幽灵类型与类型状态](#432-幽灵类型与类型状态)
      - [22.4.3.3 类型系统的高级模式](#433-类型系统的高级模式)
    - [22.4.4 FFI与外部代码集成](#44-ffi与外部代码集成)
      - [22.4.4.1 C语言互操作](#441-c语言互操作)
      - [22.4.4.2 内存管理与类型转换](#442-内存管理与类型转换)
  - [22.5 语言哲学与设计原则](#5-语言哲学与设计原则)
    - [22.5.1 Rust的设计哲学](#51-rust的设计哲学)
      - [22.5.1.1 安全、并发、控制](#511-安全并发控制)
      - [22.5.1.2 表达性与工程性](#512-表达性与工程性)
      - [22.5.1.3 权衡与取舍](#513-权衡与取舍)
    - [22.5.2 类型安全与表达能力](#52-类型安全与表达能力)
      - [22.5.2.1 类型驱动开发](#521-类型驱动开发)
      - [22.5.2.2 表达式语言](#522-表达式语言)
      - [22.5.2.3 类型推导与模式匹配](#523-类型推导与模式匹配)
    - [22.5.3 生态系统与社区](#53-生态系统与社区)
      - [22.5.3.1 包管理与生态系统健壮性](#531-包管理与生态系统健壮性)
- [23. 基本依赖](#基本依赖)
- [24. 条件依赖](#条件依赖)
- [25. 开发依赖](#开发依赖)
- [26. 特性配置](#特性配置)
- [27. 平台特定依赖](#平台特定依赖)
- [28. 构建脚本依赖](#构建脚本依赖)
      - [28.0.0.1 线程安全中的内存模型和不变条件](#001-线程安全中的内存模型和不变条件)
      - [28.0.0.2 Rust的未来发展与演进方向](#002-rust的未来发展与演进方向)
    - [28.0.1 总结](#01-总结)
## 1.1 基础语法与表达式
...
#### 1.1.2.1 整数类型（i8/u8 至 i128/u128, isize/usize）
...
#### 1.1.5.3 提前返回（return）与循环控制（break, continue）
...
# 18. 兼容版本（接受1.2.3到1.3.0之前的任何版本）
compatible = "~1.2.3"

# 19. 主版本兼容（接受1.2.3到2.0.0之前的任何版本）

...
- [1. Rust中的PhantomData](#1-rust中的phantomdata)
  - [目录](#目录)
  - [1.1 使用`PhantomData`控制类型变型的技术](#11-使用phantomdata控制类型变型的技术)
  - [1.2 为什么需要使用`PhantomData`](#12-为什么需要使用phantomdata)
  - [1.3 不使用`PhantomData`可能产生的问题](#13-不使用phantomdata可能产生的问题)
    - [1.3.1 反例 1：类型不安全](#131-反例-1类型不安全)
    - [1.3.2 反例 2：生命周期问题](#132-反例-2生命周期问题)
    - [1.3.3 反例 3：编译器警告](#133-反例-3编译器警告)
    - [1.3.4 反例 4：不正确的类型推导](#134-反例-4不正确的类型推导)
    - [1.3.5 总结](#135-总结)

## 1.1 使用`PhantomData`控制类型变型的技术

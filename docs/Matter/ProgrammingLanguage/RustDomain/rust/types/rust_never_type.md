...
- [1 定义](#1-定义)
- [2 使用场景](#2-使用场景)
  - [2.1 无限循环：当一个函数需要无限循环时，可以使用 类型作为返回值](#21-无限循环当一个函数需要无限循环时可以使用-类型作为返回值)
  - [2.2 错误处理：在某些情况下，函数可能会遇到错误情况并立即退出，使用 可以表示这种情况](#22-错误处理在某些情况下函数可能会遇到错误情况并立即退出使用-可以表示这种情况)
  - [2.3 异步编程：在异步编程中， 类型用于表示一个 Future 可能永远不会完成](#23-异步编程在异步编程中-类型用于表示一个-future-可能永远不会完成)
- [3 特点](#3-特点)
- [4 负特征 (Negative Trait Bounds)](#4-负特征-negative-trait-bounds)
...
### 2.1 无限循环：当一个函数需要无限循环时，可以使用 类型作为返回值
...
### 2.2 错误处理：在某些情况下，函数可能会遇到错误情况并立即退出，使用 可以表示这种情况
...
### 2.3 异步编程：在异步编程中， 类型用于表示一个 Future 可能永远不会完成

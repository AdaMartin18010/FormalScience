...
- [分布式共识、一致性与 Raft 协议：从理论到 Rust 实现](#分布式共识一致性与-raft-协议从理论到-rust-实现)
  ...
  - [10 深入探讨 (续): `raft-rs` 驱动循环与外部交互的细节](#10-深入探讨-续-raft-rs-驱动循环与外部交互的细节)
  ...
  - [11 深入探讨 (续): `Storage` 持久化实现要点与客户端交互](#11-深入探讨-续-storage-持久化实现要点与客户端交互)
    - [1 部分一：`Storage` Trait 的持久化实现要点](#1-部分一storage-trait-的持久化实现要点)
    - [2. `Storage` 层面的处理细节](#2-storage-层面的处理细节)
      - [1 持久化实现的要点：](#1-持久化实现的要点)
      - [2.2. 快照中的 `ConfState`](#22-快照中的-confstate)
      - [2.3. 日志截断与 `ConfState`](#23-日志截断与-confstate)
    - [11.2 部分二：客户端如何与 Raft 集群交互](#112-部分二客户端如何与-raft-集群交互)
  ...
  - [12 深入探讨 (续): Raft 集群成员变更 (`ConfChangeV2`) 的处理细节](#12-深入探讨-续-raft-集群成员变更-confchangev2-的处理细节)

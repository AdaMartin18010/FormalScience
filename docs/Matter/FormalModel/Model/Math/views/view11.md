...
- [1. 数理逻辑：形式模型、理论与关联性分析](#1-数理逻辑形式模型理论与关联性分析)
  - [目录](#目录)
  - [1.1 基础概念：形式语言与结构](#11-基础概念形式语言与结构)
  - [1.2 主要分支与核心模型](#12-主要分支与核心模型)
    - [1.2.1 模型论 (Model Theory)](#121-模型论-model-theory)
    - [1.2.2 递归论 (Recursion Theory / Computability Theory)](#122-递归论-recursion-theory--computability-theory)
    - [1.2.3 证明论 (Proof Theory)](#123-证明论-proof-theory)
  - [1.3 层次、关联性与元模型分析](#13-层次关联性与元模型分析)
    - [1.3.1 层次划分](#131-层次划分)
    - [1.3.2 关联性分析](#132-关联性分析)
    - [1.3.3 元模型与视角切换](#133-元模型与视角切换)
  - [1.4 总结](#14-总结)
  - [1.5 思维导图 (Text)](#15-思维导图-text)
  - [1.6 深入探讨：关键定理的深层意涵与理论拓展](#16-深入探讨关键定理的深层意涵与理论拓展)
    - [1.6.1 模型论的拓展与哲学反思](#161-模型论的拓展与哲学反思)
    - [1.6.2 递归论的边界与计算复杂性](#162-递归论的边界与计算复杂性)
    - [1.6.3 证明论的结构与建设性数学](#163-证明论的结构与建设性数学)
  - [1.7 数理逻辑的哲学维度与未来展望](#17-数理逻辑的哲学维度与未来展望)
  - [1.8 理论间的交织：协同与张力](#18-理论间的交织协同与张力)
    - [1.8.1 递归论对模型论和证明论的渗透](#181-递归论对模型论和证明论的渗透)
    - [1.8.2 模型论方法在证明论和递归论中的应用](#182-模型论方法在证明论和递归论中的应用)
    - [1.8.3 证明论、类型论与计算机科学](#183-证明论类型论与计算机科学)
  - [1.9 数理逻辑在知识图谱中的位置与应用拓展](#19-数理逻辑在知识图谱中的位置与应用拓展)
  - [1.10 重新切换视角：超越辩证法，走向多维整合](#110-重新切换视角超越辩证法走向多维整合)
  - [1.11 思维导图 (Text) - 扩展部分](#111-思维导图-text---扩展部分)
...
- **3.1.2 关键定理**
  - **紧致性定理 (Compactness Theorem)**: 一个理论 `T` 有模型，当且仅当它的所有有限子集都有模型。
    - _论证_: 这个定理极其强大，它说明了有限性质可以决定无限理论的可满足性。例如，它可以用来证明存在非标准算术模型。
  - **勒文海姆-斯科伦定理 (Löwenheim–Skolem Theorem)**: 如果一个可数的一阶理论 `T` 有一个无限模型，那么它对任意无限基数 `κ` 都有一个基数为 `κ` 的模型。
    - _论证_: 这揭示了一阶逻辑的局限性：它无法完全刻画无限结构（如自然数或实数）的基数。例如，存在实数理论的可数模型。
...
### 1.2.2 递归论 (Recursion Theory / Computability Theory)
...
- **3.2.1 定义与核心模型**
  ...
  - **Lambda 演算 (Lambda Calculus)**: 一种基于函数抽象和应用的计算模型。
    - _论证_: 这些看似不同的模型（图灵机、递归函数、Lambda演算等）已被证明在计算能力上是等价的，这支持了**丘奇-图灵论题**。
...
- **3.3.3 关键概念与定理**
  ...
  - **切消定理 (Cut-Elimination Theorem - Gentzen)**: 在相继式演算中，任何有“切规则”（Cut Rule，类似于引理的使用）的证明都可以被转化为没有切规则的证明。
    - _论证_: 这说明证明可以被规范化，避免了“绕弯路”，并且是证明算术一致性的关键步骤。

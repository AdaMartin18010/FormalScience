...
- [数学逻辑](#数学逻辑)
  ...
  - [2. 模型论 (Model Theory)](#2-模型论-model-theory)
    - [1 核心概念：形式语言与结构](#1-核心概念形式语言与结构)
    - [2.2 定义：模型](#22-定义模型)
    - [2.3 解释与示例](#23-解释与示例)
    - [2.4 元模型与论证：可靠性、完备性、紧致性](#24-元模型与论证可靠性完备性紧致性)
    - [2.5 拓展与关联](#25-拓展与关联)
  ...
  - [2 . 关键证明技巧与哲学意涵](#2-关键证明技巧与哲学意涵)
    ...
    - [2.2 哲学意涵](#22-哲学意涵)
  - [3 . 现代逻辑分支](#3-现代逻辑分支)
    - [1 模态逻辑 (Modal Logic)](#1-模态逻辑-modal-logic)
    - [3.2 非经典逻辑 (Non-Classical Logics)](#32-非经典逻辑-non-classical-logics)
    - [3.3 范畴逻辑 (Categorical Logic) / 范畴论与逻辑 (Category Theory and Logic)](#33-范畴逻辑-categorical-logic-范畴论与逻辑-category-theory-and-logic)
  ...
  - [1 . 非经典逻辑：缺省逻辑与回答集编程 (ASP)](#1-非经典逻辑缺省逻辑与回答集编程-asp)
    - [1 缺省逻辑 (Default Logic)](#1-缺省逻辑-default-logic)
    - [1.2 回答集编程 (Answer Set Programming, ASP)](#12-回答集编程-answer-set-programming-asp)
  - [2 . 自动规划算法：SATPlan 的工作原理](#2-自动规划算法satplan-的工作原理)
  - [3 . 逻辑与自然语言处理 (NLP) 及机器学习 (ML) 的交叉](#3-逻辑与自然语言处理-nlp-及机器学习-ml-的交叉)
    - [1 逻辑与自然语言处理 (NLP)](#1-逻辑与自然语言处理-nlp)
    - [3.2 逻辑与机器学习 (ML)](#32-逻辑与机器学习-ml)
  ...
  - [2 . AI 安全：价值对齐的其他方法——AI 辩论 (Debate) 与迭代放大 (Iterated Amplification)](#2-ai-安全价值对齐的其他方法ai-辩论-debate-与迭代放大-iterated-amplification)
    - [1 AI 辩论 (AI Debate)](#1-ai-辩论-ai-debate)
    - [2.2 迭代放大 (Iterated Amplification) / 迭代蒸馏与放大 (Iterated Distillation and Amplification, IDA)](#22-迭代放大-iterated-amplification-迭代蒸馏与放大-iterated-distillation-and-amplification-ida)
  ...
  - [2 . AI 安全：对齐税 (Alignment Tax) 与规范可学习性 (Specification Learnability)](#2-ai-安全对齐税-alignment-tax-与规范可学习性-specification-learnability)
    - [1 对齐税 (Alignment Tax)](#1-对齐税-alignment-tax)
    - [2.2 规范可学习性 (Specification Learnability)](#22-规范可学习性-specification-learnability)
  - [3 . 量子信息论与量子纠错：保护脆弱的量子态](#3-量子信息论与量子纠错保护脆弱的量子态)
  - [1 . 量子算法示例：Deutsch-Jozsa 与 Simon 算法](#1-量子算法示例deutsch-jozsa-与-simon-算法)
    - [1 Deutsch-Jozsa 算法 (1992)](#1-deutsch-jozsa-算法-1992)
    - [1.2 Simon 算法 (1994)](#12-simon-算法-1994)
  ...
  - [1 . 量子纠错：表面码 (Surface Code) 详解](#1-量子纠错表面码-surface-code-详解)
    - [1 基本结构与概念](#1-基本结构与概念)
    - [1.2 错误检测与纠正](#12-错误检测与纠正)
    - [1.3 容错 (Fault Tolerance)](#13-容错-fault-tolerance)
  ...
  - [1 . 量子纠错：其他编码方案与容错门](#1-量子纠错其他编码方案与容错门)
    - [1 其他量子纠错码 (QEC Codes)](#1-其他量子纠错码-qec-codes)
    - [1.2 容错逻辑门 (Fault-Tolerant Logical Gates)](#12-容错逻辑门-fault-tolerant-logical-gates)
  - [2 . AI 对齐：内在对齐 vs. 外在对齐](#2-ai-对齐内在对齐-vs-外在对齐)
  - [3 . 组合拍卖：获胜者确定问题与算法](#3-组合拍卖获胜者确定问题与算法)
    - [1 获胜者确定问题 (Winner Determination Problem, WDP)](#1-获胜者确定问题-winner-determination-problem-wdp)
    - [3.2 解决 WDP 的算法方法](#32-解决-wdp-的算法方法)
  - [1 . 量子 LDPC 码：概览与示例](#1-量子-ldpc-码概览与示例)
    - [1 表示方法：Tanner 图 (Tanner Graph)](#1-表示方法tanner-图-tanner-graph)
    - [1.2 CSS 构造 (Calderbank-Shor-Steane Construction)](#12-css-构造-calderbank-shor-steane-construction)
    - [1.3 QLDPC 码的参数与性能](#13-qldpc-码的参数与性能)
    - [1.4 示例与研究方向](#14-示例与研究方向)
    - [1.5 解码 (Decoding)](#15-解码-decoding)
  ...
  - [1 . 计算热力学：量子点信息引擎模型](#1-计算热力学量子点信息引擎模型)
    - [1 单电子量子点信息引擎](#1-单电子量子点信息引擎)
    - [1.2 简化工作循环 (类比西拉德引擎)](#12-简化工作循环-类比西拉德引擎)
    - [1.3 量子效应的潜在作用](#13-量子效应的潜在作用)
    - [1.4 实际挑战与意义](#14-实际挑战与意义)
  - [2 . 计算认知科学：差分可编程神经计算机 (Differentiable Neural Computer, DNC)](#2-计算认知科学差分可编程神经计算机-differentiable-neural-computer-dnc)
    - [1 核心组件与机制](#1-核心组件与机制)
    - [2.2 能力与应用](#22-能力与应用)
    - [2.3 优势与局限性](#23-优势与局限性)
  - [3 . 算法公平性：COMPAS 争议与因果视角下的再犯风险评估](#3-算法公平性compas-争议与因果视角下的再犯风险评估)
    - [1 ProPublica 的调查与核心争议](#1-propublica-的调查与核心争议)
    - [3.2 公平性定义的冲突](#32-公平性定义的冲突)
    - [3.3 因果视角下的分析](#33-因果视角下的分析)
    - [3.4 结论与启示](#34-结论与启示)
  - [4 DNC 与 Transformer 在记忆机制上的对比](#4-dnc-与-transformer-在记忆机制上的对比)
    - [1 Differentiable Neural Computer (DNC) 的记忆机制](#1-differentiable-neural-computer-dnc-的记忆机制)
    - [4.2 Transformer 的记忆机制 (主要通过自注意力 Self-Attention)](#42-transformer-的记忆机制-主要通过自注意力-self-attention)
    - [4.3 对比总结](#43-对比总结)
  - [5 COMPAS 案例中具体的因果图构建尝试](#5-compas-案例中具体的因果图构建尝试)
    - [1 . 确定关键变量 (Nodes in the DAG)](#1-确定关键变量-nodes-in-the-dag)
    - [2 . 假设因果关系 (Edges in the DAG)](#2-假设因果关系-edges-in-the-dag)
    - [5.3 示例性简化 DAG (部分路径)](#53-示例性简化-dag-部分路径)
    - [5.4 使用因果图进行公平性分析](#54-使用因果图进行公平性分析)
    - [5.5 挑战](#55-挑战)
  - [6 Transformer-XL 的记忆机制：片段级循环与相对位置编码](#6-transformer-xl-的记忆机制片段级循环与相对位置编码)
    ...
    - [6.3 Transformer-XL 记忆机制总结](#63-transformer-xl-记忆机制总结)
  - [7 COMPAS 案例中关于标签偏见的具体研究](#7-compas-案例中关于标签偏见的具体研究)
    - [1 关键研究问题与发现](#1-关键研究问题与发现)
    - [7.2 结论与影响](#72-结论与影响)

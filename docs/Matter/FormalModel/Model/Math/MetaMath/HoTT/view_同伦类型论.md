...
- [同伦类型论 (Homotopy Type Theory, HoTT)](#同伦类型论-homotopy-type-theory-hott)
  ...
  - [A. 核心概念与定义](#a-核心概念与定义)
    - [1 A.1. 什么是同伦类型论 (What is Homotopy Type Theory)?](#1-a1-什么是同伦类型论-what-is-homotopy-type-theory)
    - [1.2 A.2. 核心思想：命题即类型，证明即程序，类型即空间](#12-a2-核心思想命题即类型证明即程序类型即空间)
    - [1.3 A.3. 基本构造块 (Basic Building Blocks)](#13-a3-基本构造块-basic-building-blocks)
      - [1 A.3.1. 类型 (Types)](#1-a31-类型-types)
      - [2 A.3.2. 项 (Terms / Elements / Inhabitants)](#2-a32-项-terms-elements-inhabitants)
      - [3 A.3.3. 依赖类型 (Dependent Types)](#3-a33-依赖类型-dependent-types)
      - [4 A.3.4. Π-类型 (Pi-types / Dependent Function Types)](#4-a34-π-类型-pi-types-dependent-function-types)
      - [5 A.3.5. Σ-类型 (Sigma-types / Dependent Pair Types)](#5-a35-σ-类型-sigma-types-dependent-pair-types)
      - [6 A.3.6. 等价类型 (Identity Types, `Id_A(a,b)` or `a =_A b`)](#6-a36-等价类型-identity-types-id_aab-or-a-_a-b)
      - [7 A.3.7. 宇宙 (Universes, `U`)](#7-a37-宇宙-universes-u)
    - [1.4 A.4. 同伦的视角 (The Homotopy Perspective)](#14-a4-同伦的视角-the-homotopy-perspective)
      - [1 A.4.1. 类型的同伦层级 (Homotopy Levels of Types / n-types)](#1-a41-类型的同伦层级-homotopy-levels-of-types-n-types)
        - [1 A.4.1.1. (-2)-类型：可收缩类型 (Contractible Types / Singletons)](#1-a411--2-类型可收缩类型-contractible-types-singletons)
        - [2 A.4.1.2. (-1)-类型：命题 (Propositions / h-Propositions / Mere Propositions)](#2-a412--1-类型命题-propositions-h-propositions-mere-propositions)
        - [3 A.4.1.3. 0-类型：集合 (Sets / h-Sets)](#3-a413-0-类型集合-sets-h-sets)
        - [4 A.4.1.4. 1-类型：广群 (Groupoids / h-Groupoids)](#4-a414-1-类型广群-groupoids-h-groupoids)
        - [5 A.4.1.5. n-类型 (n-Groupoids)](#5-a415-n-类型-n-groupoids)
      - [2 A.4.2. 路径与等价 (Paths and Identity)](#2-a42-路径与等价-paths-and-identity)
      - [3 A.4.3. 函数外延性 (Function Extensionality)](#3-a43-函数外延性-function-extensionality)
    - [1.5 A.5. 单价公理 (Univalence Axiom)](#15-a5-单价公理-univalence-axiom)
      - [1 A.5.1. 等价即相等 (Equivalence is Equality)](#1-a51-等价即相等-equivalence-is-equality)
      - [2 A.5.2. 对数学结构的影响 (Impact on Mathematical Structures)](#2-a52-对数学结构的影响-impact-on-mathematical-structures)
    - [1.6 A.6. 高阶归纳类型 (Higher Inductive Types, HITs)](#16-a6-高阶归纳类型-higher-inductive-types-hits)
      - [1 A.6.1. 定义与动机 (Definition and Motivation)](#1-a61-定义与动机-definition-and-motivation)
      - [2 A.6.2. 例子](#2-a62-例子)
    - [1.7 A.7. 与构造性数学的关系 (Relation to Constructive Mathematics)](#17-a7-与构造性数学的关系-relation-to-constructive-mathematics)
  ...
  - [5 E. 与其他数学及计算机科学分支的联系](#5-e-与其他数学及计算机科学分支的联系)
  ...
  - [6 F. 在形式化证明与程序设计中的应用](#6-f-在形式化证明与程序设计中的应用)

...
- [1. workflow工作流的分析和综合](#1-workflow工作流的分析和综合)
  - [目录](#目录)
  - [1.1 分析和综合](#11-分析和综合)
    - [1.1.1 复杂性的必然性与转移](#111-复杂性的必然性与转移)
      ...
      - [1.1.1.2 三个层面的重要性与互联](#1112-三个层面的重要性与互联)
        ...
        - [1.1.1.2.3 数据控制流层面 (DataControl Flow Layer) 运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息](#11123-数据控制流层面-datacontrol-flow-layer-运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息)
    - [1.1.2 综合方向架构的评价](#112-综合方向架构的评价)
  - [1.2 总结](#12-总结)
    - [1.2.1 补充运行层面的实践](#121-补充运行层面的实践)
...
##### 1.1.1.2.3 数据控制流层面 (DataControl Flow Layer) 运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息
...
手段：
    这通常是前两个层面的交叉点。
    设计层面定义了_预期_的流，运行层面通过观测工具（追踪、日志分析）
    来_验证和理解_实际的流。

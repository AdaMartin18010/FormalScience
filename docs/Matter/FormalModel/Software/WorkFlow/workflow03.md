...
- [1 分析和综合](#1-分析和综合)
  - [1.1 复杂性的必然性与转移](#11-复杂性的必然性与转移)
    ...
    - [1.1.2 三个层面的重要性与互联](#112-三个层面的重要性与互联)
      - [2.1.2.1 设计层面 (Design Layer) 控制-设计-验证-修正](#2121-设计层面-design-layer-控制-设计-验证-修正)
      - [2.1.2.2 运行层面 (Runtime Layer) 执行-反馈-debug-日志-metric-等](#2122-运行层面-runtime-layer-执行-反馈-debug-日志-metric-等)
      - [2.1.2.3 数据控制流层面 (DataControl Flow Layer) 运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息](#2123-数据控制流层面-datacontrol-flow-layer-运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息)
  - [1.2 综合方向架构的评价](#12-综合方向架构的评价)
- [2 总结](#2-总结)
...
##### 2.1.2.1 设计层面 (Design Layer) 控制-设计-验证-修正
...
##### 2.1.2.2 运行层面 (Runtime Layer) 执行-反馈-debug-日志-metric-等
...
##### 2.1.2.3 数据控制流层面 (DataControl Flow Layer) 运行时核心调度控制流信息--业务层执行流信息---分布式时间上的数据流信息

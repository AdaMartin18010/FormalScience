...
- [1. Temporal 分析](#1-temporal-分析)
  - [目录](#目录)
  - [1.1 引言](#11-引言)
  - [1.2 架构分析 (Architecture Analysis)](#12-架构分析-architecture-analysis)
    - [1.2.1 核心服务组件](#121-核心服务组件)
    - [1.2.2 核心交互流程](#122-核心交互流程)
    - [1.2.3 设计原则：可伸缩、可靠、持久](#123-设计原则可伸缩可靠持久)
    - [1.2.4 依赖与横切关注点](#124-依赖与横切关注点)
  - [1.3 代码结构分析 (Code Structure Analysis)](#13-代码结构分析-code-structure-analysis)
    - [1.3.1 Temporal Server (`temporalio/temporal`)](#131-temporal-server-temporaliotemporal)
    - [1.3.2 Go SDK (`temporalio/sdk-go`)](#132-go-sdk-temporaliosdk-go)
    - [1.3.3 主要设计模式](#133-主要设计模式)
  - [1.4 实现分析 (Implementation Analysis)](#14-实现分析-implementation-analysis)
    - [1.4.1 事件溯源与确定性重放](#141-事件溯源与确定性重放)
    - [1.4.2 任务队列与调度 (Matching \& Task Queues)](#142-任务队列与调度-matching--task-queues)
    - [1.4.3 持久化层抽象与实现](#143-持久化层抽象与实现)
    - [1.4.4 可靠定时器管理](#144-可靠定时器管理)
    - [1.4.5 活动(Activity)的重试与错误处理](#145-活动activity的重试与错误处理)
    - [1.4.6 查询(Query)的路由与一致性](#146-查询query的路由与一致性)
    - [1.4.7 分片、成员关系与故障转移 (Sharding \& Membership)](#147-分片成员关系与故障转移-sharding--membership)
  - [1.5 应用分析 (Application Analysis)](#15-应用分析-application-analysis)
    - [1.5.1 常见工作流设计模式](#151-常见工作流设计模式)
    - [1.5.2 活动(Activity)设计最佳实践](#152-活动activity设计最佳实践)
    - [1.5.3 工作流(Workflow)设计最佳实践](#153-工作流workflow设计最佳实践)
    - [1.5.4 集成模式与用例](#154-集成模式与用例)
    - [1.5.5 常见反模式](#155-常见反模式)
  - [1.6 总结](#16-总结)
...
## 1.2 架构分析 (Architecture Analysis)
...
## 1.3 代码结构分析 (Code Structure Analysis)

分析 Temporal Server 和 Go SDK 的代码库有助于理解其内部组织和设计理念。

### 1.3.1 Temporal Server (`temporalio/temporal`)
...
### 1.3.2 Go SDK (`temporalio/sdk-go`)
...
## 1.4 实现分析 (Implementation Analysis)
...
### 1.4.2 任务队列与调度 (Matching & Task Queues)
...
### 1.4.7 分片、成员关系与故障转移 (Sharding & Membership)

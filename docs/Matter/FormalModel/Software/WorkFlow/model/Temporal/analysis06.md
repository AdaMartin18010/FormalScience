...
- [1. Temporal 的代码实现层面](#1-temporal-的代码实现层面)
  - [目录](#目录)
  - [1.1 引言：深入代码实现](#11-引言深入代码实现)
  - [1.2 核心机制的代码实现 (Go 语言视角)](#12-核心机制的代码实现-go-语言视角)
    - [1.2.1 事件溯源与确定性重放引擎 (SDK `internal/workflow`)](#121-事件溯源与确定性重放引擎-sdk-internalworkflow)
      - [1.2.1.1 Workflow Task 处理循环](#1211-workflow-task-处理循环)
      - [1.2.1.2 重放状态机 (`workflowExecutionEventHandler`)](#1212-重放状态机-workflowexecutioneventhandler)
      - [1.2.1.3 命令与事件的转换](#1213-命令与事件的转换)
      - [1.2.1.4 确定性保障的实现 (`DetRand`, `Now`, `SideEffect` API)](#1214-确定性保障的实现-detrand-now-sideeffect-api)
    - [1.2.2 任务轮询与处理 (SDK `internal/worker`)](#122-任务轮询与处理-sdk-internalworker)
      - [1.2.2.1 长轮询实现 (`TaskPoller`)](#1221-长轮询实现-taskpoller)
      - [1.2.2.2 任务分发与并发控制 (`WorkflowWorker`, `ActivityWorker`)](#1222-任务分发与并发控制-workflowworker-activityworker)
      - [1.2.2.3 粘性队列处理 (`StickyWorkflowTaskPoller`)](#1223-粘性队列处理-stickyworkflowtaskpoller)
    - [1.2.3 任务匹配与调度 (Server `service/matching`)](#123-任务匹配与调度-server-servicematching)
      - [1.2.3.1 任务队列数据结构 (`TaskQueueManager`, `DBTaskQueue`)](#1231-任务队列数据结构-taskqueuemanager-dbtaskqueue)
      - [1.2.3.2 匹配引擎 (`matchingEngine`)](#1232-匹配引擎-matchingengine)
      - [1.2.3.3 同步/异步匹配模式](#1233-同步异步匹配模式)
    - [1.2.4 工作流状态管理 (Server `service/history`)](#124-工作流状态管理-server-servicehistory)
      - [1.2.4.1 历史分片 (`historyEngine`, `shardController`)](#1241-历史分片-historyengine-shardcontroller)
      - [1.2.4.2 工作流上下文 (`workflowContext`, `mutableState`)](#1242-工作流上下文-workflowcontext-mutablestate)
      - [1.2.4.3 事件持久化 (`historyEventNotifier`, `persistence.ExecutionManager`)](#1243-事件持久化-historyeventnotifier-persistenceexecutionmanager)
    - [1.2.5 可靠定时器实现 (Server `service/history`)](#125-可靠定时器实现-server-servicehistory)
      - [1.2.5.1 定时器队列处理器 (`timerQueueProcessorBase`)](#1251-定时器队列处理器-timerqueueprocessorbase)
      - [1.2.5.2 定时器任务的持久化与加载](#1252-定时器任务的持久化与加载)
    - [1.2.6 活动心跳与取消 (SDK `internal/activity` \& Server `service/history`)](#126-活动心跳与取消-sdk-internalactivity--server-servicehistory)
      - [1.2.6.1 SDK 心跳发送 (`RecordHeartbeat`)](#1261-sdk-心跳发送-recordheartbeat)
      - [1.2.6.2 Server 心跳处理与超时检测](#1262-server-心跳处理与超时检测)
      - [1.2.6.3 取消请求的传递 (`RequestCancelActivityTask`)](#1263-取消请求的传递-requestcancelactivitytask)
  - [1.3 代码组织与设计原则体现](#13-代码组织与设计原则体现)
    - [1.3.1 清晰的职责边界 (Service vs. Common vs. SDK)](#131-清晰的职责边界-service-vs-common-vs-sdk)
    - [1.3.2 接口与依赖注入](#132-接口与依赖注入)
    - [1.3.3 并发原语的应用 (Goroutines, Channels, Mutex)](#133-并发原语的应用-goroutines-channels-mutex)
    - [1.3.4 错误处理模式](#134-错误处理模式)
    - [1.3.5 可测试性设计](#135-可测试性设计)
  - [1.4 结论：健壮实现支撑强大功能](#14-结论健壮实现支撑强大功能)
...
## 1.2 核心机制的代码实现 (Go 语言视角)

### 1.2.1 事件溯源与确定性重放引擎 (SDK `internal/workflow`)

这是 Temporal SDK 的核心，负责在 Worker 端执行工作流代码并确保确定性。

#### 1.2.1.1 Workflow Task 处理循环
...
#### 1.2.1.2 重放状态机 (`workflowExecutionEventHandler`)
...
#### 1.2.1.4 确定性保障的实现 (`DetRand`, `Now`, `SideEffect` API)
...
### 1.2.2 任务轮询与处理 (SDK `internal/worker`)

负责从 Temporal Server 获取任务并分发给相应的执行器。

#### 1.2.2.1 长轮询实现 (`TaskPoller`)
...
#### 1.2.2.2 任务分发与并发控制 (`WorkflowWorker`, `ActivityWorker`)
...
#### 1.2.2.3 粘性队列处理 (`StickyWorkflowTaskPoller`)
...
### 1.2.3 任务匹配与调度 (Server `service/matching`)

负责高效地将任务放入队列并将它们交给等待的 Worker。

#### 1.2.3.1 任务队列数据结构 (`TaskQueueManager`, `DBTaskQueue`)
...
### 1.2.4 工作流状态管理 (Server `service/history`)

维护工作流执行的核心状态和事件日志。

#### 1.2.4.1 历史分片 (`historyEngine`, `shardController`)
...
#### 1.2.4.2 工作流上下文 (`workflowContext`, `mutableState`)
...
#### 1.2.4.3 事件持久化 (`historyEventNotifier`, `persistence.ExecutionManager`)
...
### 1.2.5 可靠定时器实现 (Server `service/history`)

#### 1.2.5.1 定时器队列处理器 (`timerQueueProcessorBase`)
...
### 1.2.6 活动心跳与取消 (SDK `internal/activity` & Server `service/history`)

#### 1.2.6.1 SDK 心跳发送 (`RecordHeartbeat`)
...
#### 1.2.6.3 取消请求的传递 (`RequestCancelActivityTask`)
...
### 1.3.1 清晰的职责边界 (Service vs. Common vs. SDK)
...
### 1.3.3 并发原语的应用 (Goroutines, Channels, Mutex)

# 1. 重新审视 Temporal 的形式化架构

## 目录

- [1. 重新审视 Temporal 的形式化架构](#1-重新审视-temporal-的形式化架构)
  - [目录](#目录)
  - [1.1 引言：视角切换](#11-引言视角切换)
  - [1.2 视角一：开发者体验 —— 被“隐藏”的复杂性](#12-视角一开发者体验--被隐藏的复杂性)
    - [1.2.1 “魔法”代码：顺序指令背后的异步与持久化](#121-魔法代码顺序指令背后的异步与持久化)
    - [1.2.2 心智模型：确定性、幂等性与状态边界](#122-心智模型确定性幂等性与状态边界)
    - [1.2.3 调试与测试：可见性与本地优先](#123-调试与测试可见性与本地优先)
    - [1.2.4 SDK 的力量：语言生态集成](#124-sdk-的力量语言生态集成)
  - [1.3 视角二：运维现实 —— 运行可靠系统的成本](#13-视角二运维现实--运行可靠系统的成本)
    - [1.3.1 部署拓扑：自建集群 vs. Temporal Cloud](#131-部署拓扑自建集群-vs-temporal-cloud)
    - [1.3.2 基础设施依赖：不仅仅是计算](#132-基础设施依赖不仅仅是计算)
    - [1.3.3 伸缩性考量：Server 与 Worker 分离](#133-伸缩性考量server-与-worker-分离)
    - [1.3.4 可观测性：理解“黑盒”内部](#134-可观测性理解黑盒内部)
    - [1.3.5 升级与维护：演进中的系统](#135-升级与维护演进中的系统)
  - [1.4 视角三：分布式原语 —— 被封装的基础能力](#14-视角三分布式原语--被封装的基础能力)
    - [1.4.1 可靠状态存储：超越数据库字段](#141-可靠状态存储超越数据库字段)
    - [1.4.2 精确一次/至少一次执行：屏蔽投递语义](#142-精确一次至少一次执行屏蔽投递语义)
    - [1.4.3 分布式锁与信号量（隐式）](#143-分布式锁与信号量隐式)
    - [1.4.4 可靠定时器：时间作为一等公民](#144-可靠定时器时间作为一等公民)
    - [1.4.5 故障检测与恢复：自动化处理](#145-故障检测与恢复自动化处理)
    - [1.4.6 Saga 模式的天然载体](#146-saga-模式的天然载体)
  - [1.5 多视角融合：理解 Temporal 的价值定位](#15-多视角融合理解-temporal-的价值定位)
  - [1.6 结论：不仅仅是工作流引擎](#16-结论不仅仅是工作流引擎)
...
## 1.2 视角一：开发者体验 —— 被“隐藏”的复杂性

对于开发者而言，Temporal 最吸引人的地方在于它极大地简化了构建可靠分布式应用的复杂度。

### 1.2.1 “魔法”代码：顺序指令背后的异步与持久化
...
## 1.3 视角二：运维现实 —— 运行可靠系统的成本

从运维角度看，Temporal 是一个强大的平台，但也带来了新的运维对象和挑战。

### 1.3.1 部署拓扑：自建集群 vs. Temporal Cloud
...
### 1.3.3 伸缩性考量：Server 与 Worker 分离

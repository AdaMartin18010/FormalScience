...
- [1. 向量数据库深度分析与评价](#1-向量数据库深度分析与评价)
  - [目录](#目录)
  - [1.1 概述](#11-概述)
  - [1.2 基本概念与定义](#12-基本概念与定义)
    - [1.2.1 向量嵌入(Vector Embedding)](#121-向量嵌入vector-embedding)
    - [1.2.2 向量相似度(Vector Similarity)](#122-向量相似度vector-similarity)
    - [1.2.3 向量索引(Vector Index)](#123-向量索引vector-index)
  - [1.3 向量数据库核心技术](#13-向量数据库核心技术)
    - [1.3.1 近似最近邻算法(ANN)](#131-近似最近邻算法ann)
    - [1.3.2 向量存储](#132-向量存储)
    - [1.3.3 分布式与扩展性](#133-分布式与扩展性)
  - [1.4 元模型与模型的关联关系](#14-元模型与模型的关联关系)
    - [1.4.1 元模型定义](#141-元模型定义)
    - [1.4.2 模型与元模型关系](#142-模型与元模型关系)
    - [1.4.3 元模型作用](#143-元模型作用)
  - [1.5 算法原理与形式化论证](#15-算法原理与形式化论证)
    - [1.5.1 HNSW算法形式化描述](#151-hnsw算法形式化描述)
    - [1.5.2 向量量化理论](#152-向量量化理论)
  - [1.6 主流向量数据库比较](#16-主流向量数据库比较)
  - [1.7 应用场景分析](#17-应用场景分析)
    - [1.7.1 检索增强生成(RAG)](#171-检索增强生成rag)
    - [1.7.2 语义搜索引擎](#172-语义搜索引擎)
    - [1.7.3 推荐系统](#173-推荐系统)
    - [1.7.4 多模态检索](#174-多模态检索)
  - [1.8 Rust实现示例](#18-rust实现示例)
    - [1.8.1 简易向量存储与检索](#181-简易向量存储与检索)
    - [1.8.2 HNSW索引简化实现](#182-hnsw索引简化实现)
...
### 1.2.1 向量嵌入(Vector Embedding)

向量嵌入是将语义信息转换为高维向量空间中的点，使语义相似的内容在向量空间中距离较近。

### 1.2.2 向量相似度(Vector Similarity)
